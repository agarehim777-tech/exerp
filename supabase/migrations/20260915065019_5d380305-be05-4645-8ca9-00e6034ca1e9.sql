...
DECLARE
  account_id uuid;
  code text := 'MAIN-' || upper(left(_tenant_id::text, 8));
  cur text := coalesce(nullif(_currency, ''), 'AZN');
BEGIN
  SELECT id INTO account_id FROM public.cash_accounts
   WHERE tenant_id = _tenant_id AND is_active = true AND currency = cur
   ORDER BY CASE WHEN account_no = code OR name = 'Əsas kassa' THEN 0 ELSE 1 END, created_at
   LIMIT 1;
  IF account_id IS NOT NULL THEN RETURN account_id; END IF;

  UPDATE public.cash_accounts
     SET is_active = true, updated_at = now()
   WHERE tenant_id = _tenant_id AND account_no = code
   RETURNING id INTO account_id;
  IF account_id IS NOT NULL THEN RETURN account_id; END IF;

  INSERT INTO public.cash_accounts(tenant_id, account_no, name, type, currency, opening_balance, is_active)
  VALUES (_tenant_id, code, 'Əsas kassa', 'cash', cur, 0, true)
  RETURNING id INTO account_id;
  RETURN account_id;
END;
$$;
...
BEGIN
  ...
  IF coalesce(_initial_payment,0) > 0 THEN
    ...
    IF resolved_account_id IS NULL THEN
      resolved_account_id := private.ensure_main_cash_account(_tenant_id, _currency);
    END IF;
...
BEGIN
  ...
  IF contract_record.order_id IS NOT NULL THEN
    ...
    IF account_id IS NULL THEN
      account_id := private.ensure_main_cash_account(_tenant_id, target_order.currency);
    END IF;
  ...
  ELSE
    IF account_id IS NULL THEN
      account_id := private.ensure_main_cash_account(_tenant_id, 'AZN');
    END IF;
