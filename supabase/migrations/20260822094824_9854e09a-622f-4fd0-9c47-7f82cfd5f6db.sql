...
BEGIN
  ...
  IF round(_principal - target_initial, 2) <= 0 THEN RAISE EXCEPTION 'invalid_financed_amount'; END IF;
...
BEGIN
  ...
  applied := LEAST(round(_amount, 2), remaining);

  IF contract_record.order_id IS NOT NULL THEN
    -- credit_contract_initial_payment trigger-i fərqi kassaya və sifarişə yazır
    UPDATE public.credit_contracts
  ...
  RETURN round(contract_record.required_initial - contract_record.initial_payment - applied, 2);
END;
$function$;
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'credit_not_found'; END IF;
  IF contract_record.status <> 'draft' THEN RAISE EXCEPTION 'credit_already_started'; END IF;
  IF round(contract_record.initial_payment, 2) < round(contract_record.required_initial, 2) THEN
    RAISE EXCEPTION 'credit_initial_payment_incomplete';
  END IF;
