...
-- Recover opening average costs from existing valued receipts. Zero-cost
-- historical rows remain visibly zero instead of inventing a financial value.
UPDATE public.stock_balances b SET avg_cost=x.avg_cost
FROM (
 SELECT tenant_id,warehouse_id,product_id,
  round(sum(abs(quantity)*unit_cost)/NULLIF(sum(abs(quantity)),0),6) avg_cost
 FROM public.stock_movements
 WHERE movement_type='receipt' AND quantity>0 AND unit_cost>0
 GROUP BY tenant_id,warehouse_id,product_id
) x WHERE b.tenant_id=x.tenant_id AND b.warehouse_id=x.warehouse_id AND b.product_id=x.product_id AND b.avg_cost=0;
...
SELECT q.tenant_id,q.warehouse_id,q.product_id,q.id,COALESCE(q.reference_type,'opening_balance'),q.reference_id,
 q.created_at,q.quantity,GREATEST(0,LEAST(q.quantity,q.on_hand-q.newer_qty)),q.unit_cost
FROM (
 SELECT m.*,b.on_hand,COALESCE(sum(m.quantity) OVER(PARTITION BY m.tenant_id,m.warehouse_id,m.product_id ORDER BY m.created_at DESC,m.id DESC ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING),0) newer_qty
 ...
 WHERE m.movement_type='receipt' AND m.quantity>0 AND m.unit_cost>0 AND m.product_id IS NOT NULL
) q
...
DECLARE s public.procurement_shipments%rowtype; r uuid; x record; m uuid; oq numeric; oc numeric; nc numeric; tv numeric:=0; je uuid; ia uuid; pa uuid;
BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM public.warehouses WHERE id=_warehouse AND tenant_id=s.tenant_id AND is_active) THEN RAISE EXCEPTION 'Seçilmiş anbar aktiv deyil'; END IF;
 ...
 VALUES(s.tenant_id,'GRN-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS'),s.id,_warehouse,_receipt_date,auth.uid()) RETURNING id INTO r;
 FOR x IN SELECT sl.*,pol.product_id,pol.product_sku,lc.unit_landed_cost,lc.landed_total FROM public.procurement_shipment_lines sl
  JOIN public.purchase_order_lines pol ON pol.id=sl.po_line_id JOIN public.procurement_landed_cost_lines lc ON lc.shipment_line_id=sl.id AND lc.costing_version=s.costing_version AND lc.is_approved WHERE sl.shipment_id=s.id
 LOOP
  IF x.product_id IS NULL OR x.received_qty<=0 THEN RAISE EXCEPTION 'Qəbul sətri düzgün deyil'; END IF;
  SELECT COALESCE(on_hand,0),COALESCE(avg_cost,0) INTO oq,oc FROM public.stock_balances WHERE tenant_id=s.tenant_id AND warehouse_id=_warehouse AND product_id=x.product_id FOR UPDATE;
  oq:=COALESCE(oq,0); oc:=COALESCE(oc,0); nc:=round(((oq*oc)+(x.received_qty*x.unit_landed_cost))/(oq+x.received_qty),6);
  INSERT INTO public.stock_balances(tenant_id,warehouse_id,product_id,on_hand,reserved,avg_cost) VALUES(s.tenant_id,_warehouse,x.product_id,x.received_qty,0,nc)
   ON CONFLICT(tenant_id,warehouse_id,product_id) DO UPDATE SET on_hand=public.stock_balances.on_hand+EXCLUDED.on_hand,avg_cost=EXCLUDED.avg_cost,updated_at=now();
  INSERT INTO public.stock_movements(tenant_id,warehouse_id,product_id,movement_type,quantity,unit_cost,reference_type,reference_id,note,created_by)
   VALUES(s.tenant_id,_warehouse,x.product_id,'receipt',x.received_qty,x.unit_landed_cost,'procurement_receipt',r,s.shipment_no||' yekun maya qəbulu',auth.uid()) RETURNING id INTO m;
  INSERT INTO public.inventory_cost_layers(tenant_id,warehouse_id,product_id,source_movement_id,source_type,source_id,received_at,original_qty,remaining_qty,unit_cost)
   VALUES(s.tenant_id,_warehouse,x.product_id,m,'procurement_receipt',r,_receipt_date,x.received_qty,x.received_qty,x.unit_landed_cost);
  INSERT INTO public.procurement_receipt_lines(receipt_id,shipment_line_id,product_id,po_line_id,lot_no,received_qty,unit_landed_cost,landed_total,stock_movement_id)
   VALUES(r,x.id,x.product_id,x.po_line_id,x.lot_no,x.received_qty,x.unit_landed_cost,x.landed_total,m); tv:=tv+x.landed_total;
 END LOOP;
 IF tv<=0 THEN RAISE EXCEPTION 'Təsdiqlənmiş maya sətri yoxdur'; END IF;
 ia:=public.gl_account_by_code(s.tenant_id,'2050'); pa:=public.gl_account_by_code(s.tenant_id,'2200');
 INSERT INTO public.journal_entries(tenant_id,entry_date,reference,description,source_type,source_id,created_by) VALUES(s.tenant_id,_receipt_date,s.shipment_no,'Maya ilə anbar qəbulu','procurement_receipt',r,auth.uid()) RETURNING id INTO je;
 INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,memo,line_no) VALUES(je,ia,round(tv,2),0,'Mal ehtiyatları',1),(je,pa,0,round(tv,2),'Təchizatçı və idxal borcu',2);
...
BEGIN
 ...
 IF _status='delivered' THEN
  ...
  FOR it IN SELECT * FROM public.order_items WHERE order_id=o.id ORDER BY line_no FOR UPDATE LOOP
   ...
   IF method='fifo' THEN
    FOR l IN SELECT * FROM public.inventory_cost_layers WHERE tenant_id=o.tenant_id AND product_id=it.product_id AND remaining_qty>0 ORDER BY received_at,id FOR UPDATE LOOP
     ...
     UPDATE public.stock_balances SET on_hand=on_hand-take,reserved=GREATEST(0,reserved-take),updated_at=now() WHERE tenant_id=o.tenant_id AND warehouse_id=l.warehouse_id AND product_id=it.product_id AND on_hand>=take;
     IF NOT FOUND THEN RAISE EXCEPTION 'Stok və maya qatı uyğun deyil: %',it.description; END IF;
     INSERT INTO public.stock_movements(tenant_id,warehouse_id,product_id,movement_type,quantity,unit_cost,reference_type,reference_id,note,created_by) VALUES(o.tenant_id,l.warehouse_id,it.product_id,'delivery',-take,l.unit_cost,'sales_order',o.id,o.order_no,auth.uid()) RETURNING id INTO mv;
   ...
   ELSE
    FOR b IN SELECT * FROM public.stock_balances WHERE tenant_id=o.tenant_id AND product_id=it.product_id AND on_hand>0 ORDER BY on_hand DESC,id FOR UPDATE LOOP
     EXIT WHEN need<=0; take:=LEAST(need,b.on_hand); UPDATE public.stock_balances SET on_hand=on_hand-take,reserved=GREATEST(0,reserved-take),updated_at=now() WHERE tenant_id=o.tenant_id AND warehouse_id=b.warehouse_id AND product_id=b.product_id;
     INSERT INTO public.stock_movements(tenant_id,warehouse_id,product_id,movement_type,quantity,unit_cost,reference_type,reference_id,note,created_by) VALUES(o.tenant_id,b.warehouse_id,it.product_id,'delivery',-take,b.avg_cost,'sales_order',o.id,o.order_no,auth.uid()) RETURNING id INTO mv;
     INSERT INTO public.sales_cost_allocations(tenant_id,order_id,order_item_id,warehouse_id,product_id,stock_movement_id,quantity,unit_cost,total_cost) VALUES(o.tenant_id,o.id,it.id,b.warehouse_id,it.product_id,mv,take,b.avg_cost,take*b.avg_cost);
     cogs:=cogs+take*b.avg_cost; need:=need-take;
    END LOOP;
   END IF;
  ...
  INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,memo,line_no) VALUES(je,ar,o.total,0,'Müştəri borcu',1),(je,rev,0,o.subtotal,'Satış gəliri',2),(je,vat,0,o.vat_total,'ƏDV',3); n:=4;
 ...
 ELSIF _status='cancelled' AND EXISTS(SELECT 1 FROM public.order_accounting_events WHERE order_id=o.id AND event_type='delivery') THEN
  ...
  FOR b IN SELECT * FROM public.sales_cost_allocations WHERE order_id=o.id AND reversed_at IS NULL FOR UPDATE LOOP
   ...
   UPDATE public.stock_balances SET avg_cost=((oldq*oldc)+(b.quantity*b.unit_cost))/(oldq+b.quantity),on_hand=oldq+b.quantity,updated_at=now() WHERE tenant_id=o.tenant_id AND warehouse_id=b.warehouse_id AND product_id=b.product_id;
  ...
  INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,memo,line_no) VALUES(je,rev,o.subtotal,0,'Gəlirin ləğvi',1),(je,vat,o.vat_total,0,'ƏDV ləğvi',2),(je,ar,0,o.total-paid,'Debitor ləğvi',3),(je,aa,0,paid,'Geri ödəniləcək məbləğ',4);
  IF cogs>0 THEN INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,memo,line_no) VALUES(je,ia,cogs,0,'Stok qaytarması',5),(je,ca,0,cogs,'Maya ləğvi',6); END IF;
  UPDATE public.journal_entries SET posted=true WHERE id=je; INSERT INTO public.order_accounting_events(tenant_id,order_id,event_type,journal_entry_id,amount,cogs,created_by) VALUES(o.tenant_id,o.id,'cancellation',je,o.total,cogs,auth.uid()); UPDATE public.orders SET status='cancelled',updated_at=now() WHERE id=o.id;
 ELSE
...
BEGIN
 ...
 cash:=COALESCE(a.gl_account_id,public.gl_account_by_code(o.tenant_id,CASE WHEN a.type='cash' THEN '1000' ELSE '1010' END)); contra:=public.gl_account_by_code(o.tenant_id,CASE WHEN o.status='delivered' THEN '1200' ELSE '2300' END);
...
-- A linked order is already recognized at delivery. Posting its invoice must
-- not duplicate revenue. Stand-alone/service invoices keep the normal entry.
...
BEGIN
 ...
 INSERT INTO public.journal_lines(entry_id,account_id,debit,credit,memo,line_no) VALUES(je,ar,inv.total,0,'Debitor borcu',1),(je,rev,0,inv.total-inv.vat_total,'Satış gəliri',2),(je,vat,0,inv.vat_total,'ƏDV öhdəliyi',3);
...
BEGIN
 ...
 -- Linked delivery accounting is reversed only by cancelling the order.
...
REVOKE ALL ON FUNCTION public.ensure_inventory_accounts(uuid),public.process_sales_order_status(uuid,text),public.register_order_payment(uuid,numeric,uuid),public.cancel_sales_invoice(uuid) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.receive_landed_cost_shipment(uuid,uuid,date),public.process_sales_order_status(uuid,text),public.register_order_payment(uuid,numeric,uuid),public.post_invoice_to_gl(uuid),public.cancel_sales_invoice(uuid) TO authenticated,service_role;
