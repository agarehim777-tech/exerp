-- Make seller bonuses follow real cash receipts and backfill receipts that were
-- recorded before the assignment was saved.
...
BEGIN
  ...
  SELECT o.tenant_id,o.id,a.id,ct.id,a.seller_name,a.rate,
    ...
    JOIN public.cash_transactions ct
     ...
     AND ct.direction='in' AND ct.category IN ('sales_payment','credit_payment','receivable_payment')
...
DECLARE o public.orders%rowtype; item jsonb; total_rate numeric := 0; seller text; seller_rate numeric;
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(_allocations) LOOP
    seller:=btrim(item->>'seller_name'); seller_rate:=COALESCE((item->>'rate')::numeric,0);
  ...
  UPDATE public.order_bonus_assignments SET effective_to=_effective_from-1
  ...
  DELETE FROM public.order_bonus_assignments a
   WHERE a.tenant_id=o.tenant_id AND a.order_id=o.id AND a.effective_from>=_effective_from
     AND NOT EXISTS (SELECT 1 FROM public.sales_bonus_entries e WHERE e.assignment_id=a.id);
  FOR item IN SELECT * FROM jsonb_array_elements(_allocations) LOOP
  ...
  END LOOP;
  PERFORM public.backfill_sales_bonus_for_order(o.id);
END $$;
...
BEGIN
  IF NEW.direction<>'in' OR NEW.category NOT IN ('sales_payment','credit_payment','receivable_payment') THEN RETURN NEW; END IF;
  SELECT * INTO o FROM public.orders WHERE tenant_id=NEW.tenant_id AND order_no=NEW.reference ORDER BY created_at DESC LIMIT 1;
  IF o.id IS NULL THEN RETURN NEW; END IF;
  FOR assignment IN SELECT * FROM public.order_bonus_assignments
    WHERE tenant_id=NEW.tenant_id AND order_id=o.id AND effective_from<=NEW.occurred_at::date
      AND (effective_to IS NULL OR effective_to>=NEW.occurred_at::date)
  LOOP
    INSERT INTO public.sales_bonus_entries(tenant_id,order_id,assignment_id,cash_transaction_id,seller_name,rate,payment_amount,bonus_amount,accrued_on,created_by)
    VALUES(NEW.tenant_id,o.id,assignment.id,NEW.id,assignment.seller_name,assignment.rate,NEW.amount,round(NEW.amount*assignment.rate/100,2),NEW.occurred_at::date,auth.uid())
    ON CONFLICT (cash_transaction_id,assignment_id) DO NOTHING;
  END LOOP;
  RETURN NEW;
END $$;
...
CREATE TRIGGER trg_accrue_sales_bonus AFTER INSERT OR UPDATE OF reference,amount,category,direction,occurred_at
  ON public.cash_transactions FOR EACH ROW EXECUTE FUNCTION public.accrue_sales_bonus_for_cash_transaction();
...
GRANT EXECUTE ON FUNCTION public.backfill_sales_bonus_for_order(uuid) TO authenticated,service_role;
