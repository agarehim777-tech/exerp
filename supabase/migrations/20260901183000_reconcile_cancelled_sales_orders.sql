-- A previous partial rollout may have marked an order cancelled before its
-- credit and cash lifecycle was compensated. Re-running cancellation must
-- reconcile those ledgers without restoring stock a second time.
...
BEGIN
  ...
  already_cancelled := target.status::text = 'cancelled';
  ...
  VALUES (
    ...
    jsonb_build_object('order_id', target.id, 'order_no', target.order_no, 'reason', trim(_reason),
      'was_delivered', delivered, 'reconciled', already_cancelled,
      'cash_reversals', to_jsonb(reversal_ids))
  );

  RETURN jsonb_build_object('order_id', target.id, 'status', 'cancelled',
    'stock_reversed', delivered, 'reconciled', already_cancelled,
    'cash_reversals', to_jsonb(reversal_ids));
END;
$$;
