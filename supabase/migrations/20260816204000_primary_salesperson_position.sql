-- The first salesperson on an order is the primary salesperson. Remaining
-- assignments are assistants for bonus purposes and do not own the sale KPI.
ALTER TABLE public.order_bonus_assignments
  ADD COLUMN IF NOT EXISTS position integer NOT NULL DEFAULT 1;

WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY tenant_id, order_id, effective_from
           -- Historical rows had no explicit position. The higher bonus rate
           -- is the safest indicator of the primary salesperson for that data.
           ORDER BY rate DESC, created_at, id
         ) AS next_position
    FROM public.order_bonus_assignments
)
UPDATE public.order_bonus_assignments assignment
   SET position = ranked.next_position
  FROM ranked
 WHERE ranked.id = assignment.id;

CREATE INDEX IF NOT EXISTS order_bonus_assignments_position_idx
  ON public.order_bonus_assignments(tenant_id, order_id, effective_from, position);
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(_allocations) LOOP
    ...
    IF seller = '' OR seller_rate <= 0 OR seller_rate > 100 THEN
      RAISE EXCEPTION 'Satıcı və bonus faizi düzgün deyil';
    END IF;
    total_rate := total_rate + seller_rate;
  END LOOP;
  IF total_rate > 100 THEN
    RAISE EXCEPTION 'Ümumi bonus faizi 100%%-dən çox ola bilməz';
  END IF;
