...
import {
  ...
  Settings,
  ShieldCheck,
  ShoppingCart,import { CompanyModulePicker, LoginScreen, PasswordChangeScreen } from "./components/AuthScreens.jsx";
...
function App() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      .from("audit_events")
      taxCalendar: filterRows(taxCalendarRows, query),      apiWebhooks: filterRows(apiWebhookRows, query),
    ...
    [
      ...
      apiWebhookRows,      setToasts((items) => items.filter((item) => item.id !== id));
    }, 3200);
  }
    ...
    setState((current) => {
      ...
      if (nextAction === "payout") {
        ...
        nextPeriod = {
          ...baseSnapshot,
          status: "Period bağlandı",
          approvalStatus: "Təsdiq edildi",        audit = {
          module: "KPI",
  ...
  async function startCredit(creditId, startDate) {
    ...
    try {
      if (!activeTenantId || !targetCredit.id) throw new Error("Kreditin database bağlantısı tapılmadı.");
      await startCreditContract({ tenantId: activeTenantId, creditId: targetCredit.id, startDate });
      await refreshDbOrders();
    } catch (error) {
      notify(`Kredit başladılmadı: ${error.message || error}`, "error");
      return false;
    }
