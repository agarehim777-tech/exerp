...
function buildSalesCreditRecord(order, storedCredit) {
  ...
  return {
    ...
    next: storedCredit?.next || basePlan.installments[0]?.due || "вЂ”",
    status,
  ...
  return orders.flatMap((order) => {
    ...
    return getOrderSellerBonuses(order).map((sellerBonus) => {
      ...
      return {
        ...
        paymentMethod: order.paymentMethod || "NaДџd",
...
function buildKpiEmployeeScoreRows(employees = [], salesBonuses = []) {
  ...
  return buildHrEmployeeRecords(employees).map((employee) => {
    ...
    return {
      ...
      performanceBonus,
      payoutAmount,
      payoutStatus: payoutAmount > 0 ? "Payout hazД±r" : Number(employee.kpi || 0) < 95 ? "HЙ™dЙ™fdЙ™n aЕџaДџД±" : "Д°zlЙ™mЙ™",
    };
  });
}
...
function buildKpiTargetRows({ targets = [], employees = [], employeeRows = [], salesBonuses = [] } = {}) {
  return ensureKpiTargets(targets).map((target) => {
    ...
    return {
      ...
      progress,
      weightedScore,
      status: progress >= 100 ? "TamamlandД±" : progress >= 75 ? "NЙ™zarЙ™t" : "Risk",
    };
  });
}
...
function buildKpiPeriodSnapshot({ period, targetRows = [], employeeRows = [], salesBonuses = [], existing = {}, stamp = "" } = {}) {
  ...
  return {
    ...
    period,
    status: existing.status || "AГ§Д±q period",
    approvalStatus: existing.approvalStatus || "HazД±rlanД±r",
    payoutStatus: existing.payoutStatus || "GГ¶zlЙ™yir",
    companyScore,
...
function getOrderProductRows(order) {
  if (Array.isArray(order.productLines) && order.productLines.length > 0) {
    return order.productLines.map((line) => {
      ...
      return {
        product: line.product || order.products || "Cihaz qeyd edilmЙ™yib",
        qty,
        amount,
  ...
  return [
    {
      product: order.products || "Cihaz qeyd edilmЙ™yib",
      qty: 1,
...
function getInstallmentStatus(installment) {
  if (Number(installment.amount || 0) <= 0) return "Г–dЙ™nilib";
  const dueDate = parsePaymentDate(installment.due);
  const today = parsePaymentDate(baseCreditDate);
  if (dueDate && today && daysBetween(dueDate, today) > 0) return "Gecikib";
  if (dueDate && today && daysBetween(dueDate, today) === 0) return "Bu gГјn";
  return "GГ¶zlЙ™yir";
}
...
function buildCustomer360(customer, { credits = [], orders = [], contracts = [] }) {
  ...
  const creditAgreements = customerCredits.map((credit) => {
    ...
    const productLines =
      order
        ...
        : [
            {
              product: credit.device || credit.product || contract?.product || "Cihaz qeyd edilmЙ™yib",
              qty: 1,
    ...
    return {
      ...
      type: "Kredit mГјqavilЙ™si",
      source: getCreditSourceLabel(credit),
      contractId: credit.contractId || contract?.id || "MГјqavilЙ™siz",
      orderId: credit.orderId || order?.id || contract?.orderId || "вЂ”",
      creditId: credit.id,
      ...
      nextDue: paymentState.nextInstallment?.due || credit.next || "вЂ”",
  ...
  const creditedOrderIds = new Set(creditAgreements.map((agreement) => agreement.orderId).filter((id) => id && id !== "вЂ”"));
  const usedContractIds = new Set(
    creditAgreements.map((agreement) => agreement.contractId).filter((id) => id && id !== "MГјqavilЙ™siz"),
  );
  const directSaleAgreements = customerOrders
    ...
    .map((order) => {
      ...
      return {
        ...
        source: "SatД±Еџ modulu",
        contractId: order.contractId || contract?.id || "MГјqavilЙ™siz",
        orderId: order.id,
        creditId: "вЂ”",
        ...
        nextDue: balance > 0 ? order.dueDate || "RazД±laЕџdД±rД±lmayД±b" : "вЂ”",
        status: balance > 0 ? "QalД±qlД± satД±Еџ" : "Г–dЙ™nilib",
        overdueDays: 0,
        plan: null,
        paymentState: null,
        payments: [],
  ...
  const standaloneContracts = customerContracts
    ...
    .map((contract) => ({
      ...
      type: "MГјqavilЙ™",
      source: "MГјqavilЙ™ modulu",
      contractId: contract.id,
      orderId: contract.orderId || "вЂ”",
      creditId: "вЂ”",
      date: contract.date || currentBusinessDate,
      product: contract.product || "Cihaz qeyd edilmЙ™yib",
      productLines: [{ product: contract.product || "Cihaz qeyd edilmЙ™yib", qty: 1, amount: Number(contract.amount || 0), serials: [] }],
      ...
      remainingMonths: 0,
      nextDue: "вЂ”",
      status: contract.status || "HazД±rlanД±r",
      overdueDays: 0,
      plan: null,
      paymentState: null,
      payments: [],
  ...
  const paymentRows = customerCredits
    .flatMap((credit) =>
      (credit.payments || []).map((payment) => ({
        ...
        product: credit.device || credit.product || "Cihaz qeyd edilmЙ™yib",
      })),
    )
  ...
  const deviceRows = customerOrders.flatMap((order) => {
    ...
    return lineRows.map((line) => {
      ...
      return {
        ...
        orderId: order.id,
        contractId: order.contractId || contract?.id || linkedCredit?.contractId || "вЂ”",
        creditId: linkedCredit?.id || "вЂ”",
        date: order.date,
        status: order.status,
  ...
  return {
    ...
    totalPaid,
    totalBalance,
    activeCreditCount: creditAgreements.filter((agreement) => !normalize(agreement.status).includes("baДџlan")).length,
    overdueCount: overdueAgreements.length,
    openOrders: customerOrders.filter((order) => order.status !== "TЙ™hvil verilib").length,
    nextPayment,
  };
}
...
function buildCrmPipelineRows(customers, credits, orders) {
  return customers.map((customer, index) => {
    ...
    const openOrders = customerOrders.filter((order) => order.status !== "TЙ™hvil verilib").length;
      ...
      : hasDeliveryFollowUp
        ...
        : customer.category === "Platin"
          ...
          : limitLeft > 3000
            ...
            : "Kredit uyДџunluДџu";
    const probability =
      stage === "Upsell" ? 82 : stage === "TЙ™klif" ? 68 : stage === "TЙ™hvil sonrasД±" ? 56 : stage === "Risk follow-up" ? 34 : 46;
    const owner = latestOrder?.sellerBonuses?.[0]?.seller || latestOrder?.seller || "TЙ™yin edilmЙ™yib";
    ...
    return {
      ...
      value,
      probability,
      source: latestOrder ? `Son sifariЕџ: ${latestOrder.id}` : customer.category,
      nextAction:
        stage === "Risk follow-up"
          ? "GecikmЙ™ ГјzrЙ™ zЙ™ng vЙ™ SMS"
          : stage === "TЙ™hvil sonrasД±"
            ? "TЙ™hvil sonrasД± mЙ™mnunluq zЙ™ngi"
            : stage === "Upsell"
              ? "Premium cihaz tЙ™klifi"
              : stage === "TЙ™klif"
                ? "LimitЙ™ uyДџun kommersiya tЙ™klifi"
                : "AKB vЙ™ limit yoxlamasД±",
      activeCreditCount,
      totalBalance,
      openOrders,
      limitLeft,
      nextPayment,
      lastOrderId: latestOrder?.id || "Yeni fГјrsЙ™t",
    };
  });
}

function matchesCrmPipelineFilter(row, filter) {
  return filter === "HamД±sД±" || row.stage === filter;
}
...
function matchesCrmCustomerSegment(entry, segment) {
  ...
  if (segment === "GecikmЙ™") return profile.overdueCount > 0 || Number(customer.delay || 0) > 0;
  if (segment === "AГ§Д±q tЙ™hvil") return profile.openOrders > 0;
...
function buildCreditRiskRows(enrichedCredits) {
  return enrichedCredits
    .map((item) => {
      ...
      const level = score >= 75 ? "YГјksЙ™k risk" : score >= 45 ? "NЙ™zarЙ™t" : "SaДџlam";
      const bucket =
        overdueDays === 0 ? "VaxtД±nda" : overdueDays <= 7 ? "1-7 gГјn" : overdueDays <= 30 ? "8-30 gГјn" : "30+ gГјn";
      ...
      return {
        ...
        recommendedAction:
          level === "YГјksЙ™k risk"
            ? "Kollektor zЙ™ngi + restruktur tЙ™klifi"
            : level === "NЙ™zarЙ™t"
              ? "SMS vЙ™ Г¶dЙ™mЙ™ linki"
              : "Avtomatik xatД±rlatma",
      };
    })
...
function buildCreditAgingBuckets(riskRows) {
  const buckets = ["VaxtД±nda", "1-7 gГјn", "8-30 gГјn", "30+ gГјn"];
...
export function isPurchaseOrderOpen(po = {}) {
  ...
  return (
    ...
    !status.includes("Г¶dЙ™n") &&
    !status.includes("oden") &&
    !status.includes("baДџ") &&
    !status.includes("bag") &&
...
export function buildWarehouseWmsRows(items, products = []) {
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      cycleCount: index % 3 === 0 ? "Bu hЙ™ftЙ™" : index % 3 === 1 ? "NГ¶vbЙ™ti hЙ™ftЙ™" : "AylД±q",
      available,
      reserved: Number(item.reserved || 0),
      serialSummary,
      sampleSerial: item.serials?.find((serial) => serial.status !== "SatД±lД±b")?.imei || "Batch",
      reorderPoint,
      reorderQty,
      status: reorderQty > 0 ? "SifariЕџ aГ§" : available <= reorderPoint + 2 ? "NЙ™zarЙ™t" : "Normal",
    };
  });
}
...
function getPreferredVendorName(product, vendors) {
  ...
  return vendors[0]?.name || "Vendor tЙ™yin edilmЙ™yib";
}
...
function normalizeVendor(values = {}, fallback = {}) {
  ...
  return {
    ...
    paymentTerms: String(values.paymentTerms ?? fallback.paymentTerms ?? "30 gГјn").trim(),
...
export function buildFinanceScenario({ orders, expenses, credits, cashEntries, openingBalance = 0 }) {
  ...
  const approvedExpense = expenses
    .filter((expense) => expense.status === "TЙ™sdiq edildi" && hasExpenseCashImpact(expense))
    .reduce((sum, expense) => sum + Number(expense.amount || 0), 0);
  const pendingExpense = expenses
    .filter((expense) => expense.status === "TЙ™sdiq gГ¶zlЙ™yir" && hasExpenseCashImpact(expense))
    .reduce((sum, expense) => sum + Number(expense.amount || 0), 0);
    ...
    const status =
      balance <= 0
        ...
        : Number(order.paid || 0) > 0
          ? "QismЙ™n Г¶dЙ™nilib"
          : "Г–dЙ™niЕџ gГ¶zlЙ™yir";

    return {
      id: `${prefix}-${String(index + 1).padStart(4, "0")}`,
      orderId: order.id,
      contractId: order.contractId || "вЂ”",
      customer: order.customer,
      fin: order.fin,
      seller: settings.company || "ERP+CRM AZ",
      voen: settings.voen || "вЂ”",
      date: order.date || currentBusinessDate,
      dueDate,
      ...
      eTaxStatus: order.eTaxStatus || (balance <= 0 ? "E-qaimЙ™ gГ¶ndЙ™rildi" : "GГ¶ndЙ™riЕџЙ™ hazД±r"),
...
function buildInvoiceSummary(invoices) {
  ...
  return {
    ...
    ready: invoices.filter((invoice) => invoice.eTaxStatus === "GГ¶ndЙ™riЕџЙ™ hazД±r").length,
  };
}

function getInvoiceAgingBucket(invoice) {
  if (Number(invoice.balance || 0) <= 0) return "Г–dЙ™nilib";
  ...
  if (days <= 0) return "VaxtД±nda";
  if (days <= 7) return "1-7 gГјn";
  if (days <= 30) return "8-30 gГјn";
  return "30+ gГјn";
}

function buildInvoiceAgingRows(invoices) {
  const buckets = ["VaxtД±nda", "1-7 gГјn", "8-30 gГјn", "30+ gГјn", "Г–dЙ™nilib"];
...
function buildInvoiceControlSummary(invoices) {
  ...
  return {
    sent: invoices.filter((invoice) => invoice.invoiceSentAt || invoice.eTaxStatus === "E-qaimЙ™ gГ¶ndЙ™rildi").length,
    ready: invoices.filter((invoice) => invoice.eTaxStatus === "GГ¶ndЙ™riЕџЙ™ hazД±r").length,
...
function buildCurrencyExposureRows({ currencyRates = [], orders, credits, cashEntries }) {
  ...
  return currencyRates.map((rate) => {
    ...
    return {
      ...
      status: rate.code === "AZN" ? "Baza" : Math.abs(Number(rate.change || 0)) >= 0.3 ? "NЙ™zarЙ™t" : rate.status,
    };
  });
}
...
function buildAccountingData({ orders, expenses, cashEntries, credits, stock, invoices, openingBalance = 0 }) {
  ...
  const pendingExpenses = expenses.filter((expense) => expense.status === "TЙ™sdiq gГ¶zlЙ™yir");
  ...
  const journalRows = [
    ...invoices.slice(0, 8).map((invoice) => ({
      ...
      credit: "SatД±Еџ gЙ™liri / ЖЏDV",
      amount: invoice.totalAmount,
      status: invoice.balance > 0 ? "AГ§Д±q" : "BaДџlandД±",
    })),
    ...cashEntries.slice(0, 4).map((entry) => ({
      ...
      debit: "Kassa",
      credit: Number(entry.penalty || 0) > 0 ? "Debitor / GecikmЙ™ gЙ™liri" : "Debitor",
      amount: entry.amount,
      status: "Kassa yazД±ldД±",
    })),
    ...expenses.slice(0, 6).map((expense) => ({
      ...
      credit: expense.status === "TЙ™sdiq edildi" && hasExpenseCashImpact(expense) ? "Kassa" : "Kreditor",
      amount: expense.amount,
      status: hasExpenseCashImpact(expense) ? expense.status : "Cash tЙ™siri yoxdur",
    })),
  ];
  ...
  const chartRows = [
    ...
    { code: "2050", account: "Mal ehtiyatlarД±", type: "Aktiv", debit: inventory, credit: 0, balance: inventory },
    { code: "4310", account: "ЖЏDV Г¶hdЙ™liyi", type: "Г–hdЙ™lik", debit: 0, credit: vatPayable, balance: vatPayable },
    { code: "5310", account: "Kreditor borclar", type: "Г–hdЙ™lik", debit: 0, credit: pendingExpenseTotal, balance: pendingExpenseTotal },
    { code: "6010", account: "SatД±Еџ gЙ™liri", type: "GЙ™lir", debit: 0, credit: salesNet, balance: salesNet },
    { code: "7010", account: "Maya dЙ™yЙ™ri", type: "XЙ™rc", debit: costOfGoods, credit: 0, balance: costOfGoods },
    { code: "7310", account: "ЖЏmЙ™liyyat xЙ™rclЙ™ri", type: "XЙ™rc", debit: approvedExpenseTotal, credit: 0, balance: approvedExpenseTotal },
  ];
...
function buildAccountingCloseChecklist(accounting, closeRun) {
  ...
  const checks = [
    {
      label: "Balans bЙ™rabЙ™rliyi",
      detail: `Aktiv - Г¶hdЙ™lik - kapital = ${money(equationDiff)}`,
      status: Math.abs(equationDiff) <= 1 ? "TamamlandД±" : "YoxlanmalД±dД±r",
    },
    {
      label: "Kassa uzlaЕџmasД±",
      detail: `1010 hesabД± ilЙ™ cash-flow fЙ™rqi ${money(cashDiff)}`,
      status: Math.abs(cashDiff) <= 1 ? "TamamlandД±" : "YoxlanmalД±dД±r",
    },
    {
      label: "Jurnal yazД±lД±ЕџlarД±",
      detail: `${journalRows.length} jurnal sЙ™tri formalaЕџД±b`,
      status: journalRows.length > 0 ? "TamamlandД±" : "GГ¶zlЙ™yir",
    },
    {
      label: "ЖЏDV Г¶hdЙ™liyi",
      detail: `${money(accounting.vatPayable)} Г¶hdЙ™lik hesablandД±`,
      status: Number(accounting.vatPayable || 0) >= 0 ? "TamamlandД±" : "YoxlanmalД±dД±r",
    },
    {
      label: "Ay baДџlanД±ЕџД±",
      detail: closeRun ? `${closeRun.period} export edilib` : "Jurnal export gГ¶zlЙ™yir",
      status: closeRun ? "TamamlandД±" : "GГ¶zlЙ™yir",
    },
  ];

  return {
    checks,
    readyCount: checks.filter((check) => check.status === "TamamlandД±").length,
    warningCount: checks.filter((check) => check.status !== "TamamlandД±").length,
    equationDiff,
    cashDiff,
    closeReady: checks.every((check) => check.status === "TamamlandД±"),
...
function getTaxAmountByType(type, { invoices, payrollTaxRows, accounting }) {
  ...
  if (normalized.includes("Й™dv")) return total(invoices, "vatAmount");
  ...
  if (normalized.includes("mЙ™nfЙ™Й™t")) return Math.max(0, Math.round(Number(accounting.pl.netProfit || 0) * 0.2));
  if (normalized.includes("sadЙ™lЙ™ЕџdirilmiЕџ")) return Math.round(Number(accounting.pl.revenue || 0) * 0.02);
  return 0;
}
...
function buildDefaultTaxCalendarItems({ invoices, payrollTaxRows, accounting }) {
  ...
  const profitAmount = getTaxAmountByType("MЙ™nfЙ™Й™t", { invoices, payrollTaxRows, accounting });
  const period = currentBusinessDate.slice(0, 7);

  return [
    {
      id: `TAX-AUTO-VAT-${period}`,
      title: "ЖЏDV bЙ™yannamЙ™si",
      type: "ЖЏDV",
      period,
    ...
    },
    {
      ...
      title: "MЙ™nfЙ™Й™t vergisi avans",
      type: "MЙ™nfЙ™Й™t",
...
function buildTaxCalendarRows({ taxCalendar = [], invoices, payrollTaxRows, accounting }) {
  ...
  return sourceRows.map((item) => {
    ...
    const status =
      item.paymentStatus ||
      (daysLeft < 0 ? "Gecikib" : daysLeft === 0 ? "Bu gГјn" : daysLeft <= 7 ? "YaxД±nlaЕџД±r" : "PlanlД±");
...
function getApiSecretHealth(secret = {}) {
  if (!secret.key) return { daysLeft: 0, status: "Secret tapД±lmadД±" };
  ...
  const status = daysLeft <= 0 ? "Rotasiya gecikib" : daysLeft <= 7 ? "Rotasiya yaxД±nlaЕџД±r" : "Aktiv";
...
function buildApiSecretRows(apiSecrets = [], apiWebhooks = []) {
  ...
  return ensureApiSecrets(apiSecrets).map((secret) => {
    ...
    return {
      ...
      linkedEvents: linkedEndpoints.map((webhook) => webhook.event).join(", ") || "BaДџlД± endpoint yoxdur",
...
function buildApiWebhookRows({ apiWebhooks = [], apiSecrets = [], apiWebhookLogs = [], invoices = [], credits = [], stock = [], products = [], purchaseOrders = [], expenses = [] }) {
  ...
  const eventCounts = {
    ...
    "po.approved": (purchaseOrders || []).filter((po) => po.status === "TЙ™sdiq edildi").length,
  ...
  return ensureApiWebhooks(apiWebhooks).map((webhook) => {
    ...
    const secretHealth = secret?.health || "Secret tapД±lmadД±";
    const derivedPayload =
      webhook.event === "credit.overdue"
        ? overdueCredits[0]?.id || "NГ¶vbЙ™ boЕџdur"
        : webhook.event === "product.low_stock"
          ? lowStock[0]?.product || "Stok normaldД±r"
          : webhook.event === "invoice.paid"
            ? invoices.find((invoice) => invoice.balance <= 0)?.id || "Г–dЙ™nilmiЕџ faktura yoxdur"
            : webhook.event === "po.approved"
              ? (purchaseOrders || []).find((po) => po.status === "TЙ™sdiq edildi")?.id || "TЙ™sdiqli PO yoxdur"
              : "HazД±r";
    ...
    return {
      ...
      retryQueue,
      retryMax,
      authLabel: `${webhook.authType || "HMAC"} В· ${secret?.maskedValue || webhook.secretKey || "Secret yoxdur"}`,
      secretStatus: secretHealth,
      lastResponseCode: webhook.lastResponseCode || lastLog?.responseCode || "вЂ”",
      ...
      retryState:
        retryQueue > 0
          ...
          : Number(webhook.failureCount || 0) >= retryMax
            ...
            : "Normal",
      health:
        secretHealth.includes("tapД±lmadД±") || secretHealth.includes("gecikib")
          ? "Secret riski"
          : lastLog?.result === "UДџurlu" || webhook.lastTestAt
            ? "Test OK"
...
function buildTodayActionRows({ credits, orders, expenses, stock, products = [], invoices, taxRows }) {
  const creditActions = credits
    ...
    .map((item) => ({
      ...
      module: "credits",
      title: item.paymentState.isOverdue ? "GecikЙ™n kredit" : "Bu gГјn kredit Г¶dЙ™niЕџi",
      detail: `${item.credit.customer} В· ${item.credit.contractId || item.credit.id}`,
      amount: Number(item.paymentState.nextInstallment?.amount || item.plan.monthly || 0),
      priority: item.paymentState.isOverdue ? "YГјksЙ™k" : "Orta",
      status: item.paymentState.isOverdue ? `${item.paymentState.daysOverdue} gГјn gecikib` : "Bu gГјn",
      icon: CreditCard,
    }));

  const deliveryActions = orders
    .filter((order) => order.status !== "TЙ™hvil verilib" && getDeliveryRisk(enrichDeliveryOrder(order)) !== "Normal")
    .slice(0, 5)
    .map((order) => ({
      id: `ACT-${order.id}`,
      module: "deliveries",
      title: "TЙ™hvil nЙ™zarЙ™ti",
      detail: `${order.id} В· ${order.customer} В· ${summarizeOrderProducts(order)}`,
      amount: getOrderBalance(order),
      priority: getDeliveryAgeDays(order) >= 5 ? "YГјksЙ™k" : "Orta",
      status: getDeliveryRisk(order),
      icon: Truck,
    }));

  const expenseActions = expenses
    .filter((expense) => expense.status === "TЙ™sdiq gГ¶zlЙ™yir")
    .map((expense) => ({
      id: `ACT-${expense.id}`,
      module: "finance",
      title: "XЙ™rc tЙ™sdiqi",
      detail: `${expense.description} В· ${expense.category}`,
      amount: Number(expense.amount || 0),
      priority: Number(expense.amount || 0) >= 5000 ? "YГјksЙ™k" : "Orta",
  ...
  const stockActions = stock
    ...
    .map((item) => ({
      ...
      module: "warehouse",
      title: "AЕџaДџД± stok",
      detail: `${item.product} В· satД±Еџ ГјГ§Гјn ${getAvailableQuantity(item)} Й™dЙ™d`,
      amount: getAvailableQuantity(item) * Number(item.price || 0),
      priority: getAvailableQuantity(item) <= 3 ? "YГјksЙ™k" : "Orta",
      status: "PO/transfer lazД±mdД±r",
      icon: Package,
    }));
  ...
  const invoiceActions = invoices
    ...
    .map((invoice) => ({
      id: `ACT-${invoice.id}`,
      module: "invoices",
      title: "AГ§Д±q faktura",
      detail: `${invoice.id} В· ${invoice.customer}`,
      amount: Number(invoice.balance || 0),
      priority: invoice.status === "Г–dЙ™niЕџ gГ¶zlЙ™yir" ? "Orta" : "AЕџaДџД±",
      status: invoice.status,
      icon: FileText,
    }));

  const taxActions = taxRows
    .filter((row) => row.status === "Bu gГјn" || row.status === "Gecikib" || row.status === "YaxД±nlaЕџД±r")
    .map((row) => ({
      id: `ACT-${row.id}`,
      module: "tax",
      title: "Vergi Г¶hdЙ™liyi",
      detail: `${row.title} В· ${row.period}`,
      amount: Number(row.amount || 0),
      priority: row.status === "Gecikib" || row.status === "Bu gГјn" ? "YГјksЙ™k" : "Orta",
      status: row.status,
      icon: CalendarClock,
    }));

  const priorityOrder = { "YГјksЙ™k": 0, Orta: 1, "AЕџaДџД±": 2 };
...
function buildExecutiveInsights({ orders, credits, vendors, employees }) {
  ...
  const vendorAtRisk = vendors.filter((vendor) => normalize(vendor.status).includes("risk") || normalize(vendor.status).includes("aЕџaДџД±"));
  const openDelivery = orders.filter((order) => order.status !== "TЙ™hvil verilib").length;

  return [
    {
      title: "Kredit portfeli",
      value: `${overdueCount} risk`,
      desc: overdueCount > 0 ? "GecikЙ™n mГјЕџtЙ™rilЙ™r ГјzrЙ™ kollektor nГ¶vbЙ™si yaradД±n" : "GecikmЙ™ siqnalД± yoxdur",
      tone: overdueCount > 0 ? "danger" : "success",
    },
    {
      title: "TЙ™hvil yГјkГј",
      value: `${openDelivery} sifariЕџ`,
      desc: "Anbar Г§Д±xД±ЕџД± vЙ™ sГјrГјcГј planД± ilЙ™ izlЙ™nir",
      tone: openDelivery > 4 ? "warning" : "info",
    },
    {
      title: "Vendor riski",
      value: vendorAtRisk.length,
      desc: "Kvota vЙ™ tЙ™chizat ГјzrЙ™ zЙ™if vendorlar",
      tone: vendorAtRisk.length > 0 ? "warning" : "success",
    },
    {
      title: "HR/KPI",
      value: topSeller?.name || "Yoxdur",
      desc: topSeller ? `${topSeller.kpi}% KPI ilЙ™ lider` : "KPI datasД± yoxdur",
      tone: "primary",
    },
  ];
}
...
function buildReportModuleRows({
  ...
  const openDeliveries = orders.filter((order) => order.status !== "TЙ™hvil verilib");
  ...
  const approvedPoAmount = (purchaseOrders || [])
    .filter((po) => po.status === "TЙ™sdiq edildi")
  ...
  return [
    {
      ...
      count: `${orders.length} sifariЕџ`,
      signal: openDeliveries.length > 0 ? `${openDeliveries.length} tЙ™hvil gГ¶zlЙ™yir` : "BaДџlД± axД±n",
      status: openDeliveries.length > 5 ? "NЙ™zarЙ™t" : "HazД±r",
    },
    {
      module: "Kredit",
      metric: money(creditBalance),
      count: `${creditRows.length} mГјqavilЙ™`,
      signal: overdueCredits.length > 0 ? `${overdueCredits.length} gecikmЙ™` : "GecikmЙ™ yoxdur",
      status: overdueCredits.length > 0 ? "Risk" : "HazД±r",
    },
    {
      module: "Anbar",
      metric: `${availableStock}/${totalStock}`,
      count: `${products.length} SKU`,
      signal: lowStockRows.length > 0 ? `${lowStockRows.length} minimum qalД±q` : "Stok saДџlam",
      status: lowStockRows.length > 0 ? "NЙ™zarЙ™t" : "HazД±r",
    },
    {
      module: "Vendor / PO",
      metric: money(approvedPoAmount),
      count: `${(purchaseOrders || []).length} PO`,
      signal: openPos.length > 0 ? `${openPos.length} tЙ™sdiqdЙ™` : `${vendors.length} vendor aktiv`,
      status: openPos.length > 0 ? "NЙ™zarЙ™t" : "HazД±r",
    },
    {
      module: "MaliyyЙ™",
      metric: money(cashInflow),
      count: `${expenses.length} xЙ™rc`,
      signal: pendingExpenses.length > 0 ? `${pendingExpenses.length} xЙ™rc tЙ™sdiqdЙ™` : `${money(total(approvedExpenses, "amount"))} tЙ™sdiqli xЙ™rc`,
      status: pendingExpenses.length > 0 ? "NЙ™zarЙ™t" : "HazД±r",
    },
    {
      module: "Faktura",
      metric: money(invoiceControl.openBalance),
      count: `${invoices.length} faktura`,
      signal: invoiceControl.overdueCount > 0 ? `${invoiceControl.overdueCount} gecikmiЕџ faktura` : `${invoiceControl.ready} e-qaimЙ™ hazД±r`,
      status: invoiceControl.overdueCount > 0 || invoiceControl.openBalance > 0 ? "NЙ™zarЙ™t" : "HazД±r",
    },
    {
      module: "HR",
      metric: money(payrollCost),
      count: `${employees.length} Й™mЙ™kdaЕџ`,
      signal: documentGaps.length > 0 ? `${documentGaps.length} sЙ™nЙ™d aГ§Д±ДџД±` : "Payroll hazД±r",
      status: documentGaps.length > 0 ? "NЙ™zarЙ™t" : "HazД±r",
    },
  ];
}
...
function buildReportRiskRows({
  ...
  credits.forEach((credit) => {
    ...
    riskRows.push({
      ...
      action: paymentState.isOverdue ? `${paymentState.daysOverdue} gГјn gecikib` : "Bu gГјn Г¶dЙ™niЕџ",
      priority: paymentState.isOverdue ? "YГјksЙ™k" : "Orta",
      status: paymentState.isOverdue ? "GecikmЙ™" : "Bu gГјn",
    });
  });

  orders
    .filter((order) => order.status !== "TЙ™hvil verilib")
    .slice(0, 8)
    .forEach((order) => {
      riskRows.push({
        id: order.id,
        area: "SatД±Еџ/TЙ™hvil",
        title: order.customer,
        amount: getOrderBalance(order),
        owner: order.warehouseName || order.seller || "Anbar",
        action: summarizeOrderProducts(order),
        priority: "Orta",
        status: "TЙ™hvil gГ¶zlЙ™yir",
      });
    });
  ...
  stockRows
    ...
    .forEach((item) => {
      ...
      riskRows.push({
        ...
        action: `${getAvailableQuantity(item)} qalД±q / min ${reorderPoint}`,
        priority: getAvailableQuantity(item) === 0 ? "YГјksЙ™k" : "Orta",
        status: "Minimum qalД±q",
      });
    });
  ...
  (purchaseOrders || [])
    ...
    .forEach((po) => {
      riskRows.push({
        ...
        owner: po.vendor,
        action: `${po.qty || 0} Й™dЙ™d sifariЕџ`,
        priority: po.status === "TЙ™sdiq gГ¶zlЙ™yir" ? "Orta" : "AЕџaДџД±",
        status: po.status || "AГ§Д±q PO",
      });
    });
  ...
  expenses
    ...
    .forEach((expense) => {
      riskRows.push({
        ...
        action: expense.cashImpact === false ? "Cash tЙ™siri yoxdur" : "TЙ™sdiq lazД±mdД±r",
        priority: Number(expense.amount || 0) >= 5000 ? "YГјksЙ™k" : "Orta",
        status: expense.status,
      });
    });
  ...
  invoices
    ...
    .forEach((invoice) => {
      ...
      riskRows.push({
        ...
        action: bucket,
        priority: bucket.includes("30+") || bucket.includes("8-30") ? "YГјksЙ™k" : "Orta",
        status: invoice.status,
      });
    });
  ...
  employees
    ...
    .forEach((employee) => {
      riskRows.push({
        ...
        action: `${Number(employee.documentsComplete || 0)}% sЙ™nЙ™d`,
        priority: Number(employee.documentsComplete || 0) < 70 ? "YГјksЙ™k" : "Orta",
        status: employee.payrollStatus || employee.hrStatus || "SЙ™nЙ™d gГ¶zlЙ™yir",
      });
    });
...
function buildReportPackage({
  ...
  const invoiceControl = buildInvoiceControlSummary(invoices);
  const creditBalance = credits.reduce((sum, credit) => sum + Number(getCreditDisplayPlan(credit).balance || 0), 0);
  const criticalCount = riskRows.filter((row) => row.priority === "YГјksЙ™k").length;
  ...
  return {
    ...
    riskRows,
    checklist: [
      { label: "SatД±Еџ vЙ™ kredit mГјqavilЙ™lЙ™ri", status: credits.length > 0 || orders.length > 0 ? "HazД±r" : "MЙ™lumat yoxdur" },
      { label: "Anbar minimum qalД±q siqnallarД±", status: moduleRows.find((row) => row.module === "Anbar")?.status || "HazД±r" },
      { label: "MaliyyЙ™ tЙ™sdiq vЙ™ faktura nЙ™zarЙ™ti", status: invoiceControl.openBalance > 0 ? "NЙ™zarЙ™t" : "HazД±r" },
      { label: "HR payroll vЙ™ sЙ™nЙ™d tamamlД±ДџД±", status: moduleRows.find((row) => row.module === "HR")?.status || "HazД±r" },
    ],
  };
}
...
function normalizeMessageThread(conversation = {}) {
  const title = conversation.title || conversation.person || conversation.name || conversation.id || "SГ¶hbЙ™t";
...
function buildMessageParticipantOptions(settings = {}, employees = []) {
  ...
  const staff = (employees || []).map((employee) => ({
    ...
    role: employee.position || employee.role || "ЖЏmЙ™kdaЕџ",
...
function buildMessageContextOptions({ customers = [], orders = [], credits = [], tickets = [] } = {}) {
  const customerOptions = customers.slice(0, 60).map((customer) => ({
    ...
    detail: customer.fin ? `MГјЕџtЙ™ri В· FД°N ${customer.fin}` : "MГјЕџtЙ™ri",
  }));
  const orderOptions = orders.slice(0, 60).map((order) => ({
    type: "order",
    id: order.id,
    label: order.id,
    detail: `${order.customer || order.customerName || "SifariЕџ"} В· ${money(order.amount || order.total || order.totalAmount || 0)}`,
  }));
  const creditOptions = credits.slice(0, 60).map((credit) => ({
    type: "credit",
    id: credit.id,
    label: credit.contractId || credit.id,
    detail: `${credit.customer || "Kredit"} В· qalД±q ${money(credit.remaining || credit.balance || credit.amount || 0)}`,
  }));
  const ticketOptions = tickets.slice(0, 60).map((ticket) => ({
    type: "support",
    id: ticket.id,
    label: ticket.title || ticket.id,
    detail: `${ticket.module || "Support"} В· ${ticket.status || "Aktiv"}`,
  }));

  return [
    { type: "", id: "", label: "BaДџlantД± yoxdur", detail: "Гњmumi daxili yazД±Еџma" },
    ...customerOptions,
    ...orderOptions,
    ...creditOptions,
    ...ticketOptions,
...
function getMessageContextPayload(linkedType, linkedId, { customers = [], orders = [], credits = [], tickets = [] } = {}) {
  ...
  if (linkedType === "order") {
    ...
    return {
      ...
      linkedLabel: order ? `${order.id} В· ${order.customer || order.customerName || "SifariЕџ"}` : linkedId,
    };
  }
...
function getReceivableAgingBucket(days = 0) {
  ...
  if (value <= 30) return "1-30 gГјn";
  if (value <= 60) return "31-60 gГјn";
  if (value <= 90) return "61-90 gГјn";
  return "90+ gГјn";
}
...
function getReceivableRiskCategory({ amount = 0, overdueDays = 0, type = "Debitor" }) {
  ...
  if (value <= 0) return "BaДџlД±";
  if (days > 90 || value >= 10000) return "Kritik";
  if (days > 30 || value >= 5000) return "YГјksЙ™k";
  if (days > 0 || value >= 1500 || type === "Kreditor") return "NЙ™zarЙ™t";
  return "SaДџlam";
}
...
function getReceivableCollectionStatus({ amount = 0, overdueDays = 0, type = "Debitor" }) {
  const value = Number(amount || 0);
  const days = Number(overdueDays || 0);
  if (value <= 0) return "BaДџlandД±";
  if (type === "Kreditor") return days > 0 ? "Г–dЙ™niЕџ prioriteti" : "Г–dЙ™niЕџ planД±";
  if (days > 90) return "HГјquqi eskalasiya";
  if (days > 30) return "Eskalasiyada";
  if (days > 7) return "Kolleksiyada";
  if (days > 0) return "XatД±rlatma";
  return "Д°zlЙ™mЙ™dЙ™";
}

function getReceivableNextAction(row) {
  if (Number(row.amount || 0) <= 0) return "ArxivdЙ™ saxla";
  if (row.type === "Kreditor") {
    return Number(row.overdueDays || 0) > 0 ? "Vendor Г¶dЙ™niЕџini baДџla" : "Г–dЙ™niЕџ tarixini tЙ™sdiqlЙ™";
  }
  if (Number(row.overdueDays || 0) > 90) return "HГјquqi mЙ™rhЙ™lЙ™yЙ™ Г¶tГјr";
  if (Number(row.overdueDays || 0) > 30) return "Menecer eskalasiyasД±";
  if (Number(row.overdueDays || 0) > 7) return "Kollektor zЙ™ngi";
  if (Number(row.overdueDays || 0) > 0) return "SMS xatД±rlatma";
  return "PlanlД± izlЙ™mЙ™";
}

function buildReceivableAgingSummary(rows = []) {
  const buckets = ["Cari", "1-30 gГјn", "31-60 gГјn", "61-90 gГјn", "90+ gГјn"];
  return buckets.map((bucket) => {
...
function buildReceivableRows({ customers = [], orders = [], credits = [], vendors = [], purchaseOrders = [] }) {
  ...
  const createCreditDebtRow = (credit, customer = null) => {
    ...
    const device = credit.device || credit.product || relatedOrder?.products || "Cihaz qeyd edilmЙ™yib";

    return enrichReceivableRow({
      id: `DB-CR-${credit.id}`,
      type: "Debitor",
      party: customer?.name || credit.customer || "MГјЕџtЙ™ri qeyd edilmЙ™yib",
      source: credit.contractId || credit.id,
      sourceType: "credit",
      sourceTypeLabel: "Kredit mГјqavilЙ™si",
      amount: balance,
      orderBalance: 0,
      ...
      status: paymentState.isOverdue ? "GecikmЙ™" : "Aktiv",
      detail: `${credit.id} В· ${device} В· ${plan.months} ay`,
    ...
    if (manualDebt > 0) {
      rows.push(
        enrichReceivableRow({
          ...
          owner: "CRM",
          status: Number(customer.delay || 0) > 0 ? "GecikmЙ™" : "Aktiv",
          detail: "MГјЕџtЙ™ri kartД±ndakД± ayrД±ca borc",
          orderIds: [],
          openOrderIds: [],
          creditIds: [],
          contractIds: [],
      ...
      .forEach((order) => {
        ...
        rows.push(
          enrichReceivableRow({
            ...
            source: order.id,
            sourceType: "order",
            sourceTypeLabel: "NaДџd/qalД±qlД± sifariЕџ",
            amount: balance,
            orderBalance: balance,
            creditBalance: 0,
            customerDebt: 0,
            overdueDays,
            owner: order.sellerBonuses?.[0]?.seller || order.seller || "SatД±Еџ",
            status: overdueDays > 0 ? "GecikmЙ™" : "Aktiv",
            detail: `${order.id} В· ${summarizeOrderProducts(order)}`,
            orderIds: [order.id],
  ...
  const vendorRows = vendors.flatMap((vendor) => {
    ...
    const payablePos = vendorPos.filter((po) => {
      const status = normalize(po.status);
      return !status.includes("Г¶dЙ™n") && !status.includes("oden") && !status.includes("imtina") && !status.includes("cancel");
    });
    ...
    return [
      ...
      overdueDays,
      owner: "Vendor/MaliyyЙ™",
      status: pendingAmount > 0 ? "Г–dЙ™niЕџ gГ¶zlЙ™yir" : vendor.status,
      detail: latestPo ? `${vendorPos.length} PO В· son: ${latestPo.id}` : `${vendor.sku} SKU В· PO yoxdur`,
  ...
  return [
    {
      ...
      name: "Avtomatik satД±Еџ layihЙ™si",
      owner: "SatД±Еџ/MaliyyЙ™",
...
function buildProjectRoiRows({ projects, orders, expenses, products = [] }) {
  ...
  return portfolio.map((project) => {
    ...
    const approvedExpenses = matchedExpenses.filter((expense) => expense.status === "TЙ™sdiq edildi");
    const pendingExpenses = matchedExpenses.filter((expense) => expense.status !== "TЙ™sdiq edildi");
    ...
    return {
      ...
      reportStatus: project.lastExportAt ? "Export olunub" : "Export gГ¶zlЙ™yir",
      nextAction: status === "Risk" || status === "BГјdcЙ™ aЕџД±mД±" ? "XЙ™rci yoxla" : status === "YД±ДџД±m riski" ? "YД±ДџД±mД± sГјrЙ™tlЙ™ndir" : "Hesabat hazД±r",
      status,
    };
  });
}
...
function buildNotificationProviderRows(providers = [], settings = {}, sendLog = []) {
  return ensureNotificationProviders(providers).map((provider) => {
    ...
    const failures = rows.filter((log) => normalize(log.status).includes("uДџursuz") || normalize(log.status).includes("blok")).length;
    return {
      ...provider,
      enabled,
      sentCount: rows.filter((log) => normalize(log.status).includes("gГ¶ndЙ™rildi")).length,
      failureCount: failures,
      lastSentAt: rows[0]?.sentAt || provider.lastSentAt || "",
      health: !enabled ? "Kanal baДџlД±dД±r" : provider.status === "Aktiv" && failures === 0 ? "HazД±r" : provider.status,
    };
  });
}
...
function buildNotificationRuleEvents(rule, { credits = [], stock = [], warehouseStock = {}, products = [], purchaseOrders = [], expenses = [], orders = [] }) {
    ...
    return credits
      ...
      .map((item) =>
        buildNotificationEvent(rule, {
          ...
          status: item.paymentState.isOverdue ? `${item.paymentState.daysOverdue} gГјn gecikib` : "bu gГјn Г¶dЙ™nilmЙ™lidir",
          context: `${item.credit.contractId || item.credit.id} В· ${item.credit.device || item.credit.product || "Cihaz"}`,
    ...
    return credits
      ...
      .map((item) =>
        buildNotificationEvent(rule, {
          ...
          actionTarget: item.credit.id,
          module: "credits",
          priority: Number(item.leadDays) <= 1 ? "YГјksЙ™k" : "Orta",
        }),
      );
  }
  ...
  if (rule.id === "RULE-PO-APPROVAL") {
    return (purchaseOrders || [])
      .filter((po) => normalize(po.status).includes("gГ¶zl"))
      .map((po) =>
        buildNotificationEvent(rule, {
          id: `${rule.id}-${po.id}`,
          entityId: po.id,
          recipient: po.vendor || "SatД±nalma",
          target: po.vendor || "procurement",
          poId: po.id,
          amount: po.amount,
          module: "vendors",
          priority: Number(po.amount || 0) >= 5000 ? "YГјksЙ™k" : "Orta",
        }),
      );
  }
  ...
  if (rule.id === "RULE-PAYROLL") {
    return expenses
      .filter((expense) => expense.source === "HR Payroll" && normalize(expense.status).includes("gГ¶zl"))
      .map((expense) =>
        buildNotificationEvent(rule, {
          id: `${rule.id}-${expense.id}`,
          entityId: expense.id,
          recipient: "HR/MaliyyЙ™",
          target: "finance-team",
          amount: expense.amount,
          module: "finance",
          priority: "Orta",
  ...
  if (rule.id === "RULE-DELIVERY-SLA") {
    return orders
      .filter((order) => order.status !== "TЙ™hvil verilib" && getDeliveryAgeDays(order) >= 5)
      .map((order) =>
        buildNotificationEvent(rule, {
          id: `${rule.id}-${order.id}`,
          entityId: order.id,
          recipient: order.customer || "TЙ™hvil komandasД±",
...
function buildNotificationAutomationRows({ notificationRules, providers = [], settings = {}, sendLog = [], credits, stock, warehouseStock = {}, products = [], purchaseOrders, expenses, orders }) {
  ...
  const pendingPo = (purchaseOrders || []).filter((po) => po.status === "TЙ™sdiq gГ¶zlЙ™yir");
  const pendingPayroll = expenses.filter((expense) => expense.source === "HR Payroll" && expense.status === "TЙ™sdiq gГ¶zlЙ™yir");
  const lateDeliveries = orders.filter((order) => order.status !== "TЙ™hvil verilib" && getDeliveryAgeDays(order) >= 5);
  const now = new Date();

  const queueByRule = {
    "RULE-CREDIT-OVERDUE": {
      count: overdueCredits.length,
      event: overdueCredits[0]?.customer || "GecikmЙ™ yoxdur",
    },
    "RULE-CREDIT-UPCOMING": {
      count: dueSoonCredits.length,
      event: dueSoonCredits[0]?.customer || "YaxД±nlaЕџan Г¶dЙ™niЕџ yoxdur",
    },
    "RULE-LOW-STOCK": {
      count: lowStock.length,
      event: lowStock[0]?.product || "Stok normaldД±r",
    },
    "RULE-PO-APPROVAL": {
      count: pendingPo.length,
      event: pendingPo[0]?.id || "PO nГ¶vbЙ™si boЕџdur",
    },
    "RULE-PAYROLL": {
      count: pendingPayroll.length,
      event: pendingPayroll[0] ? money(pendingPayroll[0].amount) : "Payroll tЙ™sdiqdЙ™ deyil",
    },
    "RULE-DELIVERY-SLA": {
      count: lateDeliveries.length,
      event: lateDeliveries[0]?.id || "TЙ™hvil SLA riski yoxdur",
    },
  };
  ...
  return ensureNotificationRules(notificationRules).map((rule) => {
    ...
    return {
      ...rule,
      provider,
      providerName: provider?.name || "Provider seГ§ilmЙ™yib",
      queueCount,
      totalEventCount,
        ...
        rule.status !== "Aktiv"
          ...
          : !channelEnabled
            ...
            : !provider || provider.status !== "Aktiv"
              ? "Provider hazД±r deyil"
              : queueCount > 0
                ? "GГ¶ndЙ™riЕџ hazД±rdД±r"
                : cooldownEvents.length > 0
                  ? "Cooldown-da saxlanД±b"
                : "NГ¶vbЙ™ boЕџdur",
    };
  });
}

function createNotificationSendLogEntry({ rule = {}, event = {}, provider = {}, stamp, status = "GГ¶ndЙ™rildi", source = "Avtomatik" }) {
  ...
  return {
    ...
    ruleName: rule.name || "Manual gГ¶ndЙ™riЕџ",
    ...
    subject: event.subject || rule.name || "BildiriЕџ",
...
function buildNotificationDeliveriesForRules({ rules = [], providerRows = [], settings = {}, stamp, source = "Avtomatik qayda" }) {
  return rules.flatMap((rule) => {
    const provider =
      ...
      {};
    const channelEnabled = getNotificationChannelEnabled(rule.channel, settings);
    const status = !channelEnabled || provider.status !== "Aktiv" ? "BloklandД±" : "GГ¶ndЙ™rildi";
...
function buildProductionPlanRows(productionPlans, stock, warehouseStock = {}, products = [], warehouses = []) {
  ...
  return (productionPlans || []).map((plan) => {
    ...
    return {
      ...plan,
      warehouseId,
      warehouseName: warehouseById.get(warehouseId)?.name || plan.warehouseName || "Anbar seГ§ilmЙ™yib",
      materials,
      materialCost,
      totalCost,
      unitCost,
      ...
      bottleneck: bottleneck?.product || (completed ? "Д°stehsal baДџlandД±" : "Xammal kifayЙ™tdir"),
      canProduce: !completed && !bottleneck && Number(plan.plannedQty || 0) > 0,
      issueStatus: completed ? "Xammal Г§Д±xД±ldД±" : bottleneck ? "Xammal Г§atmД±r" : "Г‡Д±xД±Еџa hazД±r",
      receiptStatus: completed ? `${producedQty} hazД±r mЙ™hsul mЙ™daxil` : "MЙ™daxil gГ¶zlЙ™yir",
      status: completed ? "Д°stehsal edildi" : bottleneck ? "Xammal riski" : plan.status || "PlanlandД±",
    };
  });
}
...
function buildOnboardingRows(onboarding = {}, snapshot = {}) {
  ...
  const defaultSteps = [
    { id: "ONB-1", title: "ЕћirkЙ™t mЙ™lumatlarД± vЙ™ VГ–EN", owner: "Administrator", module: "settings", detail: "Ayarlar bГ¶lmЙ™sindЙ™ ЕџirkЙ™t adД±, VГ–EN vЙ™ Й™sas rekvizitlЙ™r doldurulur." },
    { id: "ONB-2", title: "Д°lk anbarД±n yaradД±lmasД±", owner: "Anbar", module: "warehouse", detail: "ЖЏn azД± bir aktiv anbar yaradД±lД±r." },
    { id: "ONB-3", title: "MЙ™hsul kataloqu vЙ™ ilkin stok", owner: "Anbar", module: "warehouse", detail: "MЙ™hsullar, minimum qalД±q vЙ™ ilkin mЙ™daxil real stok kimi yazД±lД±r." },
    { id: "ONB-4", title: "Д°stifadЙ™Г§i rollarД± vЙ™ permission", owner: "Administrator", module: "settings", detail: "Rol vЙ™ modul icazЙ™lЙ™ri real dГјymЙ™lЙ™rЙ™ tЙ™tbiq olunur." },
    { id: "ONB-5", title: "Kassa vЙ™ bank aГ§Д±lД±Еџ balansД±", owner: "MaliyyЙ™", module: "finance", detail: "Kassa/bank hesablarД± maliyyЙ™ modulunda yaradД±lД±r." },
    { id: "ONB-6", title: "HR Й™mЙ™kdaЕџlarД± vЙ™ payroll", owner: "HR", module: "hr", detail: "ЖЏmЙ™kdaЕџ kartlarД±, sЙ™nЙ™d faizi vЙ™ cash-neutral payroll accrual tamamlanД±r." },
    { id: "ONB-7", title: "SatД±Еџ vЙ™ kredit UAT ssenarisi", owner: "SatД±Еџ/MaliyyЙ™", module: "sales", detail: "SatД±Еџdan kredit, mГјqavilЙ™, rezerv vЙ™ tЙ™hvil axД±nД± test edilir." },
    { id: "ONB-8", title: "Vendor vЙ™ PO axД±nД±", owner: "Procurement", module: "vendors", detail: "Vendor, minimum stok vЙ™ PO tЙ™sdiq/mЙ™daxil axД±nД± hazД±rlanД±r." },
    { id: "ONB-9", title: "Hesabat vЙ™ analitika export", owner: "RЙ™hbЙ™rlik", module: "reports", detail: "Executive hesabat, ROI vЙ™ audit snapshot-larД± export edilir." },
    { id: "ONB-10", title: "Backup vЙ™ go-live yoxlamasД±", owner: "Administrator", module: "settings", detail: "Integrity, backup vЙ™ go-live readiness yoxlamasД± saxlanД±lД±r." },
  ];
  ...
  const steps = sourceSteps.map((step) => {
    ...
    const status = completed ? "TamamlandД±" : nextFound ? "GГ¶zlЙ™yir" : "Д°crada";
    if (!completed) nextFound = true;
    return {
      ...step,
      status,
      completed,
      action: completed ? "YoxlanД±lД±b" : "Modula keГ§",
    };
  });
  const completed = steps.filter((step) => step.status === "TamamlandД±").length;
  const blockers = steps.filter((step) => step.status === "Д°crada").length;
  ...
  return {
    ...
    nextStep: steps.find((step) => step.status !== "TamamlandД±") || null,
  };
}
...
function buildHelpModuleGuides({ moduleReadiness = { items: [] }, onboardingRows = { steps: [] } }) {
  ...
  return navItems
    ...
    .map((item) => {
      ...
      const permission = getPageActionPermission(item.id) || navPermissionByType[item.id] || "OxunuЕџ";
      return {
        id: item.id,
        module: item.label,
        title: meta.title || item.label,
        purpose: meta.subtitle || "Modul ГјzrЙ™ Й™mЙ™liyyatlar vЙ™ nЙ™zarЙ™t.",
        action: meta.action || "BaxД±Еџ",
        permission,
        readiness: readiness?.status || onboarding?.status || "NЙ™zarЙ™t",
        next: readiness?.next || onboarding?.detail || "Modul ГјzrЙ™ Й™sas Й™mЙ™liyyatД± tamamlayД±n.",
...
function buildHelpArticles(articles = [], moduleGuides = [], onboardingRows = { steps: [] }) {
  const generated = [
    {
      id: "HELP-START",
      title: "Sistemi real istifadЙ™yЙ™ necЙ™ baЕџlatmaq olar",
      category: "Onboarding",
      answer: "Ayarlar, anbar, mЙ™hsul, rol, maliyyЙ™, HR, satД±Еџ/kredit, vendor, hesabat vЙ™ backup addД±mlarД±nД± ardД±cД±l tamamlayД±n.",
    ...
    {
      id: "HELP-PERMISSIONS",
      title: "Rollar vЙ™ icazЙ™lЙ™r necЙ™ iЕџlЙ™yir",
      category: "Ayarlar",
      answer: "HЙ™r modul Й™mЙ™liyyatД± permission ilЙ™ qorunur. Д°stifadЙ™Г§inin rolunda permission yoxdursa dГјymЙ™ deaktiv olur vЙ™ ya Й™mЙ™liyyat bloklanД±r.",
      tags: ["rol", "permission", "tЙ™hlГјkЙ™sizlik"],
      createdAt: currentBusinessDate,
      generated: true,
    },
    {
      id: "HELP-AUDIT",
      title: "Audit log vЙ™ hesabat izi",
      category: "Audit",
      answer: "SatД±Еџ, kredit, anbar, maliyyЙ™, HR, ROI, support vЙ™ export Й™mЙ™liyyatlarД± audit log-a yazД±lД±r. Hesabat snapshot-larД± reportExports iГ§indЙ™ saxlanД±r.",
  ...
  const moduleArticles = moduleGuides.slice(0, 12).map((guide) => ({
    ...
    category: "Modul izahД±",
    answer: `${guide.purpose} NГ¶vbЙ™ti addД±m: ${guide.next}`,
...
function getOrderDeliveryStatus(order) {
  if (order.status === "TЙ™hvil verilib") return "TЙ™hvil verilib";
  return order.deliveryStatus || "TЙ™hvil gГ¶zlЙ™yir";
}

function getOrderPaymentMethod(order) {
  return order.paymentMethod || (getOrderBalance(order) > 0 ? "QalД±qlД±" : "NaДџd");
}
...
function getDeliveryDisplayStage(order) {
  if (order?.status === "TЙ™hvil verilib") return "TЙ™hvil verildi";
  return "Anbarda";
}
...
// QismЙ™n tЙ™hvil planД±: hЙ™r sЙ™tir ГјГ§Гјn nЙ™ qЙ™dЙ™ri anbardan verilЙ™ bilЙ™r, nЙ™ qЙ™dЙ™ri backorder qalД±r.
...
export function getDeliveryStockCheck(order, warehouseStock = {}) {
  if (!order) {
    return { ok: false, status: "SifariЕџ yoxdur", reason: "SifariЕџ tapД±lmadД±.", issues: [] };
  }

  if (order.status === "TЙ™hvil verilib") {
    return { ok: false, status: "TЙ™hvil verilib", reason: "Bu sifariЕџ artД±q tЙ™hvil verilib.", issues: [], plan: null };
  }

  const productLines = normalizeOrderProductLines(order.productLines || []);
  if (productLines.length === 0) {
    return { ok: false, status: "MЙ™hsul yoxdur", reason: "SifariЕџdЙ™ tЙ™hvil verilЙ™cЙ™k mЙ™hsul yoxdur.", issues: [] };
  }

  const warehouseId = order.warehouseId;
  if (!warehouseId) {
    return { ok: false, status: "Anbar seГ§ilmЙ™yib", reason: "SifariЕџ ГјГ§Гјn anbar seГ§ilmЙ™yib.", issues: [] };
  }

  const plan = getDeliveryPlan({ ...order, warehouseId }, warehouseStock);

  if (plan.remainingTotal === 0) {
    return { ok: false, status: "TЙ™hvil verilib", reason: "SifariЕџin bГјtГјn mЙ™hsullarД± tЙ™hvil verilib.", issues: [], plan };
  }
  ...
  const issues = plan.lines
    ...
    .map(
      (line) =>
        `${line.product}: anbarda ${line.deliverable}/${line.remaining} Й™dЙ™d var вЂ” ${line.shortage} Й™dЙ™d tЙ™chizat gГ¶zlЙ™nilir (backorder)`,
    );

  if (plan.deliverableTotal === 0) {
    return {
      ok: false,
      status: "Г‡atД±ЕџmazlД±q (backorder)",
      reason: issues[0] || "Anbarda tЙ™hvil ГјГ§Гјn qalД±q yoxdur.",
      issues,
      plan,
    };
  }
  ...
  if (plan.shortageTotal > 0) {
    return {
      ok: true,
      partial: true,
      status: "QismЙ™n tЙ™hvilЙ™ hazД±r",
      reason: `${plan.deliverableTotal}/${plan.remainingTotal} Й™dЙ™d indi tЙ™hvil verilЙ™ bilЙ™r, ${plan.shortageTotal} Й™dЙ™d backorder qalД±r.`,
      issues,
      plan,
    };
  }
  ...
  return {
    ok: true,
    partial: false,
    status: "TЙ™hvilЙ™ hazД±r",
    reason: "BГјtГјn qalan mЙ™hsullar anbardadД±r vЙ™ Г§Д±xarД±la bilЙ™r.",
    issues: [],
    plan,
  };
}
...
function buildSalesCreditForOrder(order, storedCredit) {
  ...
  if (paidPrincipal <= 0) {
    return isCreditClosed(storedCredit || baseCredit, { ...creditPlan, installments })
      ? { ...baseCredit, balance: 0, paidMonths: creditPlan.months, rate: 100, status: "TamamlandД±" }
      : baseCredit;
  }
  ...
  return {
    ...
    status: paymentResult.nextBalance <= 0 ? "TamamlandД±" : paymentResult.status,
  };
}
...
function matchesSalesOrderFilter(order, filter) {
  ...
  if (filter === "NaДџd") return getOrderPaymentMethod(order) === "NaДџd";
  if (filter === "QalД±qlД±") return getOrderBalance(order) > 0;
  if (filter === "TЙ™hvil gГ¶zlЙ™yЙ™n") return order.status !== "TЙ™hvil verilib";
  if (filter === "Tamamlanan") return order.status === "TЙ™hvil verilib";
  if (filter === "Riskli") return getOrderBalance(order) > 0 || (order.status !== "TЙ™hvil verilib" && getDeliveryAgeDays(order) >= 3);
  return true;
}
...
function getSalesOrderRiskStatus(order) {
  if (order.status === "TЙ™hvil verilib" && getOrderBalance(order) <= 0) return "TamamlanД±b";
  if (getOrderBalance(order) > 0 && order.status !== "TЙ™hvil verilib") return "Borclu + tЙ™hvil";
  if (getOrderBalance(order) > 0) return "Borclu";
  if (order.status !== "TЙ™hvil verilib" && getDeliveryAgeDays(order) >= 3) return "TЙ™hvil riski";
  if (order.status !== "TЙ™hvil verilib") return "TЙ™hvil gГ¶zlЙ™yir";
  return "SaДџlam";
}
...
function getDeliveryRisk(order) {
  if (order.status === "TЙ™hvil verilib") return "TamamlandД±";
  if (getDeliveryStageIndex(order) >= 2 && (!order.driver || order.driver === "вЂ”")) return "SГјrГјcГј yoxdur";
  if (order.status === "TЙ™hvilЙ™ Г§Д±xД±b" || order.status === "HazД±rdД±r") return "Bu gГјn prioritet";
  if (getDeliveryAgeDays(order) >= 6) return "GecikmЙ™ riski";
  return "Normal";
}

function getDeliveryActionLabel(order) {
  if (order.status === "TЙ™hvil verilib") return "TamamlanД±b";
  if (order.status === "TЙ™hvilЙ™ Г§Д±xД±b") return "TЙ™hvili tamamla";
  return "NГ¶vbЙ™ti mЙ™rhЙ™lЙ™";
}
...
function matchesDeliveryFilter(order, filter) {
  if (filter === "HamД±sД±") return true;
  if (filter === "Aktiv") return order.status !== "TЙ™hvil verilib";
  if (filter === "GecikmЙ™ riski") return order.risk === "GecikmЙ™ riski";
  if (filter === "SГјrГјcГј yoxdur") return order.risk === "SГјrГјcГј yoxdur";
  if (filter === "Tamamlanan") return order.status === "TЙ™hvil verilib";
...
function buildDriverDeliveryStats(orders) {
  ...
  orders
    .filter((order) => order.driver && order.driver !== "вЂ”")
    .forEach((order) => {
      ...
      if (order.status === "TЙ™hvilЙ™ Г§Д±xД±b" || order.status === "YoldadД±r") {
        current.outForDelivery += 1;
      }
      if (order.risk !== "Normal" && order.risk !== "TamamlandД±") {
        current.risk += 1;
      }
...
export function buildFinanceLedger({ orders, expenses, cashEntries }) {
  const salesRows = orders
    ...
    .map((order) => {
      ...
      const account = ["Kart", "KГ¶Г§ГјrmЙ™"].includes(paymentMethod) ? "Bank hesabД±" : "Kassa";
      return {
        id: `SALE-${order.id}`,
        date: order.date,
        type: "SatД±Еџ",
  ...
  const creditRows = cashEntries.map((entry) => ({
    ...
    category: entry.category || "Kredit Г¶dЙ™niЕџi",
    account: entry.account || "Kassa",
    title: entry.creditId || entry.orderId || entry.receivableId || entry.id,
    description: entry.contractId || entry.note || "MГјqavilЙ™ qeyd edilmЙ™yib",
  ...
  const expenseRows = expenses.map((expense) => {
    const approved = expense.status === "TЙ™sdiq edildi";
    const rejected = expense.status === "Д°mtina edildi";
    ...
    return {
      ...
      date: expense.date,
      type: "XЙ™rc",
      source: expense.source || "MaliyyЙ™ modulu",
      category: expense.category,
      account: !cashImpact ? "UГ§ot xЙ™rci" : direction === "ignored" ? "TЙ™sirsiz" : expense.account || "Kassa",
      title: expense.description,
      description: expense.category,
      party: "ЕћirkЙ™t xЙ™rci",
      principal: 0,
      penalty: 0,
      amount: Number(expense.amount || 0),
      direction,
      status: cashImpact ? expense.status : `${expense.status} В· cash tЙ™siri yoxdur`,
...
function matchesFinanceFilter(row, filter) {
  ...
  if (filter === "XЙ™rc") return row.type === "XЙ™rc";
  if (filter === "SatД±Еџ") return row.type === "SatД±Еџ";
  if (filter === "Kredit") return row.type === "Kredit";
  if (filter === "GecikmЙ™ gЙ™liri") return Number(row.penalty || 0) > 0;
  if (filter === "TЙ™sdiq gГ¶zlЙ™yir") return row.direction === "pending";
  if (filter === "Cash tЙ™sirsiz") return row.direction === "accrual";
  return true;
}
...
export function buildExpenseCategoryRows(expenses) {
  const byCategory = expenses.reduce((map, expense) => {
    ...
    if (expense.status === "TЙ™sdiq edildi") current.approved += Number(expense.amount || 0);
    if (expense.status === "TЙ™sdiq gГ¶zlЙ™yir") current.pending += Number(expense.amount || 0);
    if (expense.status === "Д°mtina edildi") current.rejected += Number(expense.amount || 0);
...
function ensureStockItemSerials(item, warehouseId = "WH") {
  ...
  const nonSoldSerials = existingSerials.filter((serial) => serial.status !== "SatД±lД±b");
  ...
  for (let index = nonSoldSerials.length; index < totalCount; index += 1) {
    serials.push({
      ...
      status: index < reservedCount ? "RezervdЙ™" : "Anbarda",
      warehouseId,
      product: item.product,
    });
  }
  ...
  const normalizedSerials = serials.map((serial) => {
    if (serial.status === "SatД±lД±b") return serial;
    reservedSeen += 1;
    return {
      ...serial,
      status: reservedSeen <= reservedCount ? "RezervdЙ™" : "Anbarda",
...
function getSerialSummary(serials = []) {
  return {
    ...
    reserved: serials.filter((serial) => serial.status === "RezervdЙ™").length,
    sold: serials.filter((serial) => serial.status === "SatД±lД±b").length,
  };
}
    ...
    return {
      ...
      serials: item.serials.map((serial) => {
        ...
        const fallback =
          ...
          serial.status !== "SatД±lД±b" &&
          fallbackCount < neededFallback &&
          (nextStatus === "SatД±lД±b" ? serial.status === "RezervdЙ™" : serial.status === "Anbarda");
        ...
        return {
          ...
          status: nextStatus,
          orderId,
          reservedAt: nextStatus === "RezervdЙ™" ? currentBusinessDate : serial.reservedAt,
          soldAt: nextStatus === "SatД±lД±b" ? currentBusinessDate : serial.soldAt,
        };
      }),
    };
  });
  ...
  return rows.map((item) => {
    ...
    return {
      ...
      serials: item.serials.map((serial) => {
        ...
        if (serial.status !== "RezervdЙ™" || (!explicit && !fallback)) return serial;
        releasedCount += 1;
        return {
          ...serial,
          status: "Anbarda",
...
function adjustStockRows(rows, quantities, { totalDelta = 0, reservedDelta = 0, createMissing = false }) {
  ...
  const next = rows.map((item) => {
    ...
    return {
      ...item,
      // QalД±q heГ§ vaxt mЙ™nfi olmur; rezerv qalД±qdan Г§ox ola bilЙ™r (backorder).
  ...
  // Anbarda sЙ™tri olmayan mЙ™hsul da satД±la bilЙ™r вЂ” sД±fД±r qalД±qla rezerv sЙ™tri aГ§Д±lД±r.
  ...
  return {
    ...
    invoiceSettings: {
      ...
      eTaxMode: normalize(snapshot.invoiceSettings?.eTaxMode).includes("demo")
        ? "E-qaimЙ™ inteqrasiya rejimi"
        : snapshot.invoiceSettings?.eTaxMode,
    },
    productionPlans: (snapshot.productionPlans || []).map((plan) => ({
      ...plan,
      product: normalize(plan.product).includes("showroom demo") ? "Showroom nГјmayiЕџ dЙ™sti" : plan.product,
    })),
  };
}
...
export function getActiveRole(settings = {}) {
  ...
  if (user?.role === "Platform Super Admin") {
    return { name: "Platform Super Admin", scope: "BГјtГјn platforma vЙ™ Й™sas ERP tenant-Д±na tam giriЕџ", permissions: permissionCatalog.map((item) => item.key) };
  }
...
function getDuplicateValues(rows = [], key, label) {
  ...
  return [...duplicates].map((value) => ({
    ...
    detail: `${value} tЙ™krar istifadЙ™ olunub`,
    fix: "ID/FIN unikal saxlanmalД±dД±r",
  }));
}
...
function buildStateIntegrityReport(snapshot = {}, creditRows = []) {
  ...
  issues.push(...getDuplicateValues(orders, "id", "SatД±Еџ"));
  ...
  issues.push(...getDuplicateValues(contracts, "id", "MГјqavilЙ™"));
  issues.push(...getDuplicateValues(credits, "id", "Kredit"));
  issues.push(...getDuplicateValues(departments, "name", "HR ЕџГ¶bЙ™"));
  issues.push(...getDuplicateValues(leaveRequests, "id", "HR mЙ™zuniyyЙ™t"));
  ...
  if (!currentUser) {
    issues.push({
      ...
      detail: "Permission yoxlamasД± ГјГ§Гјn aktiv istifadЙ™Г§i seГ§ilmЙ™lidir",
      fix: "Ayarlar bГ¶lmЙ™sindЙ™ aktiv istifadЙ™Г§i ilЙ™ giriЕџ edin",
    });
  }
  ...
  if (users.filter((user) => user.status === "Aktiv").length === 0) {
    issues.push({
      ...
      severity: "Kritik",
      area: "Auth",
      title: "Aktiv istifadЙ™Г§i yoxdur",
      detail: "SistemЙ™ giriЕџ ГјГ§Гјn Й™n azД± bir aktiv istifadЙ™Г§i lazД±mdД±r",
      fix: "Super Admin istifadЙ™Г§isini aktiv saxlayД±n",
    });
  }

  const stockGroups = [
    { label: "Гњmumi stok", rows: stock },
    ...Object.entries(warehouseStock).map(([warehouseId, rows]) => ({
    ...
    group.rows.forEach((item) => {
      ...
      if (totalQty < 0 || reservedQty < 0) {
        issues.push({
          ...
          severity: "Kritik",
          area: "Anbar",
          title: "MЙ™nfi stok gГ¶stЙ™ricisi",
          detail: `${group.label}: ${item.product}`,
          fix: "MЙ™daxil/mЙ™xaric tarixГ§Й™sini yoxlayД±n",
        });
      }
      if (reservedQty > totalQty) {
        issues.push({
          id: `STOCK-RES-${group.label}-${item.product}`,
          severity: "XЙ™bЙ™rdarlД±q",
          area: "Anbar",
          title: "Г‡atД±ЕџmazlД±q (backorder)",
          detail: `${group.label}: ${item.product} вЂ” rezerv ${reservedQty}, qalД±q ${totalQty}, Г§atД±Еџmayan ${reservedQty - totalQty}`,
          fix: "TЙ™chizatdan mЙ™daxil edin vЙ™ ya sifariЕџi baДџlayД±n",
        });
      }
      if (Array.isArray(item.serials) && item.serials.length > 0) {
        const activeSerials = item.serials.filter((serial) => serial.status !== "SatД±lД±b").length;
        if (activeSerials < totalQty) {
          issues.push({
            id: `SERIAL-MISS-${group.label}-${item.product}`,
            severity: "XЙ™bЙ™rdarlД±q",
            area: "IMEI",
            title: "Serial sayД± stokdan azdД±r",
            detail: `${group.label}: ${item.product} ГјГ§Гјn aktiv serial ${activeSerials}, stok ${totalQty}`,
            fix: "IMEI/serial reyestrini tamamlayД±n",
          });
        }
      }
    });
  ...
  orders.forEach((order) => {
    ...
    if (paid > amount) {
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "SatД±Еџ",
        title: "Г–dЙ™niЕџ sifariЕџ mЙ™blЙ™Дџini keГ§ir",
        detail: `${order.id}: Г¶dЙ™nilib ${money(paid)}, mЙ™blЙ™Дџ ${money(amount)}`,
        fix: "ArtД±q Г¶dЙ™niЕџi kredit vЙ™ ya avans kimi ayД±rД±n",
      });
    }
    if (order.paymentMethod === "Kredit" && !creditIds.has(order.id) && !creditIds.has(order.creditId)) {
      issues.push({
        ...
        severity: "Kritik",
        area: "SatД±Еџ/Kredit",
        title: "Kredit satД±ЕџД± kredit portfelindЙ™ yoxdur",
        detail: `${order.id}: ${order.customer}`,
        fix: "SatД±Еџdan kredit datasД±nД± yenidЙ™n sinxronlaЕџdД±rД±n",
      });
    }
  });
  ...
  credits.forEach((credit) => {
    ...
    if (Number(plan.balance || 0) < 0 || Number(plan.balance || 0) > Number(plan.total || 0)) {
      issues.push({
        ...
        severity: "Kritik",
        area: "Kredit",
        title: "Kredit balansД± uyДџunsuzdur",
        detail: `${credit.id}: balans ${money(plan.balance)} / total ${money(plan.total)}`,
        fix: "Г–dЙ™niЕџ tarixГ§Й™sini vЙ™ Й™sas mЙ™blЙ™Дџ silinmЙ™sini yoxlayД±n",
      });
    }
    if (!isCreditClosed(credit, plan) && !getCreditPaymentState(credit, plan).nextInstallment) {
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "Kredit",
        title: "NГ¶vbЙ™ti Г¶dЙ™niЕџ tarixi yoxdur",
        detail: `${credit.id}: ${credit.customer}`,
        fix: "Г–dЙ™niЕџ cЙ™dvЙ™lini yenilЙ™yin",
      });
    }
  });
  ...
  employees.forEach((employee) => {
    ...
    if (employee.managerId === employeeKey || employee.managerName === employee.name) {
      issues.push({
        ...
        severity: "Kritik",
        area: "HR",
        title: "ЖЏmЙ™kdaЕџ Г¶zГј-Г¶zГјnЙ™ tabedir",
        detail: `${employee.name} ГјГ§Гјn rЙ™hbЙ™r Й™laqЙ™si Г¶zГјnЙ™ iЕџarЙ™ edir`,
        fix: "ЖЏmЙ™kdaЕџ kartД±nda birbaЕџa rЙ™hbЙ™rlik vЙ™ ya dГјzgГјn rЙ™hbЙ™r seГ§in",
      });
      ...
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "HR",
        title: "RЙ™hbЙ™r qeydi tapД±lmadД±",
        detail: `${employee.name}: ${employee.managerId}`,
        fix: "ЖЏmЙ™kdaЕџ kartД±nda tabeГ§ilik Й™laqЙ™sini yenilЙ™yin",
      });
      ...
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "HR",
        title: "RЙ™hbЙ™r adД± reyestrdЙ™ yoxdur",
        detail: `${employee.name}: ${employee.managerName}`,
        fix: "RЙ™hbЙ™ri siyahД±dan yenidЙ™n seГ§in",
      });
    }
  });
  ...
  departments.forEach((department) => {
    if (department.parentDepartment === department.name) {
      issues.push({
        ...
        severity: "Kritik",
        area: "HR",
        title: "ЕћГ¶bЙ™ Г¶zГј-Г¶zГјnГјn Гјst ЕџГ¶bЙ™sidir",
        detail: department.name,
        fix: "ЕћГ¶bЙ™ kartД±ndakД± Гјst ЕџГ¶bЙ™ dЙ™yЙ™rini dЙ™yiЕџin",
      });
      ...
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "HR",
        title: "Гњst ЕџГ¶bЙ™ tapД±lmadД±",
        detail: `${department.name}: ${department.parentDepartment}`,
        fix: "ЕћГ¶bЙ™ strukturunu yenilЙ™yin vЙ™ ya Гјst ЕџГ¶bЙ™ni yaradД±n",
      });
    }
  });
  ...
  leaveRequests.forEach((request) => {
    ...
    if (!employeeFound) {
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "HR",
        title: "MЙ™zuniyyЙ™t Й™mЙ™kdaЕџД± tapД±lmadД±",
        detail: request.employeeName || request.employeeId || request.id,
        fix: "MЙ™zuniyyЙ™t qeydini aktiv Й™mЙ™kdaЕџla baДџlayД±n",
      });
    }
    if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end < start || Number(request.days || 0) <= 0) {
      issues.push({
        ...
        severity: "XЙ™bЙ™rdarlД±q",
        area: "HR",
        title: "MЙ™zuniyyЙ™t tarixi uyДџunsuzdur",
        detail: `${request.employeeName || request.id}: ${request.from || "вЂ”"} - ${request.to || "вЂ”"}`,
        fix: "BaЕџlanДџД±c, bitiЕџ vЙ™ gГјn sayД±nД± yenilЙ™yin",
      });
    }
  });
  ...
  return {
    ...
    score,
    status: critical > 0 ? "Risk" : warnings > 0 ? "NЙ™zarЙ™t" : "SaДџlam",
    critical,
    warnings,
  ...
  const items = [
    {
      ...
      status: isExternalDb ? "HazД±r" : "Bloker",
      risk: "YГјksЙ™k",
      next: isExternalDb
        ? "Connection monitor aktiv saxlanД±lsД±n"
        : `Brauzer local adapteri ${targetDbProvider} backend adapteri ilЙ™ Й™vЙ™z edilmЙ™lidir`,
    },
    {
      area: "Auth/RBAC",
      requirement: "Aktiv admin, rol vЙ™ kritik permission xЙ™ritЙ™si",
      status: hasActiveAdmin && hasBackupPermission ? "HazД±r" : "Bloker",
      risk: hasActiveAdmin ? "Orta" : "YГјksЙ™k",
      next: hasActiveAdmin ? "Server-side token yoxlamasД± Й™lavЙ™ olunmalД±dД±r" : "Super Admin istifadЙ™Г§isi aktiv edilmЙ™lidir",
    },
    {
      area: "Data integrity",
      requirement: "Stok, kredit, satД±Еџ vЙ™ audit bГјtГ¶vlГјyГј",
      status: integrityClear ? "HazД±r" : "Bloker",
      risk: integrityClear ? "AЕџaДџД±" : "YГјksЙ™k",
      next: integrityClear ? "PlanlД± gГјnlГјk yoxlama saxlanД±lsД±n" : "Integrity panelindЙ™ kritik siqnallar baДџlanmalД±dД±r",
    },
    {
      area: "Backup/Restore",
      requirement: "Export/import vЙ™ bЙ™rpa izi",
      status: hasBackupTrace ? "HazД±r" : "NЙ™zarЙ™t",
      risk: hasBackupTrace ? "AЕџaДџД±" : "Orta",
      next: hasBackupTrace ? "Avtomatik backup qrafiki qurulsun" : "Д°lk backup export/import yoxlamasД± aparД±lsД±n",
    },
    {
      area: "Audit",
      requirement: "ЖЏmЙ™liyyatlarД±n audit log-a dГјЕџmЙ™si",
      status: auditLog.length >= 5 ? "HazД±r" : "NЙ™zarЙ™t",
      risk: auditLog.length >= 5 ? "AЕџaДџД±" : "Orta",
      next: auditLog.length >= 5 ? "Audit log serverdЙ™ immutable saxlanmalД±dД±r" : "Kritik Й™mЙ™liyyatlar ГјzrЙ™ audit nГјmunЙ™lЙ™ri yaradД±lmalД±dД±r",
    },
    {
      area: "Biznes axД±nlarД±",
      requirement: "SatД±Еџ, kredit, anbar, tЙ™hvil, maliyyЙ™, HR data zЙ™nciri",
      status: hasCriticalBusinessData ? "HazД±r" : "Bloker",
      risk: hasCriticalBusinessData ? "AЕџaДџД±" : "YГјksЙ™k",
      next: hasCriticalBusinessData ? "UAT ssenarilЙ™ri imzalanmalД±dД±r" : "ЖЏsas modullara baЕџlanДџД±c data Й™lavЙ™ olunmalД±dД±r",
    },
    {
      area: "Monitoring",
      requirement: "Error, uptime, webhook vЙ™ backup monitorinqi",
      status: "NЙ™zarЙ™t",
      risk: "Orta",
      next: "Production hostda error logging vЙ™ uptime monitor qurulmalД±dД±r",
    },
    {
      area: "Deployment",
      requirement: "Docker/nginx/env/smoke test fayllarД±",
      status: deploymentToolkitReady ? "HazД±r" : "HazД±rlanД±r",
      risk: deploymentToolkitReady ? "AЕџaДџД±" : "Orta",
      next: deploymentToolkitReady
        ? "Staging serverdЙ™ npm run smoke nЙ™ticЙ™si saxlanД±lsД±n"
        : "Build artefaktД± staging-dЙ™ smoke testdЙ™n keГ§irilmЙ™lidir",
    },
  ];
  const blockers = items.filter((item) => item.status === "Bloker").length;
  const watch = items.filter((item) => item.status === "NЙ™zarЙ™t" || item.status === "HazД±rlanД±r").length;
  const ready = items.filter((item) => item.status === "HazД±r").length;
  const score = Math.max(0, Math.round((ready / items.length) * 100 - blockers * 8));

  return {
    checkedAt: new Date().toISOString(),
    status: blockers > 0 ? "Bloker var" : watch > 0 ? "Staging hazД±rdД±r" : "Go-live hazД±r",
    score,
    blockers,
    watch,
    ready,
...
function buildProductionHardeningReport(snapshot = {}, { goLiveReport = {}, apiWebhookRows = [], apiSecretRows = [], notificationProviderRows = [] } = {}) {
  ...
  const secretsHealthy = apiSecretRows.length > 0 && apiSecretRows.every((secret) => !normalize(secret.health).includes("tapД±lmadД±"));

  const items = [
    {
      area: "Backend sЙ™rtlЙ™ЕџdirmЙ™",
      status: isServerRuntime && immutableAudit ? "HazД±r" : "NЙ™zarЙ™t",
      score: isServerRuntime && immutableAudit ? 100 : immutableAudit ? 70 : 45,
      detail: `${dbMeta.provider || defaultDbProvider} В· ${dbMeta.runtime || "browser"} В· audit ${dbMeta.auditMode || "client audit"}`,
      next: isServerRuntime ? "Server-side permission vЙ™ migration yoxlamasД± davamlД± saxlanД±lsД±n" : "Frontend local cache Й™sas data mЙ™nbЙ™yi olmamalД±dД±r",
    },
    {
      area: "Backup/Restore",
      status: backupReady ? "HazД±r" : "NЙ™zarЙ™t",
      score: backupReady ? 100 : 65,
      detail: dbMeta.lastBackupAt || dbMeta.lastRestoreAt || "Backup izi hЙ™lЙ™ yaradД±lmayД±b",
      next: backupReady ? "GГјndЙ™lik avtomatik server snapshot planlaЕџdД±rД±lsД±n" : "Д°lk backup export/import vЙ™ ya server snapshot yaradД±lmalД±dД±r",
    },
    {
      area: "Deployment monitorinqi",
      status: deploymentReady && goLiveReport.score >= 70 ? "HazД±r" : "NЙ™zarЙ™t",
      score: deploymentReady ? Math.max(70, Number(goLiveReport.score || 70)) : 55,
      detail: `Go-live ${goLiveReport.status || "yoxlanmayД±b"} В· deploy toolkit ${deploymentToolkitReady ? "hazД±r" : "yoxdur"}`,
      next: "Health endpoint, smoke test vЙ™ deploy nЙ™ticЙ™si release checklist-dЙ™ saxlanmalД±dД±r",
    },
    {
      area: "Real provider inteqrasiyalarД±",
      status: apiReady && activeNotificationProviders.length >= 2 && secretsHealthy ? "HazД±r" : "NЙ™zarЙ™t",
      score: apiReady && activeNotificationProviders.length >= 2 && secretsHealthy ? 95 : 68,
      detail: `${activeNotificationProviders.length} bildiriЕџ provider В· ${apiWebhookRows.length} webhook В· ${apiSecretRows.length} secret`,
      next: "SMS/email/payment provider-lЙ™r backend secret-lЙ™rlЙ™ canlД± mГјhitdЙ™ yoxlanmalД±dД±r",
    },
    {
      area: "Kodun modullara parГ§alanmasД±",
      status: "NЙ™zarЙ™t",
      score: 60,
      detail: "UI vЙ™ biznes mЙ™ntiqi hЙ™lЙ™ Й™sasЙ™n App qatД±ndadД±r",
      next: "SatД±Еџ, kredit, anbar, maliyyЙ™, HR, KPI vЙ™ API ayrД±ca route/module paketlЙ™rinЙ™ Г§Д±xarД±lmalД±dД±r",
    },
  ];
  const score = Math.round(items.reduce((sum, item) => sum + item.score, 0) / items.length);
  const blockers = items.filter((item) => item.status === "Bloker").length;
  const watch = items.filter((item) => item.status === "NЙ™zarЙ™t").length;

  return {
    checkedAt: new Date().toISOString(),
    status: blockers > 0 ? "Bloker var" : watch > 0 ? "Production nЙ™zarЙ™ti" : "Production hazД±r",
    score,
    blockers,
    watch,
    ready: items.filter((item) => item.status === "HazД±r").length,
    items,
  };
}
...
function buildModuleReadiness(snapshot = {}, integrityReport = {}) {
  ...
  const approvedCashExpenses = expenses.filter((expense) => expense.status === "TЙ™sdiq edildi" && hasExpenseCashImpact(expense));
  const payrollExpense = expenses.find((expense) => expense.source === "HR Payroll");
  const openPurchaseOrders = purchaseOrders.filter((po) => !["TЙ™sdiq edildi", "LЙ™Дџv edildi"].includes(po.status));
  const approvedPurchaseOrders = purchaseOrders.filter((po) => po.status === "TЙ™sdiq edildi");
  ...
  const statusFrom = ({ blockers = 0, warnings = 0, hasData = true }) => {
    ...
    if (warnings > 0 || !hasData) return "NЙ™zarЙ™t";
    return "HazД±r";
  };
  ...
  const items = [
    {
      ...
      primary: `${orders.length} sifariЕџ`,
      secondary: `${creditOrders.length} kredit satД±Еџ`,
      detail: "SifariЕџ, rezerv, bonus, kredit vЙ™ mГјqavilЙ™ baДџlantД±sД±",
      next: salesLinkIssues.length
        ? "Kredit satД±Еџlarda mГјqavilЙ™/kredit linklЙ™rini tamamlayД±n"
        : orders.length
          ? "UAT ГјГ§Гјn real satД±Еџ ssenarilЙ™rini iЕџlЙ™din"
          : "Д°lk satД±Еџ sifariЕџini yaradД±n",
    },
    {
      module: "warehouse",
      title: "Anbar",
      ...
      primary: `${totalStock} Гјmumi stok`,
      secondary: `${reservedStock} rezerv`,
      detail: "QalД±q, minimum stok, mЙ™daxil, import/export vЙ™ PO Г¶rtГјyГј",
      next: lowStockRows.length
        ? "Minimumdan aЕџaДџД± mЙ™hsullar ГјГ§Гјn PO yaradД±n"
        : totalStock
          ? "Cycle count vЙ™ export yoxlamasД±nД± saxlayД±n"
          : "Anbara ilk mЙ™daxili edin",
    },
    {
      module: "deliveries",
      title: "TЙ™hvil",
      status: statusFrom({ blockers: blockedDeliveries.length, warnings: deliveryOrders.length, hasData: true }),
      primary: `${deliveryOrders.length} gГ¶zlЙ™yir`,
      secondary: `${blockedDeliveries.length} bloklu`,
      detail: "YalnД±z rezervli sifariЕџlЙ™r gГ¶rГјnГјr, TЙ™hvil verildi stokdan Г§Д±xД±r",
      next: blockedDeliveries.length
        ? "Bloklu tЙ™hvillЙ™rdЙ™ rezerv/stok uyДџunsuzluДџunu baДџlayД±n"
        : deliveryOrders.length
          ? "HazД±r sifariЕџlЙ™ri TЙ™hvil verildi ilЙ™ tamamlayД±n"
          : "AГ§Д±q tЙ™hvil nГ¶vbЙ™si yoxdur",
    },
    {
      module: "credits",
      title: "Kredit",
      status: statusFrom({ blockers: salesLinkIssues.length, warnings: overdueCredits.length, hasData: credits.length > 0 }),
      primary: `${credits.length} mГјqavilЙ™`,
      secondary: money(unpaidCreditBalance),
      detail: "HЙ™r kredit satД±ЕџД± ayrД±ca mГјqavilЙ™ vЙ™ Г¶dЙ™niЕџ cЙ™dvЙ™li kimi qalД±r",
      next: overdueCredits.length
        ? "GecikЙ™n kreditlЙ™r ГјzrЙ™ Г¶dЙ™niЕџ vЙ™ xatД±rlatma iЕџlЙ™din"
        : credits.length
          ? "AylД±q Г¶dЙ™niЕџ tarixlЙ™rini UAT-da yoxlayД±n"
          : "Kredit satД±Еџ yaradД±n",
    },
    {
      module: "crm",
      title: "CRM 360",
      status: statusFrom({ warnings: customers.length > 0 && customer360Rows.length === 0 ? 1 : 0, hasData: customers.length > 0 }),
      primary: `${customers.length} mГјЕџtЙ™ri`,
      secondary: `${customer360Rows.length} 360 kart`,
      detail: "MГјЕџtЙ™ri, mГјqavilЙ™, cihaz, Г¶dЙ™niЕџ vЙ™ qalД±q borc gГ¶rГјnГјЕџГј",
      next: customers.length
        ? "MГјЕџtЙ™ri kartlarД±nda kredit vЙ™ satД±Еџ tarixГ§Й™sini yoxlayД±n"
        : "Д°lk mГјЕџtЙ™rini yaradД±n",
    },
    {
      module: "finance",
      title: "MaliyyЙ™",
      status: statusFrom({ warnings: expenses.length + cashEntries.length === 0 ? 1 : 0, hasData: true }),
      primary: `${cashEntries.length} kassa giriЕџi`,
      secondary: `${approvedCashExpenses.length} tЙ™sdiqli xЙ™rc`,
      detail: "SatД±Еџ, kredit Г¶dЙ™niЕџi, PO xЙ™rci vЙ™ payroll accrual ayrД±lД±r",
      next: payrollExpense?.cashImpact === false
        ? "Kassa/bank hesablarД± ГјzrЙ™ real aГ§Д±lД±Еџ balanslarД±nД± yoxlayД±n"
        : "HR payroll cash-neutral accrual kimi yaradД±lmalД±dД±r",
    },
    {
      module: "vendors",
      title: "Vendor + PO",
      ...
      secondary: `${approvedPurchaseOrders.length} tЙ™sdiqli`,
      detail: "Zavod sifariЕџi, tЙ™sdiq, anbara mЙ™daxil vЙ™ maliyyЙ™ xЙ™rci",
      next: missingVendorPo.length
        ? "Vendoru olmayan PO-larД± vendor kartД±na baДџlayД±n"
        : openPurchaseOrders.length
          ? "AГ§Д±q PO-larД± tЙ™sdiqlЙ™yin vЙ™ anbara mЙ™daxil edin"
          : "Minimum stok siqnallarД±ndan yeni PO yaradД±n",
    },
    {
      module: "hr",
      title: "HR",
      ...
      primary: `${employees.length} Й™mЙ™kdaЕџ`,
      secondary: `${departments.length} ЕџГ¶bЙ™`,
      detail: "Struktur aДџacД±, Й™mЙ™kdaЕџ CRUD, mЙ™zuniyyЙ™t, vakansiya vЙ™ payroll",
      next: employeesWithManagers.length
        ? "Struktur aДџacД±nД± periodik yenilЙ™yin"
        : "Kim kimЙ™ tabedir mЙ™lumatД±nД± tamamlayД±n",
    },
    {
      module: "kpi",
      title: "KPI",
      status: statusFrom({ warnings: salesBonuses.length === 0 ? 1 : 0, hasData: employees.length > 0 }),
      primary: `${salesBonuses.length} bonus sЙ™tri`,
      secondary: `${employees.length} Й™mЙ™kdaЕџ`,
      detail: "SatД±Еџ bonuslarД± KPI nЙ™ticЙ™lЙ™rinЙ™ ayrД±ca dГјЕџГјr",
      next: salesBonuses.length
        ? "AylД±q bonus filterlЙ™rini yoxlayД±n"
        : "Bonuslu satД±Еџ yaradД±n",
    },
  ];
  ...
  return {
    checkedAt: new Date().toISOString(),
    status: blockers > 0 ? "Bloker var" : watch > 0 ? "TЙ™hvil nЙ™zarЙ™tdЙ™" : "TЙ™hvilЙ™ hazД±r",
    ready,
    watch,
    blockers,
    score,
...
function buildPayrollExpense(employees) {
  ...
  return {
    ...
    status: "TЙ™sdiq gГ¶zlЙ™yir",
    source: "HR Payroll",
    cashImpact: false,
    cashImpactNote: "HR payroll uГ§ot/accrual xЙ™rcidir, real kassadan avtomatik Г§Д±xД±lmД±r.",
    grossTotal,
    deductions,
    netTotal,
    employerCost,
...
// Bu kolleksiyalar artД±q real Supabase cЙ™dvЙ™llЙ™rindЙ™ saxlanД±lД±r вЂ” blob snapshot-a yazД±lmД±r.
...
function App() {
  ...
  useEffect(() => {
    ...
    // KГ¶hnЙ™/alias URL-lЙ™rini kanonik yeni Гјnvana yГ¶nlЙ™ndir.
  ...
  const [notificationFilter, setNotificationFilter] = useState("CЙ™mi");
  ...
  const visibleNavItems = useMemo(
    () => navItems.filter((item) => {
      // "ЕћirkЙ™tlЙ™r" (platform) is only for super-admins
  ...
  useEffect(() => {
    ...
    setState((current) => {
      ...
      const sentCount = freshDeliveries.filter((item) => item.status === "GГ¶ndЙ™rildi").length;
      ...
      const providerUse = freshDeliveries.reduce((map, item) => {
        ...
        map.set(item.providerId, (map.get(item.providerId) || 0) + (item.status === "GГ¶ndЙ™rildi" ? 1 : 0));
        return map;
      }, new Map());
      ...
      return appendAudit(
        {
            ...
            touchedRuleIds.has(rule.id)
              ? {
                  ...
                  sentCount:
                    ...
                    freshDeliveries.filter((item) => item.ruleId === rule.id && item.status === "GГ¶ndЙ™rildi").length,
                  failedCount:
                    Number(rule.failedCount || 0) +
                    freshDeliveries.filter((item) => item.ruleId === rule.id && item.status !== "GГ¶ndЙ™rildi").length,
                  lastStatus: freshDeliveries.some((item) => item.ruleId === rule.id && item.status !== "GГ¶ndЙ™rildi")
                    ? "QismЙ™n bloklandД±"
                    : "GГ¶ndЙ™rildi",
                }
              : rule,
          ...
          notifications: [
            ...freshDeliveries.slice(0, 10).map((delivery) => ({
              ...
              body: `${delivery.recipient} В· ${delivery.body}`,
              time: stamp,
              unread: true,
        ...
        {
          module: "BildiriЕџ",
          action: "Avtomatik kredit monitoru iЕџlЙ™di",
          detail: `${sentCount} gГ¶ndЙ™rildi В· ${blockedCount} bloklandД± В· ${creditRules.length} qayda`,
          status: "Avtomatik",
          role: "System",
        },
      );
  ...
  useEffect(() => {
    ...
    getRemoteSession()
      ...
      .catch((error) => {
        ...
        setRemoteAuthStatus("signedOut");
        setAuthError(error instanceof Error ? error.message : "Server sessiyasД± aГ§Д±lmadД±.");
      });
  ...
  useEffect(() => {
    try {
      ...
      notify("Local DB yazД±lД±ЕџД± mГјmkГјn olmadД±.", "warning");
    }
        ...
        supabase
          ...
          .then(({ error }) => {
            if (error) {
              ...
              setAuthError("ЕћirkЙ™t datasД± serverdЙ™ saxlanmadД±. Administrator icazЙ™sini yoxlayД±n.");
            }
          });
      }, 800);
    }
    ...
    remoteSaveTimer.current = window.setTimeout(() => {
      saveRemoteState(state).catch((error) => {
        setAuthError(error instanceof Error ? error.message : "ServerЙ™ yazД±lД±Еџ alД±nmadД±.");
      });
    }, 500);
    return () => {
      window.clearTimeout(remoteSaveTimer.current);
      ...
      .upsert(
        {
          ...
          action: entry.action || "ЖЏmЙ™liyyat",
          detail: entry.detail || "",
          status: entry.status || "TamamlandД±",
          payload: entry,
  ...
  useEffect(() => {
    ...
    setState((current) => {
      ...
      return appendAudit(
        ...
        {
          module: "HR/MaliyyЙ™",
          action: existing ? "Payroll expense yenilЙ™ndi" : "Payroll expense yaradД±ldД±",
          detail: `${money(payrollExpense.amount)} payroll maliyyЙ™yЙ™ avtomatik dГјЕџdГј`,
          status: "Avtomatik",
          role: "System",
        },
      );
  ...
  const dashboardStats = useMemo(() => {
    const openOrders = state.orders.filter((order) => order.status !== "TЙ™hvil verilib");
    const pending = state.expenses.filter((expense) => expense.status === "TЙ™sdiq gГ¶zlЙ™yir");
    return {
  ...
  const handleGitHubPush = useCallback(
    (commit) => {
      notify(`GitHub-a son push uДџurlu: ${commit.sha}`, "success");
    },
    [notify]
  );
  ...
  function requirePermission(permission, action) {
    ...
    const roleName = activeRoleInfo?.name || "Rol seГ§ilmЙ™yib";
    notify(`${roleName}: ${action} ГјГ§Гјn icazЙ™ yoxdur.`, "warning");
    setState((current) =>
      appendAudit(current, {
        module: "Permission",
        action: "ЖЏmЙ™liyyat bloklandД±",
        detail: `${roleName}: ${action}`,
        status: "Д°cazЙ™ yoxdur",
        role: roleName,
      }),
    );
    return false;
  ...
  function getCreateAudit(type, values) {
    const auditByType = {
      crm: { module: "CRM", action: "MГјЕџtЙ™ri yaradД±ldД±", detail: values.name },
      sales: {
        module: "SatД±Еџ",
        action: values.paymentMethod === "Kredit" ? "Kredit satД±Еџ yaradД±ldД±" : "SatД±Еџ yaradД±ldД±",
        detail: `${values.customer || "MГјЕџtЙ™ri"} В· ${money(Number(values.orderTotal ?? values.amount ?? 0))}`,
      },
      dashboard: {
        module: "SatД±Еџ",
        action: values.paymentMethod === "Kredit" ? "Kredit satД±Еџ yaradД±ldД±" : "SatД±Еџ yaradД±ldД±",
      ...
      finance: {
        module: "MaliyyЙ™",
        action: "XЙ™rc yaradД±ldД±",
        detail: `${values.description || "XЙ™rc"} В· ${money(Number(values.amount || 0))}`,
      },
      credits: {
        module: "Kredit",
        action: "Manual kredit yaradД±ldД±",
        detail: `${values.customer || "MГјЕџtЙ™ri"} В· ${money(Number(values.total || 0))}`,
      },
      vendors: { module: "Vendor", action: "Vendor yaradД±ldД±", detail: values.name },
      product: { module: "MЙ™hsul", action: "MЙ™hsul yaradД±ldД±", detail: values.name },
      hr: { module: "HR", action: "ЖЏmЙ™kdaЕџ yaradД±ldД±", detail: values.name },
      contracts: { module: "MГјqavilЙ™", action: "MГјqavilЙ™ yaradД±ldД±", detail: values.customer },
    };

    return auditByType[type] || { module: "Sistem", action: "Qeyd yaradД±ldД±", detail: type };
  }
  ...
  function hasCreatePermission(type, values) {
    ...
    const action = isCreditSale ? "kredit satД±Еџ yaratmaq" : createConfig[type]?.title || "qeyd yaratmaq";
    return requirePermission(permission, action);
  }

  function choosePage(id) {
    if (!canAccessNavItem(state.settings, id)) {
      notify("Bu modul aktiv istifadЙ™Г§i ГјГ§Гјn gizlЙ™dilib.", "warning");
      return;
    }
    setActive(id);
    setMobileNav(false);
  ...
  function loginUser(userId) {
    ...
    if (!user) {
      notify("Aktiv istifadЙ™Г§i seГ§in.", "warning");
      return;
    }
    ...
    setState((current) =>
      appendAudit(
        ...
        {
          module: "Auth",
          action: "GiriЕџ edildi",
          detail: `${user.name} В· ${user.role}`,
          role: user.role,
        },
      ),
    );
    notify(`${user.name} kimi giriЕџ edildi.`);
  }
  ...
  async function loginWithPassword({ email, password }) {
    ...
    try {
      ...
      if (login.user.mustChangePassword) {
        setRemoteAuthStatus("signedIn");
        notify("Д°lk giriЕџ ГјГ§Гјn yeni parol tЙ™yin edin.");
        return;
      }
      const payload = await loadRemoteState();
      const tenantUsers = payload.state?.settings?.users || initialState.settings.users || [];
      ...
      setRemoteAuthStatus("signedIn");
      notify(`${login.user.name} kimi giriЕџ edildi.`);
    } catch (error) {
      setRemoteToken("");
      setRemoteUser(null);
      setRemoteAuthStatus("signedOut");
      setAuthError(error instanceof Error ? error.message : "GiriЕџ alД±nmadД±.");
    }
  }

  function logoutUser() {
    const userName = currentUser?.name || "Д°stifadЙ™Г§i";
    setState((current) =>
      appendAudit(
        {
          ...current,
    ...
    notify(`${userName} sistemdЙ™n Г§Д±xdД±.`);
  }

  async function createUser(values) {
    if (!requirePermission("settings.manage", "istifadЙ™Г§i yaratmaq")) return;
    ...
    if (!userName || !email) {
      notify("Д°stifadЙ™Г§i adД± vЙ™ email daxil edin.", "warning");
      return;
    }
    ...
    if (remoteApiEnabled) {
      ...
      if (password.length < 8) {
        notify("Server istifadЙ™Г§isi ГјГ§Гјn Й™n azД± 8 simvoldan ibarЙ™t parol daxil edin.", "warning");
        return;
      }
      try {
        ...
        notify(error instanceof Error ? error.message : "Д°stifadЙ™Г§i yaradД±lmadД±.", "warning");
        return;
      }
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Ayarlar/Auth",
          action: "Д°stifadЙ™Г§i yaradД±ldД±",
          detail: `${user.name} В· ${user.role}`,
        },
      ),
    );
    notify(`${user.name} istifadЙ™Г§isi yaradД±ldД±.`);
  }

  function updateUserStatus(userId, status) {
    if (!requirePermission("settings.manage", "istifadЙ™Г§i statusunu dЙ™yiЕџmЙ™k")) return;
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "Ayarlar/Auth",
          action: "Д°stifadЙ™Г§i statusu dЙ™yiЕџdi",
  ...
  function toggleUserModuleAccess(userId, moduleId) {
    if (!requirePermission("settings.manage", "istifadЙ™Г§i modul icazЙ™sini dЙ™yiЕџmЙ™k")) return;
    ...
    setState((current) => {
      ...
      if (module?.permission && !(role?.permissions || []).includes(module.permission)) {
        return appendAudit(current, {
          module: "Ayarlar/Auth",
          action: "Modul icazЙ™si bloklandД±",
      ...
      return auditCurrentState(
        ...
        {
          module: "Ayarlar/Auth",
          action: "Modul icazЙ™si dЙ™yiЕџdi",
  ...
  function runInvoiceAction() {
    const targetInvoice =
      invoiceRows.find((invoice) => invoice.eTaxStatus === "GГ¶ndЙ™riЕџЙ™ hazД±r") ||
      invoiceRows.find((invoice) => !invoice.invoiceSentAt);

    if (!targetInvoice) {
      notify("GГ¶ndЙ™rilЙ™cЙ™k faktura tapД±lmadД±.", "warning");
      return;
    }
      ...
      auditCurrentState(
        {
          ...
          orders: current.orders.map((order) =>
            order.id === targetInvoice.orderId
              ? {
                  ...order,
                  eTaxStatus: "E-qaimЙ™ gГ¶ndЙ™rildi",
        ...
        {
          module: "Faktura",
          action: "E-qaimЙ™ gГ¶ndЙ™rildi",
          detail: `${targetInvoice.id} В· ${targetInvoice.customer} В· ${batchId}`,
        },
      ),
    );
    notify(`${targetInvoice.id} e-qaimЙ™ nГ¶vbЙ™sinЙ™ gГ¶ndЙ™rildi.`);
  }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          ...
          detail: `${journalCount} jurnal sЙ™tri export ГјГ§Гјn hazД±rlandД±`,
        },
      ),
    );
    notify("Jurnal export hazД±rlandД± vЙ™ mГјhasibat baДџlanД±ЕџД±na yazД±ldД±.");
  }

  function runTaxAction() {
    const targetTax =
      taxCalendarRows.find(
        (row) => ["Gecikib", "Bu gГјn", "YaxД±nlaЕџД±r"].includes(row.status) && !row.paymentTaskId,
      ) ||
      taxCalendarRows.find((row) => Number(row.amount || 0) > 0 && !row.paymentTaskId);

    if (!targetTax) {
      notify("Aktiv vergi Г¶hdЙ™liyi tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) => {
      ...
      const taxCalendar = currentTaxCalendar.some((item) => item.id === targetTax.id)
        ? currentTaxCalendar.map((item) =>
            item.id === targetTax.id
              ? {
                  ...item,
                  paymentStatus: "Г–dЙ™niЕџ tapЕџД±rД±ДџД±",
      ...
      return auditCurrentState(
        ...
        {
          module: "Vergi/MaliyyЙ™",
          action: "Г–dЙ™niЕџ tapЕџД±rД±ДџД± yaradД±ldД±",
          detail: `${targetTax.title} В· ${money(amount)}`,
        },
      );
    });
    notify(`${targetTax.title} ГјГ§Гјn Г¶dЙ™niЕџ tapЕџД±rД±ДџД± maliyyЙ™yЙ™ dГјЕџdГј.`);
  }
  ...
  function runReceivableSyncAction() {
    ...
    const highRiskCount = receivableRows.filter((row) => ["Kritik", "YГјksЙ™k"].includes(row.riskCategory)).length;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          ...
          detail: `${receivableRows.length} tЙ™rЙ™f В· net ${money(debtorTotal - creditorTotal)}`,
        },
      ),
    );
    notify("Debitor/kreditor reyestri yenilЙ™ndi.");
  }

  function closeReceivableDebt(rowId) {
    if (!requirePermission("receivables.manage", "debitor/kreditor borcunu baДџlamaq")) return;

    const targetRow = receivableRows.find((row) => row.id === rowId);
    if (!targetRow || Number(targetRow.amount || 0) <= 0) {
      notify("BaДџlanacaq aГ§Д±q borc tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) => {
      ...
      if (targetRow.type === "Debitor") {
        ...
        const nextOrders = (current.orders || []).map((order) => {
          ...
          if (paymentResult) {
            nextOrder = {
              ...
              paymentStatus: paymentResult.nextBalance <= 0 ? "Г–dЙ™nilib" : "Kredit satД±Еџ",
            };
          }
        ...
        const cashEntry = {
          ...
          date: baseFinanceDate,
          note: `${targetRow.detail} В· ${targetRow.riskCategory}`,
          closureId,
        };
        const closure = {
          ...closureBase,
        ...
        return auditCurrentState(
          {
            ...
            customers: (current.customers || []).map((customer) =>
              ...
              (customer.fin === targetRow.source || normalize(customer.name) === normalize(targetRow.party))
                ? { ...customer, debt: 0, delay: 0, receivableStatus: "BaДџlandД±" }
                : customer,
            ),
            orders: nextOrders,
            credits: nextCredits,
          ...
          {
            module: "Debitor/Kreditor",
            action: "Debitor borcu baДџlandД±",
            detail: `${targetRow.party} В· ${money(cashEntry.amount)} В· ${targetRow.riskCategory}`,
          },
        );
      }
      ...
      const paymentExpenses = payablePos
        ...
        .map((po) => ({
          id: `EXP-${po.id}`,
          description: `Kreditor baДџlanД±ЕџД± - ${po.product}`,
          category: "Vendor Г¶dЙ™niЕџi",
          date: baseFinanceDate,
          amount: Number(po.amount || 0),
          status: "TЙ™sdiq edildi",
          source: "Debitor/Kreditor",
          cashImpact: true,
          poId: po.id,
          vendor: po.vendor,
      ...
      return auditCurrentState(
        ...
        {
          module: "Debitor/Kreditor",
          action: "Kreditor borcu baДџlandД±",
          detail: `${targetRow.party} В· ${money(closedAmount)} В· ${targetRow.riskCategory}`,
        },
      );
    });

    notify(`${targetRow.party} ГјzrЙ™ borc baДџlanД±Еџ workflow-u tamamlandД±.`, "success");
  }

  function exportReport(title = "PDF export", format = "PDF") {
    if (!requirePermission("reports.export", "hesabat export etmЙ™k")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Hesabat",
          action: "Export hazД±rlandД±",
          detail: `${title} В· ${format} В· ${exportRow.rows} sЙ™tir В· score ${reportPackage.score}`,
        },
      ),
    );
    notify(`${title} ${format} export siyahД±sД±na Й™lavЙ™ edildi.`);
    queueMicrotask(() => {
      ...
      }).catch((error) => notify(`Hesabat exportu sinxronlaР•СџmadР”В±: ${error.message}`, "warning"));
    });
  }

  function runProjectsExportAction() {
    if (projectRoiRows.length === 0) {
      notify("Export ГјГ§Гјn layihЙ™ tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          module: "LayihЙ™ ROI",
          action: "ROI export",
          detail: `${projectRoiRows.length} layihЙ™ В· gЙ™lir ${money(summary.revenue)} В· ROI ${percent(summary.avgRoi)}`,
        },
      ),
    );
    notify("ROI export hazД±rlandД± vЙ™ layihЙ™lЙ™rЙ™ qeyd edildi.");
  }
  ...
  function runProductionAction() {
    ...
    setState((current) => {
      ...
      if (planMaterials.length === 0) {
        notify("Д°stehsal planД± ГјГ§Гјn anbarda xammal tapД±lmadД±.", "warning");
        return current;
      }
      ...
      const plan = {
        ...
        product: "Yeni satД±Еџ komplekti",
        plannedQty: maxPlannedQty,
        warehouseId,
        warehouseName: warehouse?.name || "Anbar seГ§ilmЙ™yib",
      ...
      return auditCurrentState(
        ...
        {
          module: "Д°stehsalat",
          action: "Plan yaradД±ldД±",
          detail: `${plan.id} В· ${plan.warehouseName} В· ${plan.materials.map((item) => item.product).join(", ")}`,
        },
      );
    });
    notify("Yeni istehsal planД± yaradД±ldД±.");
  }
  ...
  function syncProductionWorkflow(plan, status, extraPayload = {}) {
    ...
    }).catch((error) => notify(`Д°stehsal workflow sinxronlaЕџmadД±: ${error.message}`, "warning"));
  }
  ...
  function syncHrWorkflow(recordType, item, status, approvals) {
    ...
    void saveWorkflowRecord({
      ...
      approvals,
    }).catch((error) => notify(`HR workflow sinxronlaР•СџmadР”В±: ${error.message}`, "warning"));
  }
  ...
  function syncCommunicationWorkflow(thread, status = "active") {
    ...
    void saveWorkflowRecord({
      ...
      record: {
        ...
        record_no: thread.id,
        status,
        title: thread.title || thread.person || "Daxili yazР”В±Р•Сџma",
        payload: {
    ...
    }).catch((error) => notify(`Mesaj workflow sinxronlaР•СџmadР”В±: ${error.message}`, "warning"));
  }
    ...
    deliveries.forEach((delivery) => {
      ...
      void queueNotification({
        ...
        notification: {
          ...
          body: delivery.body || delivery.subject || "BildiriР•Сџ",
      ...
      }).catch((error) => notify(`BildiriР•Сџ logu sinxronlaР•СџmadР”В±: ${error.message}`, "warning"));
    });
  }

  function createProductionPlan(values) {
    if (!requirePermission("production.manage", "istehsal planД± yaratmaq")) return;
    const stamp = getActionStamp();
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "Д°stehsalat",
          action: "Д°stehsal planД± yaradД±ldД±",
          detail: `${plan.id} В· ${plan.product} В· ${plan.plannedQty} Й™dЙ™d В· ${plan.warehouseName}`,
        },
      );
    });
    queueMicrotask(() => createdPlan && syncProductionWorkflow(createdPlan, "draft"));
    notify("Д°stehsal planД± vЙ™ BOM yaradД±ldД±.");
  }

  function updateProductionPlan(planId, values) {
    if (!requirePermission("production.manage", "istehsal planД±nД± redaktЙ™ etmЙ™k")) return;
    const stamp = getActionStamp();
    let syncedPlan = null;
    ...
    setState((current) => {
      ...
      if (normalize(existing.status).includes("istehsal edildi") || normalize(existing.status).includes("istehsaldadД±r")) {
        notify("BaЕџlanmД±Еџ vЙ™ ya tamamlanmД±Еџ istehsal planД± redaktЙ™ edilЙ™ bilmЙ™z.", "warning");
        return current;
      }
      const warehouse = current.warehouses.find((item) => item.id === values.warehouseId);
      const updated = {
      ...
      return auditCurrentState(
        ...
        {
          module: "Д°stehsalat",
          action: "Д°stehsal planД± redaktЙ™ edildi",
          detail: `${planId} В· ${updated.product} В· ${updated.plannedQty} Й™dЙ™d`,
        },
      );
    });
    queueMicrotask(() => syncedPlan && syncProductionWorkflow(syncedPlan, "draft"));
    notify("Д°stehsal planД±ndakД± dЙ™yiЕџikliklЙ™r saxlanД±ldД±.");
  }

  function deleteProductionPlan(planId) {
    if (!requirePermission("production.manage", "istehsal planД±nД± silmЙ™k")) return;
    let cancelledPlan = null;
    setState((current) => {
      const existing = (current.productionPlans || []).find((item) => item.id === planId);
      if (!existing) return current;
      if (normalize(existing.status).includes("istehsal edildi") || normalize(existing.status).includes("istehsaldadД±r")) {
        notify("BaЕџlanmД±Еџ vЙ™ ya tamamlanmД±Еџ plan audit vЙ™ stok izi sЙ™bЙ™bilЙ™ silinЙ™ bilmЙ™z.", "warning");
        return current;
      }
      ...
      return auditCurrentState(
        ...
        {
          module: "Д°stehsalat",
          action: "Д°stehsal planД± silindi",
          detail: `${planId} В· ${existing.product}`,
          status: "Silindi",
        },
      );
    });
    queueMicrotask(() => cancelledPlan && syncProductionWorkflow(cancelledPlan, "cancelled", { cancelled: true }));
    notify("Д°stehsal planД± silindi.");
  }

  function startProductionPlan(planId) {
    if (!requirePermission("production.manage", "istehsala baЕџlamaq")) return;
    const stamp = getActionStamp();
    ...
    setState((current) => {
      ...
      if (!plan || !plan.canProduce) {
        notify(`Д°stehsala baЕџlamaq mГјmkГјn deyil: ${plan?.bottleneck || "plan tapД±lmadД±"}.`, "warning");
        return current;
      }
      startedPlan = { ...plan, status: "Д°stehsaldadД±r", startedAt: stamp };
      return auditCurrentState(
        {
          ...current,
          productionPlans: (current.productionPlans || []).map((item) =>
            item.id === planId ? { ...item, status: "Д°stehsaldadД±r", startedAt: stamp, updatedAt: stamp } : item,
          ),
        },
        {
          module: "Д°stehsalat",
          action: "Д°stehsala baЕџlandД±",
          detail: `${plan.id} В· ${plan.product} В· ${plan.plannedQty} Й™dЙ™d`,
        },
      );
    });
    queueMicrotask(() => startedPlan && syncProductionWorkflow(startedPlan, "in_progress", { material_reserved: true }));
    notify(`${planId} istehsala buraxД±ldД±.`);
  }

  function completeProductionPlan(planId) {
    if (!requirePermission("production.manage", "istehsal planД±nД± tamamlamaq")) return;
    ...
    setState((current) => {
      ...
      const plan = rows.find((item) => item.id === planId);
      if (!plan) {
        notify("Д°stehsal planД± tapД±lmadД±.", "warning");
        return current;
      }
      if (normalize(plan.status).includes("istehsal edildi")) {
        notify("Bu istehsal planД± artД±q tamamlanД±b.", "warning");
        return current;
      }
      if (!plan.canProduce) {
        notify(`${plan.id} tamamlanmadД±: ${plan.bottleneck}.`, "warning");
        return current;
      }
      ...
      const finishedProduct = existingProduct
        ? {
            ...existingProduct,
            category: existingProduct.category || "Д°stehsalat",
            unit: existingProduct.unit || "Й™dЙ™d",
      ...
      return auditCurrentState(
        ...
        {
          module: "Д°stehsalat/Anbar",
          action: "Xammal Г§Д±xД±ЕџД± vЙ™ hazД±r mЙ™hsul mЙ™daxili",
          detail: `${plan.id} В· ${finishedProduct.name} ${plan.plannedQty} Й™dЙ™d В· maya ${money(plan.unitCost)}`,
        },
      );
    });
    queueMicrotask(() => syncedCompletedPlan && syncProductionWorkflow(syncedCompletedPlan, "completed", { stock_posted: true }));
    notify(`${planId} ГјzrЙ™ xammal Г§Д±xД±ldД± vЙ™ hazД±r mЙ™hsul anbara mЙ™daxil edildi.`);
  }
  ...
  function runSupportAction() {
    ...
    const ticket = {
      ...
      title: signal ? `${signal.title}: ${signal.detail}` : "Modul daxili yoxlama sorДџusu",
      ...
      status: "AГ§Д±q",
      owner: signal?.module === "credits" ? "MaliyyЙ™Г§i" : "Admin",
      slaHours: signal?.priority === "YГјksЙ™k" ? 4 : 12,
      createdAt: stamp,
      ...
      tasks: [
        {
          ...
          title: signal?.title || "Platform yoxlamasД±",
          owner: signal?.module === "credits" ? "MaliyyЙ™Г§i" : "Admin",
          dueAt: stamp,
          status: "AГ§Д±q",
        },
      ...
      comments: [
        {
          ...
          text: `Task yaradД±ldД±: ${signal?.detail || "Гњmumi platform yoxlamasД±"}`,
          at: stamp,
          mine: true,
        },
      ],
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Support",
          action: "SorДџu yaradД±ldД±",
          detail: `${ticket.id} В· ${ticket.title} В· ${ticket.linkedLabel}`,
        },
      ),
    );
    setSelectedSupportTicketId(ticket.id);
    selectMessageThread(getSupportThreadId(ticket));
    notify(`${ticket.id} support nГ¶vbЙ™sinЙ™ Й™lavЙ™ edildi.`);
  }
  ...
  function runHelpAction() {
    ...
    const article = {
      ...
      title: "Д°stifadЙ™Г§i tЙ™lim paketi yenilЙ™ndi",
      category: "TЙ™lim",
      answer: "Modul izahlarД±, permission qaydalarД±, onboarding addД±mlarД± vЙ™ hesabat/audit izi yenidЙ™n formalaЕџdД±rД±ldД±.",
      tags: ["tЙ™lim", "modul", "onboarding", "audit"],
      createdAt: stamp,
    };
    const guideSnapshot = {
      generatedAt: stamp,
      ...
      readyModules: helpModuleGuides.filter((guide) => guide.readiness === "HazД±r").length,
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "KГ¶mЙ™k",
          action: "TЙ™lim paketi yenilЙ™ndi",
          detail: `${guideSnapshot.modules} modul В· ${guideSnapshot.onboardingSteps} onboarding addД±mД±`,
        },
      ),
    );
    notify("Д°stifadЙ™Г§i tЙ™limatД± vЙ™ modul izahlarД± yenilЙ™ndi.");
  }
  ...
  function runOnboardingAction() {
    ...
    if (!nextStep) {
      notify("Onboarding addД±mlarД±nД±n hamД±sД± faktiki mЙ™lumatlarla tamamlanД±b.");
      return;
    }
    ...
    auditOperation({
      module: "Onboarding",
      action: "Qurulum addД±mД±na keГ§id",
      detail: `${nextStep.id} В· ${nextStep.title}`,
    });
    notify(`${nextStep.title} ГјГ§Гјn uyДџun modul aГ§Д±ldД±.`);
  }

  function rotateApiSecret(secretId) {
    if (!requirePermission("api.manage", "API secret rotasiya etmЙ™k")) return;
    ...
    setState((current) => {
      ...
      const nextSecret = {
        ...secret,
        maskedValue: `${prefix}_вЂўвЂўвЂўвЂўвЂўвЂўвЂўвЂў${suffix}`,
        status: "Aktiv",
      ...
      return auditCurrentState(
        ...
        },
        {
          ...
          detail: `${secret.key} В· v${nextSecret.version} В· ${stamp}`,
        },
      );
    });
    notify("API secret/token rotasiyasД± tamamlandД±.");
  }
  ...
  function runApiAction(mode = "auto") {
    ...
    if (!targetWebhook) {
      notify("Webhook qaydasД± tapД±lmadД±.", "warning");
      return;
    }

    const stamp = getActionStamp();
    let toast = `${targetWebhook.id} webhook testi iЕџlЙ™nir.`;
    ...
    setState((current) => {
      ...
      const result = success ? "UДџurlu" : nextFailureCount >= retryMax ? "BloklandД±" : "Retry gГ¶zlЙ™yir";
      const latencyMs = success ? 118 + Math.round(Math.random() * 35) : 620 + Math.round(Math.random() * 120);
      const error = success
        ? ""
        : !secretReady
          ? "Secret/token aktiv deyil vЙ™ ya tapД±lmadД±"
          : "Endpoint cavab vermЙ™di";
      const log = {
        ...
        error,
        mode: isRetry ? "Retry test" : "CanlД± test",
        at: stamp,
      ...
      const nextWebhook = {
        ...
        retryQueue: nextRetryQueue,
        lastPayloadOverride: `${sourceWebhook.event} test В· ${stamp}`,
        lastTestAt: stamp,
      ...
      toast = success
        ? `${sourceWebhook.id} webhook testi uДџurludur (${responseCode}).`
        : `${sourceWebhook.id} test uДџursuz oldu, retry nГ¶vbЙ™sinЙ™ yazД±ldД±.`;
      ...
      return auditCurrentState(
        ...
        {
          module: "API/Webhook",
          action: isRetry ? "Webhook retry testi" : "Webhook test nЙ™ticЙ™si",
          detail: `${sourceWebhook.id} В· ${sourceWebhook.event} В· ${responseCode} В· ${result}`,
        },
      );
    });
    notify(toast, toast.includes("uДџursuz") ? "warning" : "success");
  }

  function runNotificationDispatchAction() {
    if (!requirePermission("notifications.manage", "bildiriЕџ nГ¶vbЙ™sini gГ¶ndЙ™rmЙ™k")) return;

    const readyRules = notificationAutomationRows.filter((rule) => rule.queueCount > 0 && rule.status === "Aktiv");
    if (readyRules.length === 0) {
      notify("GГ¶ndЙ™riЕџ ГјГ§Гјn aktiv bildiriЕџ nГ¶vbЙ™si yoxdur.", "warning");
      return;
    }
    ...
    const deliveries = buildNotificationDeliveriesForRules({
      ...
      settings: state.settings,
      stamp,
      source: "Manual nГ¶vbЙ™",
    });

    if (deliveries.length === 0) {
      notify("Qaydalarda hadisЙ™ tapД±lmadД±.", "warning");
      return;
    }

    const sentCount = deliveries.filter((item) => item.status === "GГ¶ndЙ™rildi").length;
      ...
      deliveries.forEach((delivery) => {
        ...
        void queueNotification({
          ...
          notification: {
            ...
            subject: delivery.subject || null,
            body: delivery.body || delivery.subject || "BildiriР•Сџ",
            status: delivery.status === "GР“В¶ndР™в„ўrildi" ? "sent" : "failed",
            sent_at: delivery.status === "GР“В¶ndР™в„ўrildi" ? delivery.sentAtIso : null,
            last_error: delivery.status === "GР“В¶ndР™в„ўrildi" ? null : delivery.status,
            metadata: {
              rule_id: delivery.ruleId,
              dedupe_key: delivery.dedupeKey,
              priority: delivery.priority,
    ...
    setState((current) =>
      auditCurrentState(
          ...
          notificationRules: ensureNotificationRules(current.notificationRules || []).map((rule) =>
            touchedRuleIds.has(rule.id)
              ? {
                  ...rule,
                  lastRunAt: stamp,
                  sentCount: Number(rule.sentCount || 0) + deliveries.filter((item) => item.ruleId === rule.id && item.status === "GГ¶ndЙ™rildi").length,
                  failedCount: Number(rule.failedCount || 0) + deliveries.filter((item) => item.ruleId === rule.id && item.status !== "GГ¶ndЙ™rildi").length,
                  lastStatus: deliveries.some((item) => item.ruleId === rule.id && item.status !== "GГ¶ndЙ™rildi") ? "QismЙ™n bloklandД±" : "GГ¶ndЙ™rildi",
                }
              : rule,
          ),
          notificationProviders: ensureNotificationProviders(current.notificationProviders || []).map((provider) =>
        ...
        {
          module: "BildiriЕџ",
          action: "Provider gГ¶ndЙ™riЕџ nГ¶vbЙ™si iЕџlЙ™ndi",
          detail: `${sentCount} gГ¶ndЙ™rildi В· ${blockedCount} bloklandД± В· ${readyRules.length} qayda`,
        },
      ),
    );

    notify(`BildiriЕџ nГ¶vbЙ™si iЕџlЙ™ndi: ${sentCount} gГ¶ndЙ™rildi, ${blockedCount} bloklandД±.`);
  }

  function runKpiPeriodAction(requestedAction = "next") {
    if (!requirePermission("kpi.manage", "KPI periodunu iЕџlЙ™mЙ™k")) return;

    const period = activeKpiPeriod.period || getKpiPeriodKey();
    const nextAction =
      requestedAction !== "next"
        ? requestedAction
        : activeKpiPeriod.status !== "Period baДџlandД±"
          ? "close"
          : activeKpiPeriod.approvalStatus !== "TЙ™sdiq edildi"
            ? "approve"
            : activeKpiPeriod.payoutStatus !== "Г–dЙ™nildi"
              ? "payout"
              : "complete";

    if (nextAction === "complete") {
      notify(`${period} KPI periodu artД±q baДџlanД±b, tЙ™sdiqlЙ™nib vЙ™ Г¶dЙ™nilib.`);
      return;
    }

    if (nextAction === "payout" && activeKpiPeriod.approvalStatus !== "TЙ™sdiq edildi") {
      notify("Payout ГјГ§Гјn Й™vvЙ™l KPI periodunu tЙ™sdiq edin.", "warning");
      return;
    }
    ...
    let toast =
      nextAction === "close"
        ? `${period} KPI periodu baДџlandД± vЙ™ tЙ™sdiq gГ¶zlЙ™yir.`
        : nextAction === "approve"
          ? `${period} KPI periodu tЙ™sdiq edildi.`
          : `${period} KPI payout-u maliyyЙ™ xЙ™rcinЙ™ yazД±ldД±.`;
    ...
    setState((current) => {
      ...
      let audit = {
        module: "KPI",
        action: "KPI periodu baДџlandД±",
        detail: `${period} В· score ${baseSnapshot.companyScore}% В· payout ${money(baseSnapshot.payoutAmount)}`,
      };

      if (nextAction === "close") {
        nextPeriod = {
          ...baseSnapshot,
          status: "Period baДџlandД±",
          approvalStatus: "TЙ™sdiq gГ¶zlЙ™yir",
          payoutStatus: baseSnapshot.payoutStatus === "Г–dЙ™nildi" ? "Г–dЙ™nildi" : "GГ¶zlЙ™yir",
      ...
      if (nextAction === "approve") {
        ...
        audit = {
          module: "KPI",
          action: "KPI periodu tЙ™sdiq edildi",
          detail: `${period} В· ${money(baseSnapshot.payoutAmount)} payout fondu tЙ™sdiqlЙ™ndi`,
        };
      }
      ...
      if (nextAction === "payout") {
        ...
        audit = {
          module: "KPI",
          action: "KPI payout Г¶dЙ™nildi",
          detail: `${period} В· ${money(baseSnapshot.payoutAmount)} maliyyЙ™ xЙ™rcinЙ™ yazД±ldД±`,
        };
      }
  ...
  function openAction() {
    ...
    if (permission && !requirePermission(permission, pageMeta[active]?.action || "Й™mЙ™liyyat")) return;
  ...
  function createRecord(type, values) {
    ...
    if (type === "sales" || type === "dashboard") {
      ...
      // Г‡atД±Еџmayan qalД±q satД±ЕџД± bloklamД±r вЂ” sifariЕџ backorder (mЙ™nfi mГ¶vcud) kimi qeyd olunur.
      const shortageLines = productLines
        .map((line) => {
          ...
          return shortage > 0 ? `${line.product} (${shortage} Й™dЙ™d)` : null;
        })
        .filter(Boolean);
      ...
      if (
        ...
        notify("SifariЕџ ГјГ§Гјn mГјЕџtЙ™ri, anbar vЙ™ Й™n azД± bir mЙ™hsul (say > 0) seГ§in.", "warning");
        return;
      }

      if (shortageLines.length > 0) {
        notify(
          `QalД±q Г§atД±ЕџmД±r вЂ” sifariЕџ backorder kimi yaradД±ldД±: ${shortageLines.join(", ")}. TЙ™chizatdan gЙ™tirilib tЙ™hvil verilmЙ™lidir.`,
          "warning",
        );
      }
      ...
      if (activeTenantId && createDbOrder) {
        ...
        createDbOrder({
          ...
          notify(`DB-yЙ™ saxlanД±lmadД±: ${err.message || err}`, "warning");
        });
      }
    }
    ...
    if (type === "warehouse") {
      ...
      setModal(null);
      notify("Yeni anbar yaradД±ldД±.");
      auditOperation({
        module: "Anbar",
        action: "Anbar yaradД±ldД±",
    ...
    if (type === "product") {
      ...
      if (!name || !sku) {
        notify("MЙ™hsul adД± vЙ™ SKU daxil edin.", "warning");
        return;
      }
      if (state.products.some((product) => normalize(product.sku) === normalize(sku))) {
        notify("Bu SKU artД±q mЙ™hsul kataloqunda var.", "warning");
        return;
      }
      const product = {
        id: `PRD-${Date.now()}`,
        name,
        sku,
        category: values.category || "DigЙ™r",
        unit: values.unit || "Й™dЙ™d",
        salePrice: Math.max(0, Number(values.salePrice || 0)),
        costPrice: Math.max(0, Number(values.costPrice || 0)),
        reorderLevel: Math.max(0, Math.round(Number(values.reorderLevel || 0))),
        serialTracked: values.serialTracked === "BЙ™li",
        status: "Aktiv",
      };
      // Persist to DB вЂ” Realtime bridge will merge into state.products
      if (activeTenantId && createDbProduct) {
        createDbProduct({
          ...
          notify(`MЙ™hsul DB-yЙ™ saxlanД±lmadД±: ${err.message || err}`, "warning");
        });
      }
      setState((current) =>
        auditCurrentState(
      ...
      setModal(null);
      notify(`${product.name} mЙ™hsul kataloquna Й™lavЙ™ edildi.`);
      return;
    }
    ...
    if (type === "crm" && activeTenantId && createDbCustomer && values.name) {
      createDbCustomer({
        ...
        notify(`MГјЕџtЙ™ri DB-yЙ™ saxlanД±lmadД±: ${err.message || err}`, "warning");
      });
    }
    ...
    setState((current) => {
      ...
      if (type === "sales" || type === "dashboard") {
        ...
        const paymentMethod = values.paymentMethod || "NaДџd";
        ...
        const paid = isCreditSale
          ...
          : ["NaДџd", "Kart", "KГ¶Г§ГјrmЙ™"].includes(paymentMethod)
            ? amount
            : 0;
        ...
        const warehouseName =
          current.warehouses.find((warehouse) => warehouse.id === warehouseId)?.name || "BaЕџ Anbar";
            ...
            ? [
                {
                  ...
                  paidMonths: 0,
                  rate: 0,
                  next: creditPlan.installments[0]?.due || "вЂ”",
                  status: "Aktiv",
                  installments: creditPlan.installments,
                  createdFrom: "SatД±Еџ sifariЕџi",
                },
                ...current.credits,
              ]
            : current.credits,
          orders: [
            {
              ...
              productLines,
              seller: sellerSummary || values.seller || "TЙ™yin edilmЙ™yib",
              sellerBonuses,
              amount,
              paid,
              status: values.status || "AnbardadД±r",
              date: values.date || currentBusinessDate,
              address: values.address || "Qeyd edilmЙ™yib",
              driver: "вЂ”",
              warehouseId,
              warehouseName,
              paymentMethod,
              paymentStatus: paymentMethod === "Kredit" ? "Kredit satД±Еџ" : "Г–dЙ™nilib",
              creditId,
              contractId,
              ...
              deliveryStatus: "TЙ™hvil gГ¶zlЙ™yir",
              note: values.note || "",
          ...
          credits: [
            {
              ...
              paidMonths: 0,
              rate: 0,
              next: values.next || creditPlan.installments[0]?.due || "вЂ”",
              status: "Aktiv",
              installments: creditPlan.installments,
            },
            ...current.credits,
          ...
          employees: [
            {
              ...
              level: values.level || "Komanda ГјzvГј",
              ...
              employmentType: values.employmentType || "Tam Еџtat",
              leaveBalance: Math.max(0, Number(values.leaveBalance || 0)),
              usedLeave: 0,
              documentsComplete,
              hrStatus: documentReviewRequired ? "MЙ™lumat gГ¶zlЙ™yir" : "Stabil",
    ...
    setModal(null);
    notify("Yeni qeyd Й™lavЙ™ olundu.");
    auditOperation(getCreateAudit(type, values));
  }

  function updateVendor(vendorKey, values) {
    if (!requirePermission("vendors.manage", "vendoru redaktЙ™ etmЙ™k")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Vendor",
          action: "Vendor redaktЙ™ edildi",
          detail: `${currentVendor.name} в†’ ${nextVendor.name}`,
        },
      ),
    );
    setModal(null);
    notify(`${nextVendor.name} vendor mЙ™lumatlarД± yenilЙ™ndi.`);
  }

  function deleteVendor(vendorKey) {
    if (!requirePermission("vendors.manage", "vendoru silmЙ™k")) return;
    ...
    if (openPoCount > 0) {
      notify("Bu vendor ГјzrЙ™ aГ§Д±q PO var. ЖЏvvЙ™l PO-nu tЙ™sdiqlЙ™yin vЙ™ ya vendoru Passiv edin.", "warning");
      return;
    }
    ...
    setModal(null);
    notify(`${targetVendor.name} vendor reyestrindЙ™n silindi.`);
  }

  function updateWarehouse(id, values) {
    if (!requirePermission("warehouse.manage", "anbarД± redaktЙ™ etmЙ™k")) return;
    ...
    setModal(null);
    notify("Anbar mЙ™lumatlarД± yenilЙ™ndi.");
    auditOperation({
      module: "Anbar",
      action: "Anbar redaktЙ™ edildi",
      detail: `${values.name} (${values.code})`,
    });
  }

  function recordStockIntake(values) {
    if (!requirePermission("warehouse.manage", "anbara mЙ™daxil etmЙ™k")) return;
    ...
    if (!warehouse || !product || qty <= 0) {
      notify("MЙ™daxil ГјГ§Гјn anbar, mЙ™hsul vЙ™ etibarlД± miqdar daxil edin.", "warning");
      return;
    }
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "Anbar",
          action: "Д°lkin mЙ™daxil edildi",
          detail: `${product}: ${qty} Й™dЙ™d В· ${warehouse.name}`,
        },
      );
    });
    setModal(null);
    notify(`${product}: ${qty} Й™dЙ™d ${warehouse.name} anbarД±na mЙ™daxil edildi.`);
  }

  function importWarehouseStock(rows) {
    if (!requirePermission("warehouse.manage", "anbara toplu import etmЙ™k")) return;
    if (!Array.isArray(rows) || rows.length === 0) {
      notify("Д°mport ГјГ§Гјn etibarlД± CSV sЙ™tri tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) => {
      ...
      rows.forEach((row, index) => {
        ...
        const product = matchedProduct
          ? {
              ...matchedProduct,
              category: row.category || matchedProduct.category || "DigЙ™r",
              unit: row.unit || matchedProduct.unit || "Й™dЙ™d",
          ...
          : {
              ...
              category: row.category || "DigЙ™r",
              unit: row.unit || "Й™dЙ™d",
      ...
      return auditCurrentState(
        ...
        },
        {
          ...
          detail: `${rows.length} sЙ™tir В· ${touchedProducts.size} mЙ™hsul В· ${touchedWarehouses.size} anbar`,
        },
      );
    });
    setModal(null);
    notify(`${rows.length} stok sЙ™tri anbara import edildi.`);
  }

  function updateProduct(productId, values) {
    if (!requirePermission("warehouse.manage", "mЙ™hsul kataloqunu redaktЙ™ etmЙ™k")) return;
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "MЙ™hsul",
          action: "MЙ™hsul redaktЙ™ edildi",
          detail: `${nextProduct.sku} В· ${nextProduct.name}`,
        },
      );
    });
    // Persist to DB вЂ” Realtime bridge will refresh state
    if (activeTenantId && updateDbProduct) {
      ...
      if (dbRow) {
        updateDbProduct(dbRow.id, {
          ...
          notify(`MЙ™hsul DB-dЙ™ yenilЙ™nmЙ™di: ${err.message || err}`, "warning");
        });
      }
    }
    setModal(null);
    notify(`${nextProduct.name} mЙ™hsul mЙ™lumatlarД± yenilЙ™ndi.`);
  }

  function deleteProduct(productId) {
    if (!requirePermission("warehouse.manage", "mЙ™hsul kataloqunu silmЙ™k")) return;
    const currentProduct = state.products.find((product) => product.id === productId);
    if (!currentProduct) {
      notify("MЙ™hsul tapД±lmadД±.", "warning");
      return;
    }
    if (!window.confirm(`${currentProduct.name} mЙ™hsulunu silmЙ™k istЙ™diyinizЙ™ Й™minsiniz?`)) return;
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        { module: "MЙ™hsul", action: "MЙ™hsul silindi", detail: `${currentProduct.sku} В· ${currentProduct.name}` },
      );
    });
    ...
    if (activeTenantId && deleteDbProduct) {
      ...
      if (dbRow) {
        deleteDbProduct(dbRow.id).catch((err) => {
          notify(`MЙ™hsul DB-dЙ™n silinmЙ™di: ${err.message || err}`, "warning");
        });
      }
    }
    setModal(null);
  ...
  function deleteCustomer(fin) {
    if (!requirePermission("crm.manage", "mГјЕџtЙ™ri silmЙ™k")) return;
    const currentCustomer = (state.customers || []).find((c) => c.fin === fin);
    if (!currentCustomer) {
      notify("MГјЕџtЙ™ri tapД±lmadД±.", "warning");
      return;
    }
    if (!window.confirm(`${currentCustomer.name} mГјЕџtЙ™risini silmЙ™k istЙ™diyinizЙ™ Й™minsiniz?`)) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        { module: "CRM", action: "MГјЕџtЙ™ri silindi", detail: `${currentCustomer.fin} В· ${currentCustomer.name}` },
      ),
    );
    ...
    if (activeTenantId && deleteDbCustomer) {
      ...
      if (dbRow) {
        deleteDbCustomer(dbRow.id).catch((err) => {
          notify(`MГјЕџtЙ™ri DB-dЙ™n silinmЙ™di: ${err.message || err}`, "warning");
        });
      }
    }
  ...
  function saveFinanceAccount(accountId, values) {
    if (!requirePermission("finance.manage", "kassa vЙ™ bank hesabД±nД± idarЙ™ etmЙ™k")) return;

    const name = String(values.name || "").trim();
    const code = String(values.code || "").trim().toUpperCase();
    if (!name || !code) {
      notify("Hesab adД± vЙ™ kodu daxil edin.", "warning");
      return;
    }
    ...
    if ((state.financeAccounts || []).some((account) => account.id !== accountId && normalize(account.code) === normalize(code))) {
      notify("Bu hesab kodu artД±q istifadЙ™ olunur.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "MaliyyЙ™",
          action: existing ? "MaliyyЙ™ hesabД± redaktЙ™ edildi" : "MaliyyЙ™ hesabД± yaradД±ldД±",
          detail: `${account.code} В· ${account.name} В· ${money(account.openingBalance)}`,
        },
      ),
    );
    setModal(null);
    notify(`${account.name} hesabД± yadda saxlanД±ldД±.`);
  }

  function deleteWarehouse(id) {
    if (!requirePermission("warehouse.manage", "anbarД± silmЙ™k")) return;
    ...
    const openPoCount = (state.purchaseOrders || []).filter(
      (po) => po.warehouseId === id && !["TЙ™sdiq edildi", "LЙ™Дџv edildi"].includes(po.status),
    ).length;
    ...
    if (totalQty > 0 || reservedQty > 0 || deliveryCount > 0 || openPoCount > 0) {
      const reason = [
        ...
        deliveryCount > 0 ? `${deliveryCount} aГ§Д±q tЙ™hvil` : "",
        openPoCount > 0 ? `${openPoCount} aГ§Д±q PO` : "",
      ].filter(Boolean).join(" В· ");
      notify(`${warehouse?.name || id} silinmЙ™di: Й™vvЙ™l ${reason} baДџlanmalД±dД±r.`, "warning");
      auditOperation({
        module: "Anbar",
        action: "Anbar silinmЙ™si bloklandД±",
        detail: `${warehouse?.name || id} В· ${reason}`,
        status: "BloklandД±",
      });
      return;
    }
  ...
  function advanceOrder(id) {
    if (!requirePermission("delivery.complete", "tЙ™hvil mЙ™rhЙ™lЙ™sini dЙ™yiЕџmЙ™k")) return;
    ...
    notify(`${id} nГ¶vbЙ™ti mЙ™rhЙ™lЙ™yЙ™ keГ§irildi.`);
    auditOperation({
      module: "TЙ™hvil",
      action: "SifariЕџ mЙ™rhЙ™lЙ™si dЙ™yiЕџdi",
      detail: id,
    });
  }

  function completeWarehouseDelivery(orderId) {
    if (!requirePermission("delivery.complete", "tЙ™hvili tamamlamaq")) return;

    const targetOrder = state.orders.find((order) => order.id === orderId);

    if (
      !targetOrder ||
      targetOrder.status === "TЙ™hvil verilib" ||
      !Array.isArray(targetOrder.productLines) ||
      targetOrder.productLines.length === 0
    ) {
      notify("Bu sifariЕџ ГјГ§Гјn tЙ™hvil Й™mЙ™liyyatД± aparД±la bilmЙ™di.", "warning");
      return;
    }
    ...
    if (!initialCheck.ok) {
      notify(initialCheck.reason || "TЙ™hvil ГјГ§Гјn anbar rezervi kifayЙ™t etmir.", "warning");
      auditOperation({
        module: "TЙ™hvil/Anbar",
        action: "TЙ™hvil bloklandД±",
        detail: `${orderId} В· ${initialCheck.reason}`,
        status: "BloklandД±",
      });
      return;
    }
    ...
    setState((current) => {
      ...
      if (!order || order.status === "TЙ™hvil verilib" || !Array.isArray(order.productLines)) {
        return current;
      }
      ...
      if (!stockCheck.ok) {
        return appendAudit(current, {
          module: "TЙ™hvil/Anbar",
          action: "TЙ™hvil bloklandД±",
          detail: `${orderId} В· ${stockCheck.reason}`,
          status: "BloklandД±",
          role: getActiveRole(current.settings)?.name || activeRoleInfo?.name || "System",
        });
      }
      ...
      return appendAudit(
          ...
          orders: current.orders.map((item) =>
            item.id === orderId
              ? {
                  ...
                  status: fullyDelivered ? "TЙ™hvil verilib" : item.status,
                  deliveryStatus: fullyDelivered
                    ? "TЙ™hvil verildi"
                    : `QismЙ™n tЙ™hvil (${plan.deliveredTotal + deliveredNow}/${plan.orderedTotal})`,
        ...
        {
          module: "TЙ™hvil/Anbar",
          action: fullyDelivered ? "TЙ™hvil tamamlandД±" : "QismЙ™n tЙ™hvil",
          detail: `${orderId} В· ${summarizeOrderProducts(order)} В· ${deliveredNow} Й™dЙ™d anbardan Г§Д±xД±ldД±${
            fullyDelivered ? "" : ` В· ${remainingAfter} Й™dЙ™d backorder qalД±r`
          }`,
          role: getActiveRole(current.settings)?.name || activeRoleInfo?.name || "System",
        },
      );
    ...
    if (resultPlan && resultPlan.shortageTotal > 0) {
      notify(
        `${orderId}: ${resultPlan.deliverableTotal} Й™dЙ™d tЙ™hvil verildi, ${resultPlan.shortageTotal} Й™dЙ™d backorder olaraq qaldД±.`,
        "warning",
      );
    } else {
      notify(`${orderId} tЙ™hvil verildi vЙ™ mЙ™hsul anbardan Г§Д±xД±ldД±.`, "success");
    }
  }


  function transferWarehouseStock({ fromWarehouseId, toWarehouseId, product, qty }) {
    if (!requirePermission("warehouse.manage", "anbar transferi etmЙ™k")) return;
    ...
    if (!sourceItem || !toWarehouseId || fromWarehouseId === toWarehouseId || available < amount) {
      notify("Transfer ГјГ§Гјn kifayЙ™t qЙ™dЙ™r satД±Еџ qalД±ДџД± yoxdur.", "warning");
      return;
    }
    ...
    notify(`${product}: ${amount} Й™dЙ™d ${sourceName} в†’ ${targetName} transfer edildi.`, "success");
    auditOperation({
      module: "Anbar",
      action: "Stok transfer edildi",
      detail: `${product}: ${amount} Й™dЙ™d ${sourceName} в†’ ${targetName}`,
    });
  }

  function setExpenseStatus(id, status) {
    if (!requirePermission("finance.manage", "xЙ™rc statusunu dЙ™yiЕџmЙ™k")) return;
    ...
    notify(`XЙ™rc Й™mЙ™liyyatД±: ${status}.`);
    auditOperation({
      module: "MaliyyЙ™",
      action: "XЙ™rc statusu dЙ™yiЕџdi",
      detail: `${id}: ${status}`,
    });
  }
  ...
  function openSalesOrderEditor(orderId) {
    if (!requirePermission("sales.create", "satД±Еџ Й™mЙ™liyyatД±nД± redaktЙ™ etmЙ™k")) return;
    setModal({ type: "salesOperation", orderId });
  }

  function openSalesOrderDelete(orderId) {
    if (!requirePermission("sales.create", "satД±Еџ Й™mЙ™liyyatД±nД± silmЙ™k")) return;
  ...
  function openLinkedSalesOrder(orderId) {
    ...
    if (!targetOrder) {
      notify("BaДџlД± sifariЕџ tapД±lmadД±.", "warning");
      return;
    }
    ...
    setMobileNav(false);
    auditOperation({
      module: "Kredit/SatД±Еџ",
      action: "BaДџlД± sifariЕџЙ™ keГ§id edildi",
      detail: `${orderId} В· ${targetOrder.customer}`,
    });
  }
  ...
  function openLinkedCredit(creditId) {
    ...
    if (!targetCredit) {
      notify("BaДџlД± kredit tapД±lmadД±.", "warning");
      return;
    }
    ...
    auditOperation({
      module: "CRM/Kredit",
      action: "BaДџlД± kreditЙ™ keГ§id edildi",
      detail: `${creditId} В· ${targetCredit.customer}`,
    });
  }
  ...
  function openVendorModule(sourceId = "") {
    ...
    setMobileNav(false);
    auditOperation({
      module: "MaliyyЙ™/Vendor",
      action: "PO mЙ™nbЙ™yinЙ™ keГ§id edildi",
  ...
  function validateSalesOrderEdit(order, values) {
    ...
    const delivered = order.status === "TЙ™hvil verilib";
    ...
    if (nextProductLines.length === 0) {
      notify("SatД±Еџ Й™mЙ™liyyatД±nda Й™n azД± bir mЙ™hsul sЙ™tri olmalД±dД±r.", "warning");
      return false;
    }
    if (Number(values.amount ?? calculateOrderLineTotal(nextProductLines)) <= 0) {
      notify("SatД±Еџ mЙ™blЙ™Дџi 0-dan bГ¶yГјk olmalД±dД±r.", "warning");
      return false;
    }
    if (delivered && productOrWarehouseChanged) {
      notify("TЙ™hvil verilmiЕџ satД±Еџda mЙ™hsul vЙ™ anbar dЙ™yiЕџmЙ™k ГјГ§Гјn geri qaytarma Й™mЙ™liyyatД± lazД±mdД±r.", "warning");
      return false;
    }
    if (delivered && nextStatus !== "TЙ™hvil verilib") {
      notify("TЙ™hvil verilmiЕџ satД±ЕџД± Й™vvЙ™lki mЙ™rhЙ™lЙ™yЙ™ qaytarmaq ГјГ§Гјn ayrД±ca geri qaytarma axД±nД± lazД±mdД±r.", "warning");
      return false;
    }
    if (!delivered && nextStatus === "TЙ™hvil verilib") {
      notify("MЙ™hsulu anbardan Г§Д±xarmaq ГјГ§Гјn TЙ™hvil modulundakД± tamamla Й™mЙ™liyyatД±ndan istifadЙ™ edin.", "warning");
      return false;
    }
    ...
    if (insufficient) {
      notify(`${insufficient[0]} ГјГ§Гјn seГ§ilЙ™n anbarda kifayЙ™t qЙ™dЙ™r satД±Еџ qalД±ДџД± yoxdur.`, "warning");
      return false;
    }
    return true;
  }
  ...
  function updateSalesOrder(orderId, values) {
    ...
    if (!existingOrder) {
      notify("SatД±Еџ Й™mЙ™liyyatД± tapД±lmadД±.", "warning");
      return;
    }
    ...
    if (activeTenantId && updateDbOrder && uuidRe.test(String(orderId))) {
      const statusMap = {
        "Yeni": "draft",
        "TЙ™sdiqlЙ™nib": "confirmed",
        "Yolda": "shipped",
        "TЙ™hvil verilib": "delivered",
        "LЙ™Дџv edilib": "cancelled",
      };
      ...
      updateDbOrder(orderId, patch).catch((err) => {
        ...
        notify(`SifariЕџ DB-dЙ™ yenilЙ™nmЙ™di: ${err.message || err}`, "warning");
      });
    }
    ...
    setState((current) => {
      ...
      const sellerSummary = summarizeSellerBonusRows(sellerBonuses) || order.seller || "TЙ™yin edilmЙ™yib";
      ...
      const paymentMethod = values.paymentMethod || order.paymentMethod || "NaДџd";
      const isCreditSale = paymentMethod === "Kredit";
      const creditPlan = isCreditSale
        ? buildCreditPlan({
            total: amount,
      ...
      const paymentStatus = isCreditSale ? "Kredit satД±Еџ" : paid >= amount ? "Г–dЙ™nilib" : paid > 0 ? "QalД±qlД±" : "Г–dЙ™niЕџ gГ¶zlЙ™yir";
      ...
      const nextOrder = {
        ...
        address: values.address || order.address || "Qeyd edilmЙ™yib",
      ...
      const nextContracts = isCreditSale
        ? (() => {
            const contractRecord = {
              ...
              amount,
              status: current.contracts.find((contract) => contract.id === contractId)?.status || "HazД±rlanД±r",
              orderId: order.id,
            };
      ...
      return auditCurrentState(
        ...
        {
          module: "SatД±Еџ",
          action: "SatД±Еџ Й™mЙ™liyyatД± redaktЙ™ edildi",
          detail: `${order.id} В· ${nextOrder.customer} В· ${money(amount)}`,
        },
      );
    });
    setModal(null);
    notify(`${orderId} satД±Еџ Й™mЙ™liyyatД± yenilЙ™ndi.`);
  }
  ...
  function deleteSalesOrder(orderId) {
    ...
    if (isUuid && deleteDbOrder) {
      Promise.resolve(deleteDbOrder(orderId)).catch((e) => {
        notify(`SilmЙ™ DB xЙ™tasД±: ${e?.message || e}`, "warning");
      });
    }
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "SatД±Еџ",
          action: "SatД±Еџ Й™mЙ™liyyatД± silindi",
          detail: `${order.id} В· ${order.customer} В· ${delivered ? "tЙ™hvil verilmiЕџ satД±Еџ" : "rezerv aГ§Д±ldД±"}`,
        },
      );
    });
    setModal(null);
    notify(`${orderId} satД±Еџ Й™mЙ™liyyatД± silindi.`);
  }

  function openExpenseEditor(expenseId) {
    if (!requirePermission("finance.manage", "xЙ™rc Й™mЙ™liyyatД±nД± redaktЙ™ etmЙ™k")) return;
    setModal({ type: "expenseOperation", expenseId });
  }

  function openExpenseDelete(expenseId) {
    if (!requirePermission("finance.manage", "xЙ™rc Й™mЙ™liyyatД±nД± silmЙ™k")) return;
  ...
  function updateExpense(expenseId, values) {
    ...
    if (!values.description || !values.category || amount <= 0) {
      notify("XЙ™rc ГјГ§Гјn tЙ™svir, kateqoriya vЙ™ dГјzgГјn mЙ™blЙ™Дџ daxil edin.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "MaliyyЙ™",
          action: "XЙ™rc Й™mЙ™liyyatД± redaktЙ™ edildi",
          detail: `${expenseId} В· ${values.description} В· ${money(amount)}`,
        },
      ),
    );
    setModal(null);
    notify(`${expenseId} xЙ™rc Й™mЙ™liyyatД± yenilЙ™ndi.`);
  }
  ...
  function deleteExpense(expenseId) {
    ...
    if (!targetExpense) {
      notify("XЙ™rc Й™mЙ™liyyatД± tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "MaliyyЙ™",
          action: "XЙ™rc Й™mЙ™liyyatД± silindi",
          detail: `${expenseId} В· ${targetExpense.description} В· ${money(targetExpense.amount)}`,
        },
      ),
    );
    setModal(null);
    notify(`${expenseId} xЙ™rc Й™mЙ™liyyatД± silindi.`);
  }

  function updateEmployeeStructure(employeeName, values) {
    if (!requirePermission("hr.manage", "HR strukturunu dЙ™yiЕџmЙ™k")) return;
    ...
    notify(`${employeeName} struktur aДџacД±nda yenilЙ™ndi.`);
    auditOperation({
      module: "HR",
      action: "Struktur yenilЙ™ndi",
      detail: employeeName,
    });
  }

  function openEmployeeEditor(employee) {
    if (!requirePermission("hr.manage", "Й™mЙ™kdaЕџ mЙ™lumatlarД±nД± redaktЙ™ etmЙ™k")) return;
  ...
  function updateEmployee(employeeId, values) {
    ...
    setState((current) => {
      ...
      const documentsComplete = Math.max(0, Math.min(100, Number(values.documentsComplete ?? 100)));
      const hrStatus = values.hrStatus === "MЙ™lumat gГ¶zlЙ™yir" ? "MЙ™lumat gГ¶zlЙ™yir" : "Stabil";
      const updatedEmployee = {
        ...
        documentsComplete,
        hrStatus,
        documentReviewRequired: hrStatus === "MЙ™lumat gГ¶zlЙ™yir" || documentsComplete < 100,
        skills: String(values.skills || "")
          .split(",")
          .map((item) => item.trim())
          .filter(Boolean),
      ...
      return auditCurrentState(
        ...
        {
          module: "HR",
          action: "ЖЏmЙ™kdaЕџ redaktЙ™ edildi",
          detail: `${existing.name} в†’ ${nextName}`,
        },
      );
    });
    setModal(null);
    notify(`${values.name} Й™mЙ™kdaЕџ mЙ™lumatlarД± yenilЙ™ndi.`);
  }

  function openDepartmentCreator() {
    if (!requirePermission("hr.manage", "ЕџГ¶bЙ™ Й™lavЙ™ etmЙ™k")) return;
  ...
  function createDepartment(values) {
    ...
    if (!name) {
      notify("ЕћГ¶bЙ™ adД± daxil edin.", "warning");
      return;
    }
    if (normalize(name) === normalize(parentDepartment)) {
      notify("ЕћГ¶bЙ™ Г¶zГјnЙ™ tabe ola bilmЙ™z.", "warning");
      return;
    }
    ...
    if (exists) {
      notify("Bu adlД± ЕџГ¶bЙ™ artД±q mГ¶vcuddur.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "HR",
          action: "ЕћГ¶bЙ™ Й™lavЙ™ edildi",
          detail: parentDepartment ? `${name} в†’ ${parentDepartment}` : name,
        },
      ),
    );
    setModal(null);
    notify(`${name} ЕџГ¶bЙ™si Й™lavЙ™ edildi.`);
  }

  function openLeaveRequestCreator() {
    if (!requirePermission("hr.manage", "mЙ™zuniyyЙ™t qeydi yaratmaq")) return;
  ...
  function createLeaveRequest(values) {
    ...
    if (!employee || !from || !to || to < from) {
      notify("ЖЏmЙ™kdaЕџ vЙ™ dГјzgГјn mЙ™zuniyyЙ™t tarixlЙ™rini seГ§in.", "warning");
      return;
    }
    ...
    const request = {
      ...
      type: values.type || "Д°llik mЙ™zuniyyЙ™t",
      from: values.from,
      to: values.to,
      days,
    ...
    setState((current) =>
      auditCurrentState(
        ...
        { module: "HR", action: "MЙ™zuniyyЙ™t qeydi yaradД±ldД±", detail: `${employee.name} В· ${days} gГјn` },
      ),
    );
    ...
    setModal(null);
    notify(`${employee.name} ГјГ§Гјn mЙ™zuniyyЙ™t qeydi yaradД±ldД±.`);
  }

  function updateLeaveRequestStatus(requestId, status) {
    if (!requirePermission("hr.manage", "mЙ™zuniyyЙ™t statusunu dЙ™yiЕџmЙ™k")) return;

    const nextStatus = status === "TЙ™sdiq edildi" ? "TЙ™sdiq edildi" : "Д°mtina edildi";
    ...
    setState((current) => {
      return auditCurrentState(
        ...
        {
          module: "HR",
          action: "MЙ™zuniyyЙ™t statusu dЙ™yiЕџdi",
          detail: `${request.employeeName || requestId} В· ${nextStatus}`,
        },
      );
    });
    notify(`MЙ™zuniyyЙ™t sorДџusu: ${nextStatus}.`);
    queueMicrotask(() => syncHrWorkflow(
      "leave_request",
      { ...request, status: nextStatus, decidedAt: stamp },
      status === "TР™в„ўsdiq edildi" ? "approved" : "rejected",
    ));
  }

  function markPayrollPaid(employeeId) {
    if (!requirePermission("hr.manage", "payroll statusunu dЙ™yiЕџmЙ™k")) return;

    const employee = state.employees.find((item) => getEmployeeKey(item) === employeeId);
    if (!employee) return;
    const documentsComplete =
      employee.documentReviewRequired || employee.hrStatus === "MЙ™lumat gГ¶zlЙ™yir"
    ...
    if (documentHealth.missingCount > 0 && !employee.payrollPaidAt) {
      notify("SЙ™nЙ™dlЙ™r tamamlanmadan payroll Г¶dЙ™niЕџi baДџlana bilmЙ™z.", "warning");
      return;
    }
    ...
    setState((current) => {
      return auditCurrentState(
        ...
        {
          module: "HR/Payroll",
          action: "Payroll Г¶dЙ™niЕџ statusu dЙ™yiЕџdi",
          detail: `${employee.name} В· Г–dЙ™nildi В· ${baseFinanceDate.slice(0, 7)}`,
        },
      );
    });
    notify("Payroll statusu Г¶dЙ™nildi kimi baДџlandД±.");
    queueMicrotask(() => syncHrWorkflow(
      "payroll_line",
  ...
  function updateEmployeeDocuments(employeeId, documentsComplete = 100) {
    if (!requirePermission("hr.manage", "Й™mЙ™kdaЕџ sЙ™nЙ™dlЙ™rini yenilЙ™mЙ™k")) return;
    ...
    setState((current) => {
      ...
      const nextPayrollStatus = employee.payrollPaidAt
        ...
        : documentReviewRequired
          ? "SЙ™nЙ™d gГ¶zlЙ™yir"
          : employee.payrollStatus === "SЙ™nЙ™d gГ¶zlЙ™yir"
            ? "Hesablama hazД±rdД±r"
            : employee.payrollStatus || "Hesablama hazД±rdД±r";
      ...
      return auditCurrentState(
        ...
        {
          module: "HR",
          action: "ЖЏmЙ™kdaЕџ sЙ™nЙ™dlЙ™ri yenilЙ™ndi",
          detail: `${employee.name} В· ${nextScore}%`,
        },
      );
    });
    notify(nextScore === 100 ? "ЖЏmЙ™kdaЕџ sЙ™nЙ™dlЙ™ri tamamlandД±." : "ЖЏmЙ™kdaЕџ sЙ™nЙ™d statusu yenilЙ™ndi.");
    queueMicrotask(() => syncHrWorkflow(
  ...
  function createVacancy(values) {
    ...
    if (!role || !department) {
      notify("Vakansiya ГјГ§Гјn rol vЙ™ ЕџГ¶bЙ™ seГ§in.", "warning");
      return;
    }
    const vacancy = {
      id: `VAC-${Date.now()}`,
    ...
    setState((current) =>
      auditCurrentState(
        ...
        { module: "HR", action: "Vakansiya yaradД±ldД±", detail: `${role} В· ${department}` },
      ),
    );
    setModal(null);
    notify(`${role} vakansiyasД± yaradД±ldД±.`);
  }

  function openEmployeeDelete(employee) {
    if (!requirePermission("hr.manage", "Й™mЙ™kdaЕџ silmЙ™k")) return;
  ...
  function deleteEmployee(employeeId, replacementManagerId = "") {
    ...
    setState((current) => {
      ...
      return auditCurrentState(
        ...
        {
          module: "HR",
          action: "ЖЏmЙ™kdaЕџ silindi",
          detail: `${employee.name} В· ${directReports.length} tabe Й™mЙ™kdaЕџ yenidЙ™n tЙ™yin edildi`,
        },
      );
    });
    setModal(null);
    notify("ЖЏmЙ™kdaЕџ silindi vЙ™ tabeГ§ilik xЙ™tti yenilЙ™ndi.");
  }

  function sendCreditSms(id) {
    if (!requirePermission("credits.manage", "kredit SMS gГ¶ndЙ™rmЙ™k")) return;
    ...
    const stamp = getActionStamp();
    const status = getNotificationChannelEnabled("SMS", state.settings) && provider.status === "Aktiv" ? "GГ¶ndЙ™rildi" : "BloklandД±";
    const delivery = createNotificationSendLogEntry({
      rule: {
        ...
        template: "{creditId} ГјzrЙ™ SMS xatД±rlatma",
      },
      provider,
      event: {
        entityId: id,
        recipient: targetCredit?.customer || "MГјЕџtЙ™ri",
        target: targetCredit?.fin || targetCredit?.customer || id,
        creditId: id,
        body: `${id} ГјzrЙ™ SMS mГјЕџtЙ™riyЙ™ gГ¶ndЙ™rildi.`,
        module: "credits",
        priority: "Orta",
      },
      stamp,
    ...
    setState((current) => ({
      ...
      notificationProviders: ensureNotificationProviders(current.notificationProviders || []).map((item) =>
        item.id === provider.id && status === "GГ¶ndЙ™rildi"
      ...
      notifications: [
        {
          id: `IN-${delivery.id}`,
          type: "SMS",
          title: `Kredit Г¶dЙ™niЕџi xatД±rlatmasД± ${status.toLowerCase()}`,
          body: delivery.body,
          time: stamp,
          unread: true,
          deliveryId: delivery.id,
    ...
    notify("SMS xatД±rlatma gГ¶ndЙ™rildi.");
    auditOperation({
      module: "Kredit",
      action: "SMS xatД±rlatma gГ¶ndЙ™rildi",
      detail: `${id} В· ${status}`,
    });
  }

  function updateCreditPaymentDate(creditId, month, due) {
    if (!requirePermission("credits.manage", "kredit Г¶dЙ™niЕџ tarixini dЙ™yiЕџmЙ™k")) return;

    const targetCredit = buildAllCreditRecords(state.orders, state.credits).find((credit) => credit.id === creditId);

    if (!targetCredit) {
      notify("Kredit tapД±lmadД±.", "warning");
      return;
    }
    ...
    notify("Г–dЙ™niЕџ tarixi yenilЙ™ndi.");
    auditOperation({
      module: "Kredit",
      action: "Г–dЙ™niЕџ tarixi redaktЙ™ edildi",
      detail: `${creditId} В· ${month}. ay В· ${due}`,
    });
  }

  function receiveCreditPayment(creditId, values) {
    if (!requirePermission("credits.manage", "kredit Г¶dЙ™niЕџi qЙ™bul etmЙ™k")) return;
    ...
    if (principalAmount <= 0 && penaltyAmount <= 0) {
      notify("Г–dЙ™niЕџ mЙ™blЙ™Дџi daxil edin.", "warning");
      return;
    }
    ...
    notify(
      `${targetCredit.id}: ${money(cashAmount)} kassaya daxil oldu. ЖЏsas borc ${money(paymentResult.appliedPrincipal)} azaldД±.`,
    );
    auditOperation({
      module: "Kredit/MaliyyЙ™",
      action: "Kredit Г¶dЙ™niЕџi qЙ™bul edildi",
      detail: `${creditId}: Й™sas ${money(paymentResult.appliedPrincipal)}, gecikmЙ™ ${money(penaltyAmount)}`,
    });
  }
  ...
  function createPurchaseOrder(row) {
    ...
    if (!warehouse || !row.product) {
      notify("PO yaratmaq ГјГ§Гјn mЙ™hsul vЙ™ anbar mЙ™lumatД± lazД±mdД±r.", "warning");
      return false;
    }
    ...
    const po = {
      ...
      procurementType: row.procurementType || "Zavod sifariЕџi",
      qty,
      unitCost,
      price: salePrice,
      amount,
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Vendor",
          action: "PO yaradД±ldД±",
          detail: `${po.product}: ${po.qty} Й™dЙ™d В· ${po.vendor}`,
        },
      ),
    );
    notify(`${po.id} yaradД±ldД± vЙ™ tЙ™sdiq gГ¶zlЙ™yir.`);
    return true;
  }

  function approvePurchaseOrder(poId) {
    if (!requirePermission("vendors.po", "PO tЙ™sdiqlЙ™mЙ™k vЙ™ anbara mЙ™daxil etmЙ™k")) return;

    const targetPo = (state.purchaseOrders || []).find((item) => item.id === poId);

    if (!targetPo || targetPo.status === "TЙ™sdiq edildi") {
      notify("PO tЙ™sdiqlЙ™nmЙ™di.", "warning");
      return;
    }

    setState((current) => {
      const po = (current.purchaseOrders || []).find((item) => item.id === poId);
      if (!po || po.status === "TЙ™sdiq edildi") return current;
      ...
      const procurementExpense = {
        id: `EXP-${po.id}`,
        description: `PO alД±Еџ - ${po.product}`,
        category: "SatД±nalma",
        date: baseFinanceDate,
        amount: Number(po.amount || 0),
        status: "TЙ™sdiq gГ¶zlЙ™yir",
      ...
      return auditCurrentState(
        ...
        {
          module: "Vendor/Anbar/MaliyyЙ™",
          action: "PO tЙ™sdiqlЙ™ndi vЙ™ mЙ™daxil edildi",
          detail: `${po.product}: ${po.qty} Й™dЙ™d ${warehouseName} anbarД±na daxil oldu`,
        },
      );
    });

    notify(`${targetPo.id} tЙ™sdiqlЙ™ndi vЙ™ anbara mЙ™daxil edildi.`, "success");
  }

  function markAllNotificationsRead() {
    if (!requirePermission("notifications.manage", "bildiriЕџlЙ™ri idarЙ™ etmЙ™k")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "BildiriЕџ",
          action: "HamД±sД± oxundu",
          detail: `Son yoxlama: ${stamp}`,
        },
      ),
    );
    notify("BГјtГјn bildiriЕџlЙ™r oxunmuЕџ iЕџarЙ™lЙ™ndi.");
  }

  function toggleSetting(key) {
    if (!requirePermission("settings.manage", "ayarlarД± dЙ™yiЕџmЙ™k")) return;
    ...
    auditOperation({
      module: "Ayarlar",
      action: "Toggle dЙ™yiЕџdi",
      detail: key,
    });
  }

  function updateCompany(field, value) {
    if (!requirePermission("settings.manage", "ЕџirkЙ™t mЙ™lumatlarД±nД± dЙ™yiЕџmЙ™k")) return;
    ...
    auditOperation({
      module: "Ayarlar",
      action: "ЕћirkЙ™t mЙ™lumatД± dЙ™yiЕџdi",
      detail: field,
    });
  }

  function saveSettings() {
    if (!requirePermission("settings.manage", "ayarlarД± yadda saxlamaq")) return;

    notify("Sistem tЙ™nzimlЙ™mЙ™lЙ™ri yadda saxlanД±ldД±.");
    auditOperation({
      module: "Ayarlar",
      action: "Ayarlar yadda saxlandД±",
      detail: "ЕћirkЙ™t vЙ™ sistem tЙ™nzimlЙ™mЙ™lЙ™ri",
    });
  }

  function runIntegrityCheck() {
    if (!requireSystemBackup("integrity yoxlamasД±")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Sistem/DB",
          action: "Integrity yoxlamasД±",
          detail: `${report.issueCount} siqnal В· score ${report.score}%`,
          status: report.status,
        },
      ),
    );
    notify(`Integrity yoxlandД±: ${report.status}, score ${report.score}%.`);
  }

  function runGoLiveCheck() {
    if (!requireSystemBackup("go-live yoxlamasД±")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          ...
          action: "Go-live readiness yoxlandД±",
          detail: `${report.blockers} bloker В· score ${report.score}%`,
          status: report.status,
        },
      ),
    );
    notify(`Go-live yoxlandД±: ${report.status}, score ${report.score}%.`);
  }

  function runProductionHardeningCheck() {
    if (!requireSystemBackup("production hardening yoxlamasД±")) return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          ...
          action: "Production hardening yoxlandД±",
          detail: `${report.ready} hazД±r В· ${report.watch} nЙ™zarЙ™tdЙ™ В· score ${report.score}%`,
          status: report.status,
        },
      ),
    );
    notify(`Production hardening yoxlandД±: ${report.status}, score ${report.score}%.`);
  }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        },
        {
          ...
          detail: `Schema v${localDbSchemaVersion} В· ${sizeKb}`,
        },
      ),
    );
    notify("Backup JSON faylД± hazД±rlandД±.");
  }
  ...
  function importBackup(file) {
    ...
    reader.onload = () => {
      try {
        ...
        setState(
          appendAudit(
            ...
            },
            {
              ...
              detail: `${file.name} В· ${report.issueCount} integrity siqnalД±`,
              status: report.status,
              role: activeRoleInfo?.name || "System",
            },
          ),
        ...
        notify("Backup faylД± oxunmadД±. JSON strukturunu yoxlayД±n.", "warning");
      }
    };
    reader.onerror = () => notify("Backup faylД± oxunarkЙ™n xЙ™ta baЕџ verdi.", "warning");
    reader.readAsText(file);
  }

  function changeCurrentRole(roleName) {
    if (!requirePermission("settings.manage", "aktiv rolu dЙ™yiЕџmЙ™k")) return;
  ...
  function addSupportTicketComment(ticketId, text) {
    if (!requirePermission("support.manage", "support taskД±na comment yazmaq")) return;
    ...
    setState((current) => {
      ...
      const nextTicket = {
        ...targetTicket,
        status: targetTicket.status === "BaДџlandД±" ? "Д°crada" : targetTicket.status,
      ...
      return auditCurrentState(
        ...
        },
        {
          ...
          action: "Task comment Й™lavЙ™ edildi",
          detail: `${ticketId} В· ${body.slice(0, 80)}`,
        },
      );
    });
    notify("Comment task vЙ™ mesaj thread-inЙ™ Й™lavЙ™ edildi.");
  }

  function updateSupportTicketStatus(ticketId, status) {
    if (!requirePermission("support.manage", "support task statusunu dЙ™yiЕџmЙ™k")) return;
    const stamp = getActionStamp();
    const comment = {
      id: `COM-${Date.now().toString().slice(-6)}`,
      author: activeRoleInfo?.name || "Admin",
      text: `Status dЙ™yiЕџdi: ${status}`,
      at: stamp,
      mine: true,
    };
    ...
    setState((current) => {
      ...
      const nextTicket = {
        ...targetTicket,
        status,
        resolvedAt: status === "BaДџlandД±" ? stamp : targetTicket.resolvedAt,
        comments: [...(targetTicket.comments || []), comment],
        tasks: (targetTicket.tasks || []).map((task) => ({
          ...task,
          status: status === "BaДџlandД±" ? "TamamlandД±" : status === "Д°crada" ? "Д°crada" : task.status,
        })),
      };
      return auditCurrentState(
        {
        ...
        {
          module: "Support",
          action: "Task statusu dЙ™yiЕџdi",
          detail: `${ticketId} В· ${status}`,
        },
      );
    });
    notify(`${ticketId} statusu yenilЙ™ndi: ${status}.`);
  }

  function openSupportConversation(ticketId) {
    const ticket = state.supportTickets.find((item) => item.id === ticketId);
    if (!ticket) {
      notify("Support task tapД±lmadД±.", "warning");
      return;
    }
    ...
    auditOperation({
      module: "Support/Mesaj",
      action: "Task thread-inЙ™ keГ§id edildi",
      detail: `${ticket.id} В· ${ticket.title}`,
    });
  }
  ...
  function openSupportTicket(ticketId) {
    ...
    auditOperation({
      ...
      action: "BaДџlД± task-a keГ§id edildi",
      detail: `${ticket.id} В· ${ticket.title}`,
    });
  }
  ...
  function openLinkedCustomer(fin) {
    ...
    if (!customer) {
      notify("BaДџlД± mГјЕџtЙ™ri tapД±lmadД±.", "warning");
      return;
    }
    setQuery(fin);
    setActive("crm");
    ...
    auditOperation({
      module: "Support/CRM",
      action: "BaДџlД± mГјЕџtЙ™riyЙ™ keГ§id edildi",
      detail: `${customer.name} В· ${fin}`,
    });
  }
  ...
  function createMessageConversation(values = {}) {
    if (!requirePermission("messages.manage", "mesaj sГ¶hbЙ™ti vЙ™ qrup yaratmaq")) return;
    const stamp = getActionStamp();
    ...
    const conversation = normalizeMessageThread({
      ...
      status: "Aktiv",
      archived: false,
      preview: body || "SГ¶hbЙ™t yaradД±ldД±",
      time: stamp,
      unread: 0,
      ...context,
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Mesajlar",
          action: type === "group" ? "Qrup yaradД±ldД±" : "SГ¶hbЙ™t yaradД±ldД±",
          detail: `${title}${contextLabel ? ` В· ${contextLabel}` : ""}`,
        },
      ),
    );
    setConversationId(conversation.id);
    setDraftMessage("");
    notify(type === "group" ? "Qrup yaradД±ldД±." : "Yeni sГ¶hbЙ™t yaradД±ldД±.");
    queueMicrotask(() => syncCommunicationWorkflow(conversation));
  }

  function archiveMessageConversation(id) {
    if (!requirePermission("messages.manage", "mesaj sГ¶hbЙ™tini arxivlЙ™mЙ™k")) return;
    const target = state.conversations.find((conversation) => conversation.id === id);
    if (!target) {
      notify("SГ¶hbЙ™t tapД±lmadД±.", "warning");
      return;
    }
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Mesajlar",
          action: nextArchived ? "SГ¶hbЙ™t arxivlЙ™ndi" : "SГ¶hbЙ™t arxivdЙ™n Г§Д±xarД±ldД±",
          detail: target.title || target.person || id,
        },
      ),
    );
    notify(nextArchived ? "SГ¶hbЙ™t arxivlЙ™ndi." : "SГ¶hbЙ™t arxivdЙ™n Г§Д±xarД±ldД±.");
  ...
  function deleteMessageConversation(id) {
    if (!requirePermission("messages.manage", "mesaj sГ¶hbЙ™tini silmЙ™k")) return;
    const target = state.conversations.find((conversation) => conversation.id === id);
    if (!target) {
      notify("SГ¶hbЙ™t tapД±lmadД±.", "warning");
      return;
    ...
    setState((current) =>
      auditCurrentState(
        ...
        {
          module: "Mesajlar",
          action: "SГ¶hbЙ™t silindi",
          detail: target.title || target.person || id,
        },
      ),
    );
    setConversationId(nextConversation?.id || "");
    notify("SГ¶hbЙ™t silindi.");
  }

  function sendMessage() {
    if (!requirePermission("messages.send", "daxili mesaj gГ¶ndЙ™rmЙ™k")) return;
    ...
    setState((current) => {
      ...
      const nextTickets = ticketId
        ? (current.supportTickets || []).map((ticket) =>
            ticket.id === ticketId
              ? {
                  ...ticket,
                  status: ticket.status === "BaДџlandД±" ? "Д°crada" : ticket.status,
      ...
      return auditCurrentState(
        ...
        },
        {
          ...
          action: ticketId ? "BaДџlД± task-a mesaj yazД±ldД±" : "Daxili mesaj gГ¶ndЙ™rildi",
    ...
    if (selectedThreadForSync) {
      queueMicrotask(() => syncCommunicationWorkflow({
        ...
        messages: [
          ...
          { id: comment.id, from: sender, text: body, time: stamp, status: "GР“В¶ndР™в„ўrildi", readAt: stamp },
        ],
      }));
    }
  }
  ...
  const actionDeniedReason =
    pageHasHeaderAction && actionPermission && !can(actionPermission)
      ? `${activeRoleInfo?.name || "Rol"}: ${meta.action} ГјГ§Гјn icazЙ™ yoxdur.`
      : "";
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="main">
          ...
          <Suspense fallback={<div className="page-suspense-loader" style={{ padding: 32, opacity: 0.6 }}>YГјklЙ™nirвЂ¦</div>}>
      ...
      {modal && (
        <CreateModal
          ...
          orderOptions={{
            ...
            warehouseStock: state.warehouseStock,
            purchaseOrders: state.purchaseOrders || [],
            sellers: state.employees.filter((employee) => employee.department === "SatД±Еџ"),
          }}
          salesDefaults={{
...
function Sidebar({ active, items = navItems, currentUser, activeRole, mobileNav, onClose, onSelect }) {
  return (
    <>
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        <div className="brand">
          ...
          <div>
            ...
            <div className="brand-subtitle">AzЙ™rbaycan Sistemi</div>
          </div>
          <button className="icon-btn sidebar-close" onClick={onClose} aria-label="Menyunu baДџla">
            <X size={18} />
          </button>
        </div>
      ...
      {mobileNav && <button className="scrim" onClick={onClose} aria-label="Menyunu baДџla" />}
    </>
  );
}
...
function formatTimeAgo(date) {
  ...
  if (minutes < 1) return "indicЙ™";
  if (minutes < 60) return `${minutes} dЙ™q Й™vvЙ™l`;
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return `${hours} saat Й™vvЙ™l`;
  return `${Math.floor(hours / 24)} gГјn Й™vvЙ™l`;
}
...
function Topbar({
  ...
  return (
    <header className="topbar">
      <button className="icon-btn mobile-menu" onClick={onMenu} aria-label="Menyunu aГ§">
        <Menu size={20} />
      </button>
      <div className="searchbox">
        ...
        <input
          ...
          placeholder="MГјЕџtЙ™ri, sifariЕџ, mЙ™hsul axtar..."
        />
        {query && (
          <button className="clear-search" onClick={() => setQuery("")} aria-label="AxtarД±ЕџД± sil">
            <X size={14} />
          </button>
        )}
      </div>
      <div className="top-actions">
        ...
        <button className="icon-btn badge-host" onClick={onNotifications} aria-label="BildiriЕџlЙ™r">
          <Bell size={20} />
          ...
          <div
            ...
            title={
              gitHubSync?.isLovableOnly
                ? "LayihЙ™ Lovable Cloud ilЙ™ sinxronizasiya olunur"
                : gitHubSync?.lastCommit
                  ? `${gitHubSync.lastCommit.sha} В· ${gitHubSync.lastCommit.message} В· ${formatTimeAgo(gitHubSync.lastSyncAt)}`
...
function PasswordChangeScreen({ user, onLogout }) {
  ...
  async function submit(event) {
    ...
    if (newPassword.length < 8 || newPassword !== confirmation) {
      setError("Yeni parol Й™n azД± 8 simvol olmalД± vЙ™ tЙ™krar ilЙ™ uyДџun gЙ™lmЙ™lidir.");
      return;
    }
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Parol dЙ™yiЕџdirilmЙ™di.");
    }
  }
  ...
  return (
    <div className="login-screen">
      <div className="login-card">
        <ShieldCheck size={36} />
        <h1>Yeni parol tЙ™yin edin</h1>
        <p>{user.name}, tЙ™hlГјkЙ™sizlik ГјГ§Гјn ilkin parolu dЙ™yiЕџdirin.</p>
        <form className="login-form" onSubmit={submit}>
          <label><span>Д°lkin parol</span><input type="password" required value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></label>
          ...
          <label><span>Yeni parolun tЙ™krarД±</span><input type="password" required minLength={8} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label>
          {error ? <div className="login-error">{error}</div> : null}
          <button type="submit" className="primary-btn">Parolu dЙ™yiЕџ</button>
          <button type="button" className="secondary-btn" onClick={onLogout}>Г‡Д±xД±Еџ</button>
        </form>
      </div>
    </div>
  );
...
const companyModuleCopy = {
  dashboard: ["Д°darЙ™etmЙ™ paneli", "ЖЏsas gГ¶stЙ™ricilЙ™r vЙ™ Гјmumi Й™mЙ™liyyat icmalД±"],
  crm: ["MГјЕџtЙ™rilЙ™r (CRM)", "MГјЕџtЙ™ri bazasД±, Й™laqЙ™lЙ™r vЙ™ 360В° gГ¶rГјnГјЕџ"],
  sales: ["SatД±Еџ", "SifariЕџlЙ™r, satД±Еџ axД±nД± vЙ™ bonuslar"],
  warehouse: ["Anbar vЙ™ stok", "QalД±qlar, rezervlЙ™r vЙ™ anbar Й™mЙ™liyyatlarД±"],
  deliveries: ["TЙ™hvil vЙ™ logistika", "Г‡atdД±rД±lma mЙ™rhЙ™lЙ™lЙ™ri vЙ™ tЙ™hvil nЙ™zarЙ™ti"],
  finance: ["MaliyyЙ™", "Kassa, xЙ™rclЙ™r vЙ™ maliyyЙ™ tЙ™sdiqlЙ™ri"],
  invoices: ["Fakturalar vЙ™ e-qaimЙ™", "Faktura yaradД±lmasД± vЙ™ Г¶dЙ™niЕџ izlЙ™mЙ™"],
  accounting: ["MГјhasibat", "MГјhasibat yazД±lД±ЕџlarД± vЙ™ maliyyЙ™ hesabatlarД±"],
  tax: ["Vergi tЙ™qvimi", "Vergi Г¶hdЙ™liklЙ™ri vЙ™ son tarixlЙ™r"],
  credits: ["KreditlЙ™r", "Kredit satД±ЕџlarД± vЙ™ Г¶dЙ™niЕџ cЙ™dvЙ™llЙ™ri"],
  receivables: ["Debitor vЙ™ kreditor", "Alacaq vЙ™ borc balanslarД±nД±n idarЙ™si"],
  vendors: ["TЙ™chizatГ§Д±lar", "Vendorlar, kvotalar vЙ™ satД±nalma Й™laqЙ™lЙ™ri"],
  projects: ["LayihЙ™lЙ™r vЙ™ ROI", "LayihЙ™ gЙ™lirliliyi vЙ™ investisiya analizi"],
  production: ["Д°stehsalat", "Д°stehsal planlarД± vЙ™ material axД±nД±"],
  hr: ["Д°nsan resurslarД± (HR)", "ЖЏmЙ™kdaЕџlar, ЕџГ¶bЙ™lЙ™r vЙ™ mЙ™zuniyyЙ™tlЙ™r"],
  kpi: ["KPI vЙ™ performans", "HЙ™dЙ™flЙ™r, nЙ™ticЙ™lЙ™r vЙ™ bonus hesablamalarД±"],
  contracts: ["MГјqavilЙ™lЙ™r", "MГјqavilЙ™ ЕџablonlarД± vЙ™ sЙ™nЙ™dlЙ™r"],
  reports: ["Hesabatlar", "Д°darЙ™etmЙ™ hesabatlarД± vЙ™ export"],
  support: ["DЙ™stЙ™k", "SorДџular, tapЕџД±rД±qlar vЙ™ xidmЙ™t izlЙ™mЙ™"],
  help: ["KГ¶mЙ™k mЙ™rkЙ™zi", "TЙ™limatlar vЙ™ istifadЙ™Г§i bЙ™lЙ™dГ§isi"],
  onboarding: ["Д°lkin quraЕџdД±rma", "ЕћirkЙ™tin sistemЙ™ qoЕџulma addД±mlarД±"],
  messages: ["Daxili mesajlar", "Komanda daxilindЙ™ yazД±Еџmalar"],
  notifications: ["BildiriЕџlЙ™r", "Sistem xЙ™bЙ™rdarlД±qlarД± vЙ™ avtomatlaЕџdД±rma"],
  api: ["API inteqrasiyalarД±", "Xarici sistemlЙ™r vЙ™ webhook baДџlantД±larД±"],
  settings: ["Sistem ayarlarД±", "Д°stifadЙ™Г§ilЙ™r, rollar vЙ™ Гјmumi sazlamalar"],
};
...
function PlatformAdminPage() {
  ...
  async function submit(e) {
    ...
    try {
      if (editing) {
        ...
        if (createdTenantId && adminEmail) {
          ...
          if (provErr || prov?.error) {
            throw new Error(provErr?.message || prov?.error || "Admin yaradД±la bilmЙ™di");
          }
      ...
      setError(err?.message || "ЖЏmЙ™liyyat alД±nmadД±.");
    } finally { setBusy(false); }
  }

  async function setStatus(t, status) {
    if (status === "frozen" && !window.confirm(`${t.name} ЕџirkЙ™ti dondurulsun?`)) return;
    if (status === "active" && !window.confirm(`${t.name} ЕџirkЙ™ti aktivlЙ™Еџdirilsin?`)) return;
    const { error: err } = await supabase.rpc("platform_set_tenant_status", { _tenant: t.id, _status: status });
    if (err) return setError(err.message);
    await refreshTenants();
  }
  async function deleteTenant(t) {
    if (!window.confirm(`${t.name} ЕџirkЙ™ti vЙ™ bГјtГјn mЙ™lumatlarД± silinsin? Bu Й™mЙ™liyyat geri qaytarД±lmД±r.`)) return;
  ...
  if (!checked) return <p className="muted">YГјklЙ™nirвЂ¦</p>;
  if (!isAdmin) {
    return (
      <Panel>
        <PanelHeader title="Platform Super Admin" subtitle="Bu bГ¶lmЙ™ yalnД±z platform administratorlarД± ГјГ§ГјndГјr." />
        <p className="muted" style={{ marginBottom: 12 }}>
          HЙ™lЙ™ heГ§ bir platform admin tЙ™yin edilmЙ™yib. Д°lk admin kimi Г¶zГјnГјzГј tЙ™yin edin.
        </p>
        <button type="button" className="primary-btn" onClick={bootstrap} disabled={busy}>
          {busy ? "TЙ™yin olunurвЂ¦" : "MЙ™ni platform admin tЙ™yin et"}
        </button>
  ...
  return (
    <div className="page-grid">
      <section className="metric-grid four" style={{ gridColumn: "1 / -1" }}>
        <MetricCard label="Aktiv Р•СџirkР™в„ўt" value={activeTenantCount} trend={`${tenants.length} tenant`} icon={Building2} tone="success" />
        <MetricCard label="Platform istifadР™в„ўР“В§isi" value={totalPlatformUsers} trend="BР“СtР“Сn tenant-lР™в„ўr" icon={Users} tone="primary" />
        <MetricCard label="DondurulmuР•Сџ" value={frozenTenantCount} trend="GiriР•Сџ mР™в„ўhdudiyyР™в„ўti" icon={ShieldCheck} tone={frozenTenantCount ? "warning" : "success"} />
        <MetricCard label="30 gР“СnР™в„ў bitР™в„ўn" value={expiringTenantCount} trend="Lisenziya nР™в„ўzarР™в„ўti" icon={CalendarClock} tone={expiringTenantCount ? "warning" : "info"} />
      </section>
      <Panel>
        <PanelHeader
          title={editing ? `ЕћirkЙ™ti redaktЙ™ et вЂ” ${editing.name}` : "Yeni ЕџirkЙ™t yarat"}
          subtitle={editing ? "DЙ™yiЕџikliklЙ™ri yadda saxlayД±n." : "Yeni tenant vЙ™ (istЙ™yЙ™ baДџlД±) admin вЂ” mГјvЙ™qqЙ™ti parol yaradД±lД±r."}
          action={editing ? <button type="button" className="secondary-btn" onClick={cancelEdit}>LЙ™Дџv et</button> : null}
        />
        <form className="form-grid" onSubmit={submit} style={{ gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label className="field">
              <span>ЕћirkЙ™t adД± *</span>
            ...
            {!editing && (
              <label className="field" style={{ gridColumn: "1 / -1" }}>
                <span>Admin e-poГ§tu (mГјvЙ™qqЙ™ti parol yaradД±lacaq)</span>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <span style={{ fontWeight: 600, fontSize: 13 }}>Д°cazЙ™ verilЙ™n modullar ({form.modules.length}/{PLATFORM_MODULE_CHOICES.length})</span>
              <div style={{ display: "flex", gap: 8 }}>
                <button type="button" className="link-btn" onClick={() => setForm({ ...form, modules: PLATFORM_MODULE_CHOICES.map((m) => m.id) })}>HamД±sД±</button>
                <button type="button" className="link-btn" onClick={() => setForm({ ...form, modules: [] })}>HeГ§ biri</button>
              </div>
            </div>
          ...
          <div className="form-actions">
            <button type="submit" className="primary-btn" disabled={busy || !form.name.trim()}>
              {busy ? "SaxlanД±lД±rвЂ¦" : (editing ? "Yadda saxla" : "ЕћirkЙ™t yarat")}
            </button>
          </div>
        </form>
        {error && <div className="form-error" style={{ marginTop: 10 }}>{error}</div>}
        {credential && (
          <div style={{ marginTop: 12, padding: 14, borderRadius: 12, background: "#e6f4ef", border: "1px solid #0d7a5f" }}>
            <div style={{ fontWeight: 700, color: "#064e3b", marginBottom: 6 }}>Admin giriЕџi yaradД±ldД±</div>
            <div style={{ fontSize: 13, color: "#0f2a20" }}>Bu mЙ™lumatД± admin ilЙ™ paylaЕџД±n вЂ” bu pЙ™ncЙ™rЙ™ni baДџladД±qdan sonra parolu yenidЙ™n gГ¶rЙ™ bilmЙ™yЙ™cЙ™ksiniz.</div>
            <div style={{ marginTop: 8, display: "grid", gap: 4, fontFamily: "monospace", fontSize: 14 }}>
              <div><strong>E-poГ§t:</strong> {credential.email}</div>
              <div><strong>MГјvЙ™qqЙ™ti parol:</strong> {credential.password}</div>
            </div>
            <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
              ...
              <button type="button" className="link-btn" onClick={() => setCredential(null)}>BaДџla</button>
            </div>
          </div>
        )}
      </Panel>
      ...
      <Panel>
        <PanelHeader
          title="BГјtГјn ЕџirkЙ™tlЙ™r"
          subtitle={`${tenants.length} tenant`}
          action={<button type="button" className="secondary-btn" onClick={refreshTenants}>YenilЙ™</button>}
        />
        {loading ? <p className="muted">YГјklЙ™nirвЂ¦</p> : (
          <DataTable
            columns={["ЕћirkЙ™t", "Plan", "Status", "Д°stifadЙ™Г§i", "BitmЙ™", "Modullar", "ЖЏmЙ™liyyat"]}
            rows={tenants.map((t) => [
              ...
              <div className="table-actions" style={{ display: "flex", gap: 4 }}>
                <button type="button" className="icon-btn" title="RedaktЙ™" onClick={() => startEdit(t)}><Pencil size={14} /></button>
                {t.status === "active" ? (
                  <button type="button" className="icon-btn" title="Dondur" onClick={() => setStatus(t, "frozen")}>вќ„</button>
                ) : (
                  <button type="button" className="icon-btn" title="AktivlЙ™Еџdir" onClick={() => setStatus(t, "active")}>в–¶</button>
                )}
  ...
  return (
    <main className="login-shell">
      <section className="login-card">
        ...
        <div>
          <h1>ERP+CRM AZ</h1>
          <p>Д°stifadЙ™Г§i seГ§in vЙ™ rol icazЙ™lЙ™ri ilЙ™ sistemЙ™ daxil olun.</p>
        </div>
        {authMode === "password" ? (
  ...
  return (
    <form className="login-password-form" onSubmit={submit}>
      ...
      <button className="primary-btn full" type="submit" disabled={isLoading}>
        <ShieldCheck size={16} />
        {isLoading ? "YoxlanД±lД±r..." : "SistemЙ™ daxil ol"}
      </button>
    </form>
  );
}
...
function CrmPage({ customers, credits, orders = [], contracts = [], onOpenSalesOrder, onOpenCredit, onDeleteCustomer }) {
  const [pipelineFilter, setPipelineFilter] = useState("HamД±sД±");
  const [selectedPipelineId, setSelectedPipelineId] = useState(null);
  const [selectedCustomerFin, setSelectedCustomerFin] = useState("");
  const [customerSearch, setCustomerSearch] = useState("");
  const [customerSegment, setCustomerSegment] = useState("HamД±sД±");
  ...
  const pipelineStages = ["HamД±sД±", "Kredit uyДџunluДџu", "TЙ™klif", "Upsell", "TЙ™hvil sonrasД±", "Risk follow-up"];
  const customerSegments = ["HamД±sД±", "Aktiv kredit", "GecikmЙ™", "AГ§Д±q tЙ™hvil", "Borcsuz"];
  ...
  return (
    <div className="stack">
      <section className="metric-grid three">
        <MetricCard label="Гњmumi mГјЕџtЙ™ri" value={customers.length} icon={Users} tone="primary" />
        <MetricCard label="Platin mГјЕџtЙ™rilЙ™r" value={platin.length} icon={ShieldCheck} tone="success" />
        <MetricCard
          label="GecikmiЕџ Г¶dЙ™niЕџ"
          value={`${delayed.length} mГјЕџtЙ™ri`}
          trend={`${money(delayedDebt)} Гјmumi borc`}
          icon={CircleAlert}
          tone="danger"
        />
      </section>
      <section className="dashboard-grid crm-command-grid">
        <Panel className="span-2 crm-pipeline-panel">
          <PanelHeader
            title="CRM Pipeline"
            subtitle="Lead, tЙ™klif, kredit uyДџunluДџu vЙ™ tЙ™hvil sonrasД± satД±Еџ fГјrsЙ™tlЙ™ri"
            icon={TrendingUp}
          />
            ...
            {kanbanColumns.map((column) => (
              <div className="crm-kanban-column" key={column.stage}>
                <div className="crm-kanban-head">
                  ...
                  <span>{column.rows.length} В· {money(column.value)}</span>
                </div>
                <div className="crm-kanban-cards">
                  {column.rows.slice(0, 3).map((row) => (
                    ...
                    >
                      ...
                      <small>{money(row.value)} В· {row.probability}%</small>
                    </button>
                  ))}
                  {column.rows.length === 0 && <span className="crm-kanban-empty">BoЕџdur</span>}
                </div>
              </div>
            ))}
          </div>
            ...
            {visiblePipeline.map((row) => (
              ...
              >
                <div>
                  <strong>{row.customer.name}</strong>
                  <span>
                    {row.source} В· {row.nextAction}
                  </span>
                </div>
                <TwoLine title={money(row.value)} subtitle={`${row.probability}% ehtimal`} />
                <TwoLine title={row.owner} subtitle={`${money(row.limitLeft)} limit qalД±ДџД±`} />
          ...
          {selectedPipeline && (
            <div className="crm-pipeline-selection" aria-live="polite">
              ...
              <div>
                ...
                <small>{selectedPipeline.owner} В· {selectedPipeline.stage}</small>
              </div>
              <div>
                <span>Fursat</span>
        ...
        <Panel className="crm-automation-panel">
          <PanelHeader title="NГ¶vbЙ™ti addД±mlar" subtitle="SatД±Еџ vЙ™ risk komandasД± ГјГ§Гјn avtomatik iЕџ siyahД±sД±" icon={CalendarClock} />
      ...
      <Panel className="customer-portal-panel">
        <PanelHeader
          title="MГјЕџtЙ™ri portalД± hazД±rlД±ДџД±"
          subtitle="Balans, mГјqavilЙ™, Г¶dЙ™niЕџ tarixi vЙ™ tЙ™hvil statusu mГјЕџtЙ™ri kabinetindЙ™ gГ¶rГјnЙ™cЙ™k"
          icon={Users}
        />
        <DataTable
          columns={["MГјЕџtЙ™ri", "Aktiv kredit", "QalД±q", "NГ¶vbЙ™ti Г¶dЙ™niЕџ", "AГ§Д±q sifariЕџ", "Portal statusu"]}
          rows={portalReady.map((row) => [
            ...
            >
              <TwoLine title={row.customer.name} subtitle={`FД°N ${row.customer.fin}`} />
            </button>,
            row.activeCreditCount,
            money(row.totalBalance),
            row.nextPayment ? `${money(row.nextPayment.amount)} В· ${row.nextPayment.due}` : "Yoxdur",
            row.openOrders,
            <StatusBadge status={row.totalBalance > 0 ? "Aktiv kabinet" : "MЙ™lumat kabineti"} />,
          ])}
        />
      </Panel>
      <Panel>
        <PanelHeader title="MГјЕџtЙ™ri Reyestri" subtitle="FД°N, mГјqavilЙ™, cihaz, qalД±q vЙ™ gecikmЙ™ ГјzrЙ™ 360 nЙ™zarЙ™t" />
        <div className="crm-registry-toolbar">
          <label className="crm-search-field">
            ...
            <div>
              ...
              <input
                ...
                placeholder="MГјЕџtЙ™ri, FД°N, mГјqavilЙ™, cihaz..."
              />
            </div>
          </label>
          <label>
          ...
          <button
            ...
            type="button"
            title="FilterlЙ™ri sД±fД±rla"
            onClick={() => {
              setCustomerSearch("");
              setCustomerSegment("HamД±sД±");
            }}
          >
            <RefreshCw size={16} />
          </button>
          <div className="crm-registry-count">
            <span>GГ¶rГјnЙ™n mГјЕџtЙ™ri</span>
            <strong>{visibleCustomerProfiles.length}</strong>
          </div>
        </div>
        <DataTable
          columns={["FД°N", "Ad Soyad", "Telefon", "Kateqoriya", "MГјqavilЙ™", "QalД±q", "NГ¶vbЙ™ti Г¶dЙ™niЕџ", "Status", "ЖЏmЙ™liyyat"]}
          rows={visibleCustomerProfiles.map(({ customer, profile }) => {
            ...
            const status =
              profile.overdueCount > 0 || customer.delay > 0
                ? `${Math.max(customer.delay || 0, nextPayment?.overdueDays || 0)} gГјn gecikmЙ™`
            ...
            return [
              ...
              nextPayment ? `${money(nextPayment.monthly)} В· ${nextPayment.nextDue}` : "Yoxdur",
...
function CustomerCreditHistory({ credits }) {
  ...
  return (
    <div className="customer-credit-history">
      <strong>
        {credits.length} kredit В· {money(totalBalance)} qalД±q
      </strong>
      <span>
        {latest.id} В· {latestPlan.months} ay В· {money(latestPlan.monthly)}/ay
      </span>
    </div>
  );
}
      ...
      <div className="modal-card customer-360-modal-card">
        <div className="modal-head customer-360-head">
          <div>
            ...
            <p>FД°N {customer.fin} ГјzrЙ™ mГјqavilЙ™, cihaz, Г¶dЙ™niЕџ vЙ™ tЙ™hvil 360 baxД±ЕџД±</p>
          </div>
          <button className="icon-btn" type="button" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        </div>
          ...
          <section className="customer-360-profile-grid">
            <div className="customer-360-profile-card">
              ...
              <div>
                ...
                <span>{customer.category} В· FД°N {customer.fin}</span>
              </div>
            </div>
            <div className="customer-360-profile-field">
              <span>Telefon</span>
              <strong>{customer.phone || "Qeyd edilmЙ™yib"}</strong>
            </div>
            <div className="customer-360-profile-field">
            ...
            <div className="customer-360-profile-field">
              ...
              <StatusBadge status={profile.overdueCount > 0 || customer.delay > 0 ? "GecikmЙ™ nЙ™zarЙ™ti" : "SaДџlam"} />
            </div>
          </section>

          <section className="customer-360-summary">
            <div>
              <span>MГјqavilЙ™ mЙ™blЙ™Дџi</span>
              <strong>{money(profile.totalPurchased)}</strong>
            </div>
            <div>
              <span>Г–dЙ™nilib</span>
          ...
          <section className="customer-360-grid">
            <div className="customer-360-main">
              <Panel className="customer-360-section">
                <PanelHeader title="Kredit vЙ™ satД±Еџ mГјqavilЙ™lЙ™ri" subtitle="HЙ™r satД±Еџ ayrД± borc kimi saxlanД±lД±r" icon={FileText} />
                <div className="customer-360-contract-list">
                  {profile.agreements.length === 0 ? (
                    <EmptyState title="MГјЕџtЙ™ri ГјzrЙ™ mГјqavilЙ™ yoxdur" />
                  ) : (
                    profile.agreements.map((agreement) => (
                      ...
                      >
                        ...
                        <div className="customer-360-contract-meta">
                          <span>
                            SifariЕџ
                            {agreement.orderId && agreement.orderId !== "вЂ”" ? (
                              <button
                          ...
                          <span>
                            Kredit
                            {agreement.creditId && agreement.creditId !== "вЂ”" ? (
                              <button
                                className="module-link-btn"
                                type="button"
                          ...
                          <span>Tarix <strong>{agreement.date || "вЂ”"}</strong></span>
                          <span>MЙ™nbЙ™ <strong>{agreement.source}</strong></span>
                        </div>
                        <div className="customer-360-contract-values">
                          ...
                          <div>
                            <span>NГ¶vbЙ™ti</span>
                            <strong>{agreement.monthly > 0 ? `${money(agreement.monthly)} В· ${agreement.nextDue}` : "вЂ”"}</strong>
                          </div>
                        </div>
                        {agreement.plan ? (
                          <details className="customer-360-schedule-preview">
                            <summary>Г–dЙ™niЕџ cЙ™dvЙ™li В· {agreement.paidMonths}/{agreement.months} ay В· {agreement.remainingMonths} qalД±b</summary>
                          ...
                          <div className="customer-360-direct-note">
                            <span>SatД±Еџ balansД±</span>
                            <strong>{agreement.balance > 0 ? `${money(agreement.balance)} qalД±q` : "Tam Г¶dЙ™nilib"}</strong>
                          </div>
                        )}
                      </article>
                    ))
              ...
              <Panel className="customer-360-section">
                <PanelHeader title="AlД±nan cihazlar" subtitle="HЙ™r cihaz ГјzrЙ™ Г¶dЙ™nilЙ™n vЙ™ qalan mЙ™blЙ™Дџ" icon={Package} />
                <DataTable
                  columns={["Cihaz", "MГјqavilЙ™", "SifariЕџ", "MЙ™blЙ™Дџ", "Г–dЙ™nilib", "QalД±q", "Status"]}
                  rows={profile.devices.map((device) => [
                    <TwoLine title={device.product} subtitle={`${device.qty} Й™dЙ™d${device.serials.length ? ` В· ${device.serials.join(", ")}` : ""}`} />,
                    device.contractId,
                    device.orderId,
                    money(device.amount),
                    money(device.paid),
            ...
            <aside className="customer-360-side">
              <Panel className="customer-360-section">
                <PanelHeader title="YД±ДџД±m vЙ™ risk" subtitle="NГ¶vbЙ™ti Г¶dЙ™niЕџ, gecikmЙ™ vЙ™ tЙ™hvil nЙ™zarЙ™ti" icon={CreditCard} />
                <div className="customer-360-risk-list">
                  <div>
                    <span>NГ¶vbЙ™ti Г¶dЙ™niЕџ</span>
                    <strong>{nextPayment ? `${money(nextPayment.monthly)} В· ${nextPayment.nextDue}` : "Yoxdur"}</strong>
                  </div>
                  <div>
                    <span>GecikЙ™n mГјqavilЙ™</span>
                    <strong>{profile.overdueCount}</strong>
                  </div>
                  <div>
                    <span>AГ§Д±q tЙ™hvil</span>
                    <strong>{profile.openOrders}</strong>
                  </div>
                  <div>
                    <span>Гњmumi qalД±q</span>
                    <strong>{money(profile.totalBalance + Number(customer.debt || 0))}</strong>
                  </div>
                </div>
              </Panel>
              ...
              <Panel className="customer-360-section">
                <PanelHeader title="Son Г¶dЙ™niЕџlЙ™r" subtitle="ЖЏsas mЙ™blЙ™Дџ vЙ™ gecikmЙ™ gЙ™liri ayrД± gГ¶stЙ™rilir" icon={Wallet} />
                <div className="customer-360-payment-feed">
                  {latestPayments.length === 0 ? (
                    <EmptyState title="Г–dЙ™niЕџ tarixГ§Й™si yoxdur" />
                  ) : (
                    latestPayments.map((payment, index) => (
                      <div className="customer-360-payment-row" key={`${payment.creditId}-${payment.date}-${index}`}>
                        ...
                        <div>
                          ...
                          <span>ЖЏsas {money(payment.principal)} В· GecikmЙ™ {money(payment.penalty)}</span>
                        </div>
                      </div>
                    ))
                  )}
              ...
              <Panel className="customer-360-section">
                <PanelHeader title="MГјЕџtЙ™ri Й™laqЙ™si" subtitle="SatД±Еџ vЙ™ yД±ДџД±m komandasД± ГјГ§Гјn sГјrЙ™tli dosye" icon={Users} />
                <div className="customer-360-contact-card">
                  ...
                  <div>
                    <span>Telefon</span>
                    <strong>{customer.phone || "вЂ”"}</strong>
                  </div>
                  <div>
...
function SalesPage({
  ...
  const [salesFilter, setSalesFilter] = useState("HamД±sД±");
  const [sellerFilter, setSellerFilter] = useState("BГјtГјn satД±cД±lar");
  const [warehouseFilter, setWarehouseFilter] = useState("BГјtГјn anbarlar");
  const [salesSearch, setSalesSearch] = useState("");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const sellers = employees.filter((employee) => employee.department === "SatД±Еџ");
  ...
  const warehouseOptions = [
    ...new Set(orders.map((order) => order.warehouseName || "Anbar seГ§ilmЙ™yib").filter(Boolean)),
  ];
  const filteredOrders = useMemo(
    () =>
      orders.filter((order) => {
        const sellerNames = getOrderSellerBonuses(order).map((item) => item.seller);
        const matchesSeller = sellerFilter === "BГјtГјn satД±cД±lar" || sellerNames.includes(sellerFilter);
        const matchesWarehouse = warehouseFilter === "BГјtГјn anbarlar" || (order.warehouseName || "Anbar seГ§ilmЙ™yib") === warehouseFilter;
        return (
  ...
  const selectedCreditPlan =
    selected?.paymentMethod === "Kredit"
      ...
      : null;
  const activeOrders = orders.filter((order) => order.status !== "TЙ™hvil verilib");
  const creditOrders = orders.filter((order) => getOrderPaymentMethod(order) === "Kredit");
  ...
  const visibleDeliveryWaiting = filteredOrders.filter((order) => order.status !== "TЙ™hvil verilib").length;
  const riskOrders = filteredOrders.filter((order) => getSalesOrderRiskStatus(order) !== "SaДџlam" && getSalesOrderRiskStatus(order) !== "TamamlanД±b");
  ...
  const deliveryWaiting = orders.filter((order) => order.status !== "TЙ™hvil verilib").length;
  const salesFilterOptions = ["HamД±sД±", "Kredit", "NaДџd", "QalД±qlД±", "TЙ™hvil gГ¶zlЙ™yЙ™n", "Tamamlanan", "Riskli"];
  ...
  const actionOrders = (riskOrders.length > 0 ? riskOrders : orders)
    .filter((order) => getOrderBalance(order) > 0 || order.status !== "TЙ™hvil verilib")
    .slice(0, 4);
  const criticalStock = [...stock]
    .sort((a, b) => a.total - a.reserved - (b.total - b.reserved))
    .slice(0, 5);
  const resetSalesFilters = () => {
    setSalesFilter("HamД±sД±");
    setSellerFilter("BГјtГјn satД±cД±lar");
    setWarehouseFilter("BГјtГјn anbarlar");
    setSalesSearch("");
    setDateFrom("");
    setDateTo("");
  };
  const exportVisibleSales = () => {
    ...
    const rows = [
      ["SifariЕџ", "Tarix", "MГјЕџtЙ™ri", "FIN", "MЙ™hsul", "Г–dЙ™niЕџ tipi", "Anbar", "MГјqavilЙ™", "Kredit", "MЙ™blЙ™Дџ", "Daxil olub", "QalД±q", "Bonus", "Status"],
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="Aktiv sifariЕџlЙ™r" value={activeOrders.length} icon={ShoppingCart} tone="primary" />
        <MetricCard label="Гњmumi dГ¶vriyyЙ™" value={money(total(orders, "amount"))} icon={Wallet} tone="success" />
      ...
      <Panel className="sales-control-panel">
        <PanelHeader
          title="SatД±Еџ nЙ™zarЙ™ti"
          subtitle="Г–dЙ™niЕџ, tЙ™hvil vЙ™ bonus ГјzrЙ™ operativ gГ¶stЙ™ricilЙ™r"
          icon={Filter}
        />
        <div className="sales-control-grid">
          <div className="sales-control-tile">
            <span>Kredit satД±ЕџlarД±</span>
            <strong>{creditOrders.length}</strong>
            <small>{money(total(creditOrders, "amount"))} portfel</small>
          </div>
          <div className="sales-control-tile">
            <span>TЙ™hvil gГ¶zlЙ™yЙ™n</span>
            <strong>{deliveryWaiting}</strong>
            <small>Anbar Г§Д±xД±ЕџД± izlЙ™nir</small>
          </div>
          <div className="sales-control-tile">
            <span>Bonus fondu</span>
            <strong>{money(bonusTotal)}</strong>
            <small>{salesBonusRows.length} bonus sЙ™tri</small>
          </div>
          <div className="sales-control-tile">
            <span>Orta Г§ek</span>
            <strong>{money(averageCheck)}</strong>
            <small>{orders.length} sifariЕџ ГјzrЙ™</small>
          </div>
        </div>
        <div className="sales-alert-list">
          {actionOrders.map((order) => (
            <button key={order.id} className="sales-alert-row" onClick={() => setSelectedOrder(order.id)}>
              <div>
                <strong>
                  {order.id} В· {order.customer}
                </strong>
                <span>
                  {getOrderBalance(order) > 0
                    ? `${money(getOrderBalance(order))} qalД±q`
      ...
      <section className="sales-registry-summary" aria-label="SatД±Еџ filter nЙ™ticЙ™lЙ™ri">
        <div>
          <span>GГ¶rГјnЙ™n satД±Еџ</span>
          <strong>{money(visibleSalesTotal)}</strong>
          <small>{filteredOrders.length} sifariЕџ</small>
        </div>
        <div>
          <span>Kassaya daxil olan</span>
          <strong>{money(visibleCashIn)}</strong>
          <small>NaДџd/kart/ilkin Г¶dЙ™niЕџ</small>
        </div>
        <div>
          <span>QalД±q borc</span>
          <strong>{money(visibleBalance)}</strong>
          <small>{riskOrders.length} risk siqnalД±</small>
        </div>
        <div>
          <span>Bonus</span>
        ...
        <div>
          ...
          <small>Anbar rezervi izlЙ™nir</small>
        </div>
      </section>

      <section className="dashboard-grid">
        <Panel>
          <PanelHeader title="SatД±cД± bonus performansД±" subtitle="SatД±Еџ sifariЕџlЙ™rindЙ™n real bonus hesabД±" />
          <div className="seller-bonus-list">
            {sellerBonusStats.length === 0 ? (
              <EmptyState title="SatД±cД± bonus datasД± yoxdur" />
            ) : (
              sellerBonusStats.map((seller) => (
                <div className="seller-bonus-row" key={seller.name}>
                  <div className="seller-bonus-main">
                    <AvatarLine initials={seller.initials} title={seller.name} subtitle={`${seller.orderCount} sifariЕџ`} />
        ...
        <Panel>
          <PanelHeader title="Stok siqnallarД±" subtitle="SatД±Еџ ГјГ§Гјn Й™n hЙ™ssas qalД±qlar" />
        ...
        <Panel>
          <PanelHeader title="SifariЕџ KartД±" subtitle={selected?.id || "SifariЕџ seГ§ilmЙ™yib"} />
          {selected ? (
            <div className="detail-card sales-order-card">
              ...
              <div className="order-detail-grid">
                ...
                <div>
                  ...
                  <small>{selected.paymentStatus || "Г–dЙ™niЕџ izlЙ™nir"}</small>
                </div>
                <div>
                  <span>Anbar</span>
                  <strong>{selected.warehouseName || "Anbar qeyd edilmЙ™yib"}</strong>
              ...
              {selectedCreditPlan && (
                <div className="selected-credit-summary">
                  <span>Д°lkin: {money(selectedCreditPlan.initialPayment)}</span>
                  <span>QalД±q: {money(selectedCreditPlan.balance)}</span>
                  <span>{selectedCreditPlan.months} ay В· {money(selectedCreditPlan.monthly)}/ay</span>
                </div>
              )}
            ...
            <EmptyState title="SifariЕџ seГ§ilmЙ™yib" />
          )}
        </Panel>
      </section>

      <Panel className="sales-registry-panel">
        <PanelHeader title="SatД±Еџ reyestri" subtitle="Filter edib sifariЕџ, Г¶dЙ™niЕџ, anbar vЙ™ bonus vЙ™ziyyЙ™tini izlЙ™yin" />
          ...
          <div className="sales-filter-controls">
            <label className="sales-search-field">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="SifariЕџ, mГјЕџtЙ™ri, FД°N, cihaz..."
                />
              </div>
            </label>
            <label className="sales-seller-filter">
              ...
              <select value={sellerFilter} onChange={(event) => setSellerFilter(event.target.value)}>
                <option>BГјtГјn satД±cД±lar</option>
            ...
            <label className="sales-seller-filter">
              ...
              <select value={warehouseFilter} onChange={(event) => setWarehouseFilter(event.target.value)}>
                <option>BГјtГјn anbarlar</option>
            ...
            <button className="secondary-btn icon-only" type="button" title="FilterlЙ™ri sД±fД±rla" onClick={resetSalesFilters}>
              <RefreshCw size={16} />
            </button>
        ...
        <DataTable
          columns={["в„–", "MГјЕџtЙ™ri", "MЙ™hsul", "Г–dЙ™niЕџ", "Anbar", "SЙ™nЙ™dlЙ™r", "Bonus", "QalД±q", "Status", "ЖЏmЙ™liyyat"]}
          rows={filteredOrders.map((order) => [
            ...
            <TwoLine title={order.customer} subtitle={`FД°N ${order.fin}`} />,
            ...
            <TwoLine title={order.warehouseName || "Anbar seГ§ilmЙ™yib"} subtitle={getOrderDeliveryStatus(order)} />,
            <TwoLine title={order.contractId || "MГјqavilЙ™ yoxdur"} subtitle={order.creditId || getSalesOrderRiskStatus(order)} />,
            <TwoLine title={money(getOrderBonusAmount(order))} subtitle={getOrderBonusText(order)} />,
            getOrderBalance(order) > 0 ? <strong>{money(getOrderBalance(order))}</strong> : <StatusBadge status="Г–dЙ™nilib" />,
            <StatusBadge status={order.status} />,
            <div className="row-actions operation-table-actions">
              <button className="text-btn" onClick={() => onEditOrder(order.id)}>RedaktЙ™</button>
...
// SatД±Еџ ГјГ§Гјn real sЙ™rbЙ™st qalД±q вЂ” mЙ™nfi ola bilЙ™r (backorder / sifariЕџ gГ¶zlЙ™yЙ™n miqdar).
  ...
  return warehouses.flatMap((warehouse) =>
    (warehouseStock[warehouse.id] || [])
      ...
      .map(({ item, reorderPoint }) => ({
          ...
          available: getAvailableQuantity(item),
          reorderPoint,
          status: getAvailableQuantity(item) <= Math.max(1, Math.floor(reorderPoint / 2)) ? "Kritik stok" : "AЕџaДџД± stok",
        })),
  );
}
  ...
  return [...byProduct.entries()].flatMap(([product, rows]) => {
    ...
    return targets.flatMap((target) => {
      ...
      return {
        ...
        qty,
        reason: `${target.warehouseName} ГјzrЙ™ satД±Еџ ГјГ§Гјn ${target.available} qalД±b`,
      };
    });
  });
}
...
export function WarehouseStockToolbar({ filter, setFilter }) {
  const filters = ["HamД±sД±", "SatД±Еџ ГјГ§Гјn var", "RezervdЙ™", "AЕџaДџД± stok"];
  return (
    <div className="warehouse-stock-toolbar">
      <div>
        <h2>Anbar ГјzrЙ™ mallar</h2>
        <p>Filter seГ§ib Гјmumi vЙ™ ya seГ§ilmiЕџ anbar ГјzrЙ™ qalД±qlara baxД±n</p>
      </div>
      <div className="tabs">
        {filters.map((item) => (
...
export function DeliveryOrdersPanel({ orders, isAllWarehouses, warehouseStock = {}, onCompleteDelivery }) {
  return (
    <div className="delivery-orders-panel">
      <PanelHeader
        title="TЙ™hvil verilmЙ™li mЙ™hsullar"
        subtitle={
          isAllWarehouses
            ? "BГјtГјn anbarlar ГјzrЙ™ rezervdЙ™ olan vЙ™ tЙ™hvil gГ¶zlЙ™yЙ™n sifariЕџlЙ™r"
            : "SeГ§ilmiЕџ anbardan Г§Д±xarД±lmalД± rezerv mЙ™hsullar"
        }
      />
      <DataTable
        columns={["SifariЕџ", "MГјЕџtЙ™ri", "MЙ™hsullar", "Anbar", "Г–dЙ™niЕџ", "Rezerv", "ЖЏmЙ™liyyat"]}
        rows={orders.map((order) => {
          ...
          return [
            ...
            order.warehouseName || "BaЕџ Anbar",
            <StatusBadge status={order.paymentStatus || order.paymentMethod || "NaДџd"} />,
            <TwoLine
              title={<StatusBadge status={stockCheck.status} />}
              subtitle={
                stockCheck.partial
                  ? `${stockCheck.plan.deliverableTotal} Й™dЙ™d indi В· ${stockCheck.plan.shortageTotal} backorder`
                  : stockCheck.ok
                    ? `${stockCheck.plan?.remainingTotal ?? getDeliveryTotalQuantity(order)} Й™dЙ™d rezervdЙ™`
                    : stockCheck.reason
              }
            />,
            <button
            ...
            >
              {stockCheck.partial ? "QismЙ™n tЙ™hvil ver" : "TЙ™hvil verildi"}
            </button>,
...
export function WarehouseControlPanel({ summary, deliveryCount, alerts, isAllWarehouses, onSelect }) {
  return (
    <Panel className="warehouse-control-panel">
      <PanelHeader
        title="Anbar nЙ™zarЙ™ti"
        subtitle={isAllWarehouses ? "BГјtГјn anbarlar ГјzrЙ™ canlД± Й™mЙ™liyyat xГјlasЙ™si" : "SeГ§ilmiЕџ anbar ГјzrЙ™ Й™mЙ™liyyat xГјlasЙ™si"}
        icon={SlidersHorizontal}
      />
      <div className="warehouse-control-grid">
        <div className="warehouse-control-tile">
          <span>SatД±Еџ ГјГ§Гјn</span>
          <strong>{summary.available} Й™dЙ™d</strong>
          <small>{money(summary.value)} dЙ™yЙ™r</small>
        </div>
        <div className="warehouse-control-tile">
          <span>Rezerv yГјkГј</span>
          <strong>{summary.reserved} Й™dЙ™d</strong>
          <small>{percent(summary.reservedRate)} stok rezervdЙ™</small>
        </div>
        <div className="warehouse-control-tile">
          <span>TЙ™hvil nГ¶vbЙ™si</span>
          <strong>{deliveryCount}</strong>
          <small>Anbardan Г§Д±xmalД± sifariЕџ</small>
        </div>
        <div className="warehouse-control-tile">
          <span>Doluluq</span>
          <strong>{summary.utilization}%</strong>
          <small>{summary.sku} mЙ™hsul Г§eЕџidi</small>
        </div>
      </div>
        ...
        {alerts.slice(0, 4).map((alert) => (
          <button key={`${alert.warehouseId}-${alert.product}`} className="warehouse-signal-row" onClick={() => onSelect(alert.warehouseId)}>
            <div>
              ...
              <span>
                {alert.warehouseName} В· satД±Еџ ГјГ§Гјn {alert.available} Й™dЙ™d
              </span>
            </div>
...
export function WarehouseTransferPanel({ suggestions, onTransferStock }) {
  return (
    <div className="warehouse-transfer-panel">
      <PanelHeader
        title="Transfer tГ¶vsiyЙ™lЙ™ri"
        subtitle="AЕџaДџД± qalД±qlД± anbarlara artД±q stok olan anbardan daxili transfer"
        icon={Truck}
      />
      <DataTable
        columns={["MЙ™hsul", "Haradan", "Haraya", "Miqdar", "SЙ™bЙ™b", "ЖЏmЙ™liyyat"]}
        rows={suggestions.map((suggestion) => [
          <strong>{suggestion.product}</strong>,
          suggestion.fromWarehouse,
          suggestion.toWarehouse,
          `${suggestion.qty} Й™dЙ™d`,
          suggestion.reason,
...
function getWarehouseBalanceStatus(available, reorderPoint) {
  if (available <= 0) return "Stok tГјkЙ™nib";
  if (reorderPoint > 0 && available <= Math.max(1, Math.floor(reorderPoint / 2))) return "Kritik stok";
  if (reorderPoint > 0 && available <= reorderPoint) return "AЕџaДџД± stok";
  return "Normal";
}
...
function buildWarehouseBalanceRows({ warehouses = [], warehouseStock = {}, products = [], purchaseOrders = [], view = "products", warehouseId = "all" }) {
  ...
  const createRow = (productName, catalogProduct = null) => ({
    ...
    sku: catalogProduct?.sku || "вЂ”",
    unit: catalogProduct?.unit || "Й™dЙ™d",
  ...
  if (view === "products") {
    ...
    return [...rowsByProduct.values()]
      .map((row) => {
        ...
        return {
          ...row,
          warehouseName: row.warehouseDistribution.length === 0 ? "вЂ”" : `${row.warehouseDistribution.length} anbar`,
          ...
          openPoCount: Number(coverage.count || 0),
          latestPoId: coverage.latest?.id || "",
          status: shortage > 0 ? "Г‡atД±ЕџmazlД±q" : getWarehouseBalanceStatus(available, row.reorderLevel),
    ...
    .flatMap(([sourceWarehouseId, items]) => {
      ...
      return (items || []).map((item) => {
        ...
        return {
          ...
          latestPoId: coverage.latest?.id || "",
          warehouseDistribution: [],
          status: shortage > 0 ? "Г‡atД±ЕџmazlД±q" : getWarehouseBalanceStatus(available, reorderLevel),
...
function filterWarehouseBalanceRows(rows, filters, globalQuery = "") {
  ...
  return rows.filter((row) => {
    ...
    const matchesStock =
      ...
      (filters.stockStatus === "below" && (row.status === "AЕџaДџД± stok" || row.status === "Kritik stok" || row.status === "Stok tГјkЙ™nib" || row.status === "Г‡atД±ЕџmazlД±q")) ||
    ...
    const matchesMinimum = !filters.belowMinimum || row.status === "AЕџaДџД± stok" || row.status === "Kritik stok" || row.status === "Stok tГјkЙ™nib" || row.status === "Г‡atД±ЕџmazlД±q";
...
function exportWarehouseBalanceCsv(rows, view) {
  const headers = ["Kateqoriya", "MЙ™hsul", "SKU", "Anbar", "QalД±q", "Minimum", "Rezerv", "MГ¶vcud", "SifariЕџdЙ™", "Vahid", "Maya", "Stok dЙ™yЙ™ri", "SatД±Еџ qiymЙ™ti", "Status"];
...
function exportDeliveryQueueCsv(rows) {
  const headers = ["SifariЕџ", "MГјqavilЙ™", "Kredit", "MГјЕџtЙ™ri", "FIN", "MЙ™hsul", "Miqdar", "Anbar", "Г–dЙ™niЕџ", "QalД±q", "Stok statusu", "Qeyd"];
  const escapeValue = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
...
const warehouseImportHeaderAliases = {
  product: ["mЙ™hsul", "mЙ™hsul adД±", "product", "name"],
  sku: ["sku", "kod", "code"],
  warehouse: ["anbar", "warehouse"],
  qty: ["miqdar", "qalД±q", "qty", "quantity"],
  salePrice: ["satД±Еџ qiymЙ™ti", "satД±Еџ", "sale price", "sale_price", "price"],
  costPrice: ["alД±Еџ qiymЙ™ti", "maya", "cost price", "cost_price"],
  category: ["kateqoriya", "category"],
  reorderLevel: ["minimum stok", "minimum", "reorder level", "reorder_level"],
  unit: ["Г¶lГ§Гј vahidi", "vahid", "unit"],
  serialTracked: ["serial izlЙ™mЙ™", "serial", "imei", "serial tracked"],
};
...
function parseWarehouseImportNumber(value) {
  const raw = String(value ?? "").trim().replace(/[в‚ј\s]/g, "");
  if (!raw) return null;
...
function parseWarehouseImportCsv(text, warehouses = []) {
  ...
  if (csvRows.length === 0) return { rows: [], errors: ["CSV faylД± boЕџdur."] };
  ...
  if (!hasProduct || !hasWarehouse || !hasQuantity) {
    return { rows: [], errors: ["CSV baЕџlД±ДџД±nda MЙ™hsul, Anbar vЙ™ Miqdar sГјtunlarД± olmalД±dД±r."] };
  }
  ...
  csvRows.slice(1).forEach((cells, index) => {
    ...
    if (!product) {
      errors.push(`SЙ™tir ${lineNumber}: mЙ™hsul adД± boЕџdur.`);
      return;
    }
    if (!warehouse) {
      errors.push(`SЙ™tir ${lineNumber}: anbar tapД±lmadД± (${warehouseInput || "boЕџ"}).`);
      return;
    }
    if (!Number.isFinite(quantity) || quantity <= 0 || !Number.isInteger(quantity)) {
      errors.push(`SЙ™tir ${lineNumber}: miqdar mГјsbЙ™t tam Й™dЙ™d olmalД±dД±r.`);
      return;
    }
    if ([salePrice, costPrice, reorderLevel].some((value) => value !== null && value < 0)) {
      errors.push(`SЙ™tir ${lineNumber}: qiymЙ™t vЙ™ minimum stok mЙ™nfi ola bilmЙ™z.`);
      return;
    }
...
function downloadWarehouseImportTemplate() {
  const headers = ["MЙ™hsul", "SKU", "Anbar", "Miqdar", "SatД±Еџ qiymЙ™ti", "AlД±Еџ qiymЙ™ti", "Kateqoriya", "Minimum stok", "Г–lГ§Гј vahidi", "Serial izlЙ™mЙ™"];
...
function WarehouseBalanceFilters({ filters, warehouses, categories, open, onChange, onApply, onClear }) {
  ...
  return (
    <section className="warehouse-balance-filters">
      <label>
        <span>MЙ™hsul / qrup</span>
        <div className="warehouse-filter-search">
          <Search size={16} />
          <input value={filters.productQuery} placeholder="MЙ™hsul adД±, SKU vЙ™ ya qrup" onChange={(event) => onChange("productQuery", event.target.value)} />
        </div>
      </label>
      <label>
        <span>Anbar</span>
        <select value={filters.warehouseId} onChange={(event) => onChange("warehouseId", event.target.value)}>
          <option value="all">BГјtГјn anbarlar</option>
      ...
      <label>
        ...
        <select value={filters.category} onChange={(event) => onChange("category", event.target.value)}>
          <option value="all">BГјtГјn kateqoriyalar</option>
          {categories.map((category) => <option key={category} value={category}>{category}</option>)}
        </select>
      </label>
      <label>
        <span>QalД±q statusu</span>
        <select value={filters.stockStatus} onChange={(event) => onChange("stockStatus", event.target.value)}>
          <option value="all">HamД±sД±</option>
          <option value="below">Minimumdan aЕџaДџД±</option>
          <option value="available">Stokda var</option>
          <option value="empty">Stok tГјkЙ™nib</option>
          <option value="shortage">Г‡atД±ЕџmazlД±q (backorder)</option>
        </select>
      </label>
      <label>
        ...
        <select value={filters.reserveStatus} onChange={(event) => onChange("reserveStatus", event.target.value)}>
          <option value="all">HamД±sД±</option>
          <option value="reserved">YalnД±z rezervli</option>
          <option value="free">Rezervsiz</option>
        </select>
      </label>
      <label>
        <span>Serial izlЙ™mЙ™</span>
      ...
      <label className="warehouse-minimum-toggle">
        ...
        <span>Minimumdan aЕџaДџД± olanlar</span>
      </label>
      <div className="warehouse-filter-actions">
        <button type="button" className="secondary-btn" onClick={onClear}>TЙ™mizlЙ™</button>
        <button type="button" className="primary-btn" onClick={onApply}>TЙ™tbiq et</button>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="warehouse-balance-table-wrap">
      <table className="warehouse-balance-table">
        <thead>
          <tr>
            <th>Kateqoriya</th><th>MЙ™hsul</th><th>SKU</th><th>{locationHeading}</th><th>QalД±q</th><th>Minimum</th><th>Rezerv</th><th>MГ¶vcud</th><th>SifariЕџdЙ™</th><th>Vahid</th><th>Maya</th><th>Stok dЙ™yЙ™ri</th><th>SatД±Еџ</th><th>Status</th><th>ЖЏmЙ™liyyat</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.key}>
              ...
              <td>{row.total}</td>
              <td>{row.reorderLevel || "вЂ”"}</td>
              <td>{row.reserved}</td>
              <td className={row.shortage > 0 || row.status !== "Normal" ? "balance-qty risk" : "balance-qty good"}>
                {row.shortage > 0 ? `-${row.shortage}` : row.available}
                {row.shortage > 0 && <small style={{ display: "block", opacity: 0.75 }}>sifariЕџ gГ¶zlЙ™yir</small>}
              </td>
              <td>{row.orderedQty > 0 ? <TwoLine title={`${row.orderedQty} Й™dЙ™d`} subtitle={row.latestPoId || `${row.openPoCount} PO`} /> : "вЂ”"}</td>
              <td>{row.unit}</td>
              ...
              <td>
                {view === "warehouses" ? (
                  <button className="text-btn" onClick={() => onSelectWarehouse(row.warehouseId)}>Anbara keГ§</button>
                ) : row.productId ? (
                  <button className="text-btn" onClick={() => onEditProduct(row.productId)}>RedaktЙ™</button>
                ) : (
                  <button className="text-btn" onClick={onCreateProduct}>Kataloqa Й™lavЙ™ et</button>
                )}
              </td>
            </tr>
          ))}
          {rows.length === 0 && <tr><td colSpan="15" className="warehouse-balance-empty">SeГ§ilmiЕџ filtrЙ™ uyДџun qalД±q tapД±lmadД±.</td></tr>}
        </tbody>
        {rows.length > 0 && (
          <tfoot>
            <tr>
              <td colSpan="4">CЙ™mi</td><td>{totals.total}</td><td>вЂ”</td><td>{totals.reserved}</td><td className="balance-qty good">{totals.available}</td><td>{totals.orderedQty || "вЂ”"}</td><td>вЂ”</td><td>вЂ”</td><td>{money(totals.stockValue)}</td><td>{money(totals.salesValue)}</td><td>вЂ”</td><td>вЂ”</td>
            </tr>
          </tfoot>
        )}
      </table>
...
export function WarehouseBalancesWorkspace({
  ...
  function showReplenishmentRows() {
    ...
    const replenishmentCount = balanceRows.filter((row) => row.status === "AЕџaДџД± stok" || row.status === "Kritik stok" || row.status === "Stok tГјkЙ™nib" || row.status === "Г‡atД±ЕџmazlД±q").length;
    setDraftFilters(next);
    setActiveFilters(next);
    setFiltersOpen(true);
    onTrackAction?.("Ehtiyat tamamlama siyahД±sД± aГ§Д±ldД±", `${replenishmentCount} mЙ™hsul/anbar sЙ™trinЙ™ minimum stok filtri tЙ™tbiq edildi`);
  }

  function handleExport() {
    exportWarehouseBalanceCsv(visibleRows, view);
    onTrackAction?.("Anbar qalД±qlarД± CSV ixrac edildi", `${visibleRows.length} sЙ™tir В· ${view === "products" ? "mЙ™hsullar ГјzrЙ™" : "anbarlar ГјzrЙ™"}`);
  }

  function handlePrint() {
    onTrackAction?.("Anbar qalД±qlarД± Г§ap ГјГ§Гјn aГ§Д±ldД±", `${visibleRows.length} sЙ™tir В· ${view === "products" ? "mЙ™hsullar ГјzrЙ™" : "anbarlar ГјzrЙ™"}`);
  ...
  return (
    <section className="warehouse-balance-workspace">
        ...
        <div className="warehouse-balance-actions">
          ...
          <div className="warehouse-action-menu">
            <button type="button" className="primary-btn" aria-expanded={actionMenuOpen} onClick={() => setActionMenuOpen((open) => !open)}>
              <ShoppingCart size={16} /> ЖЏmЙ™liyyatlar <ChevronRight size={15} />
            </button>
            {actionMenuOpen && (
              <div className="warehouse-action-menu-popover">
                <button type="button" onClick={() => { setActionMenuOpen(false); onReceiveStock(); }}><Plus size={15} /> MЙ™daxil et</button>
                <button type="button" onClick={() => { setActionMenuOpen(false); onOpenImport(); }}><Upload size={15} /> Toplu import</button>
                <button type="button" onClick={() => { setActionMenuOpen(false); onCreateProduct(); }}><Package size={15} /> MЙ™hsul yarat</button>
                <button type="button" onClick={() => { setActionMenuOpen(false); onOpenOperations(); }}><Warehouse size={15} /> Anbar idarЙ™etmЙ™si</button>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="warehouse-balance-table-meta">
        <span>{visibleRows.length} qalД±q sЙ™tri</span>
        <strong>{view === "products" ? "MЙ™hsullar ГјzrЙ™ cari qalД±q" : "Anbarlar ГјzrЙ™ cari qalД±q"}</strong>
      </div>
...
function DeliveriesPage({ orders, warehouseStock = {}, warehouses = [], onCompleteDelivery }) {
  const [deliveryFilter, setDeliveryFilter] = useState("HamД±sД±");
  ...
  const filterItems = [
    { label: "HamД±sД±", count: deliveryOrders.length },
    { label: "TЙ™hvilЙ™ hazД±r", count: readyOrders.length },
    { label: "Stok problemi", count: blockedOrders.length },
    { label: "Kredit", count: creditOrders.length },
    { label: "NaДџd", count: deliveryOrders.filter((order) => getOrderPaymentMethod(order) !== "Kredit").length },
  ];
  ...
  const visibleOrders = deliveryOrders.filter((order) => {
    ...
    const matchesFilter =
      deliveryFilter === "HamД±sД±" ||
      (deliveryFilter === "TЙ™hvilЙ™ hazД±r" && order.stockCheck.ok) ||
      (deliveryFilter === "Stok problemi" && !order.stockCheck.ok) ||
      (deliveryFilter === "Kredit" && getOrderPaymentMethod(order) === "Kredit") ||
      (deliveryFilter === "NaДџd" && getOrderPaymentMethod(order) !== "Kredit");
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="TЙ™hvil nГ¶vbЙ™si" value={deliveryOrders.length} trend={`${totalQty} Й™dЙ™d cihaz`} icon={Truck} tone="primary" />
        <MetricCard label="TЙ™hvilЙ™ hazД±r" value={readyOrders.length} trend="Rezerv vЙ™ stok uyДџundur" icon={Check} tone="success" />
        <MetricCard label="Stok problemi" value={blockedOrders.length} trend="BloklanmД±Еџ tЙ™hvil" icon={CircleAlert} tone={blockedOrders.length ? "danger" : "success"} />
        <MetricCard label="Orta gГ¶zlЙ™mЙ™" value={`${averageAge} gГјn`} trend={formatPaymentDate(parsePaymentDate(currentBusinessDate))} icon={CalendarClock} tone="info" />
      </section>

      <Panel className="delivery-control-panel">
        <PanelHeader
          title="TЙ™hvil verilmЙ™li mЙ™hsullar"
          subtitle="YalnД±z satД±Еџdan rezervЙ™ dГјЕџmГјЕџ vЙ™ anbardan Г§Д±xarД±lmalД± sifariЕџlЙ™r gГ¶rГјnГјr"
          icon={Truck}
        />
        <div className="delivery-filter-toolbar delivery-command-bar">
          ...
          <label className="delivery-search">
            ...
            <input
              ...
              placeholder="MГјЕџtЙ™ri, mГјqavilЙ™, cihaz..."
        ...
        <div className="delivery-control-grid">
          <div className="delivery-control-tile">
            ...
            <small>Son Й™mЙ™liyyat: TЙ™hvil verildi</small>
          </div>
          <div className="delivery-control-tile">
            <span>Kreditli sifariЕџ</span>
            <strong>{creditOrders.length}</strong>
            <small>HЙ™r satД±Еџ ayrД±ca borc kimi qalД±r</small>
          </div>
          <div className="delivery-control-tile">
            <span>RezervdЙ™ mЙ™hsul</span>
            <strong>{readyOrders.reduce((sum, order) => sum + Number(order.deliveryQty || 0), 0)}</strong>
            <small>TЙ™hvilЙ™ hazД±r Й™dЙ™d</small>
          </div>
          <div className="delivery-control-tile">
            <span>Filter nЙ™ticЙ™si</span>
            <strong>{visibleOrders.length}</strong>
            <small>{deliveryFilter} В· {warehouseFilter === "all" ? "BГјtГјn anbarlar" : "SeГ§ilmiЕџ anbar"}</small>
          </div>
        </div>
      </Panel>
      ...
      <section className="delivery-queue-layout">
        <Panel className="delivery-registry-panel delivery-registry-main">
          <PanelHeader title="TЙ™hvil reyestri" subtitle="SifariЕџЙ™ daxil olun, cihazlarД± yoxlayД±n vЙ™ anbardan Г§Д±xarД±n" />
          <DataTable
            columns={["SifariЕџ / mГјqavilЙ™", "MГјЕџtЙ™ri", "Cihazlar", "Anbar", "Г–dЙ™niЕџ", "Stok", "ЖЏmЙ™liyyat"]}
            rows={visibleOrders.map((order) => [
              <button
                className={`row-link ${selectedOrder?.id === order.id ? "active" : ""}`}
                onClick={() => setSelectedDeliveryId(order.id)}
              >
                <TwoLine title={order.id} subtitle={order.contractId || "MГјqavilЙ™ yoxdur"} />
              </button>,
              <TwoLine title={order.customer} subtitle={order.fin || order.phone || "MГјЕџtЙ™ri mЙ™lumatД±"} />,
              <OrderProductLines lines={order.productLines} />,
              <TwoLine title={order.warehouseName || "Anbar qeyd edilmЙ™yib"} subtitle={order.displayStage} />,
              <TwoLine
                title={order.paymentStatus || getOrderPaymentMethod(order)}
                subtitle={order.balance > 0 ? `${money(order.balance)} qalД±q` : "QalД±q yoxdur"}
              />,
              <TwoLine
                title={<StatusBadge status={order.stockCheck.status} />}
                subtitle={
                  order.stockCheck.partial
                    ? `${order.stockCheck.plan.deliverableTotal} Й™dЙ™d indi В· ${order.stockCheck.plan.shortageTotal} backorder`
                    : order.stockCheck.ok
                      ? `${order.stockCheck.plan?.remainingTotal ?? order.deliveryQty} Й™dЙ™d rezervdЙ™`
                      : order.stockCheck.reason
                }
              />,
              <button
              ...
              >
                {order.stockCheck.partial ? "QismЙ™n tЙ™hvil ver" : "TЙ™hvil verildi"}
              </button>,
        ...
        <Panel className="delivery-detail-panel">
          <PanelHeader title="TЙ™hvil kartД±" subtitle={selectedOrder?.id || "SifariЕџ seГ§ilmЙ™yib"} />
          {selectedOrder ? (
            <div className="delivery-detail-card">
              <div className="delivery-detail-head">
                <div>
                  <h3>{selectedOrder.customer}</h3>
                  <span>{selectedOrder.address || "Гњnvan qeyd edilmЙ™yib"}</span>
                </div>
              ...
              <div className="delivery-detail-grid">
                <div>
                  <span>MГјqavilЙ™</span>
                  <strong>{selectedOrder.contractId || "вЂ”"}</strong>
                ...
                <div>
                  ...
                  <small>{selectedOrder.balance > 0 ? `${money(selectedOrder.balance)} qalД±q` : "QalД±q yoxdur"}</small>
                </div>
                <div>
                  <span>GГ¶zlЙ™mЙ™</span>
                  <strong>{selectedOrder.ageDays} gГјn</strong>
                ...
                <div className="delivery-plan-lines">
                  {selectedOrder.stockCheck.plan.lines.map((line) => (
                    <div className="delivery-plan-line" key={line.product}>
                      ...
                      <small>
                        SifariЕџ {line.ordered} В· TЙ™hvil verilib {line.delivered} В· Д°ndi {line.deliverable}
                        {line.shortage > 0 ? ` В· Backorder ${line.shortage}` : ""}
                      </small>
                    </div>
                  ))}
                </div>
              ...
              <div className={`delivery-stock-check ${selectedOrder.stockCheck.ok ? "ok" : "danger"}`}>
                ...
                <b>{selectedOrder.stockCheck.plan?.remainingTotal ?? selectedOrder.deliveryQty} Й™dЙ™d</b>
              </div>
              <button
              ...
              >
                ...
                {selectedOrder.stockCheck.partial
                  ? `QismЙ™n tЙ™hvil ver (${selectedOrder.stockCheck.plan.deliverableTotal})`
                  : "TЙ™hvil verildi"}
              </button>

            </div>
          ) : (
            <EmptyState title="TЙ™hvil gГ¶zlЙ™yЙ™n sifariЕџ yoxdur" />
          )}
        </Panel>
      </section>

      <Panel className="delivery-history-panel">
        <PanelHeader title="Son tЙ™hvil tarixГ§Й™si" subtitle="Tamamlanan sifariЕџlЙ™r yalnД±z izlЙ™mЙ™ ГјГ§Гјn gГ¶stЙ™rilir" />
        <DataTable
          columns={["SifariЕџ", "MГјЕџtЙ™ri", "Cihaz", "Anbar", "TЙ™hvil tarixi"]}
          rows={completedOrders.map((order) => [
            <strong>{order.id}</strong>,
            <TwoLine title={order.customer} subtitle={order.fin} />,
            summarizeOrderProducts(order),
            order.warehouseName || "вЂ”",
            order.deliveredAt || "вЂ”",
          ])}
        />
      </Panel>
    </div>
...
function InvoicesPage({ invoices, summary, invoiceSettings = {}, onExport, onOpenSalesOrder }) {
  const [invoiceFilter, setInvoiceFilter] = useState("HamД±sД±");
  ...
  const exportInvoices = () => {
    const escapeCsv = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
    const rows = [
      ["Faktura", "Sifaris", "Musteri", "FIN", "Mehsullar", "Net", "EDV", "Cemi", "Qaliq", "E-qaimЙ™"],
  ...
  const filterItems = [
    { label: "HamД±sД±", count: invoices.length },
    { label: "Г–dЙ™nilib", count: invoices.filter((invoice) => invoice.status === "Г–dЙ™nilib").length },
    {
      label: "QismЙ™n Г¶dЙ™nilib",
      count: invoices.filter((invoice) => invoice.status === "QismЙ™n Г¶dЙ™nilib").length,
    },
    {
      label: "Г–dЙ™niЕџ gГ¶zlЙ™yir",
      count: invoices.filter((invoice) => invoice.status === "Г–dЙ™niЕџ gГ¶zlЙ™yir").length,
    },
    {
      label: "GГ¶ndЙ™riЕџЙ™ hazД±r",
      count: invoices.filter((invoice) => invoice.eTaxStatus === "GГ¶ndЙ™riЕџЙ™ hazД±r").length,
    },
  ];
  const visibleInvoices = invoices.filter((invoice) => {
    if (invoiceFilter === "HamД±sД±") return true;
    if (invoiceFilter === "GГ¶ndЙ™riЕџЙ™ hazД±r") return invoice.eTaxStatus === invoiceFilter;
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="Faktura sayД±" value={summary.count} icon={FileText} tone="primary" />
        <MetricCard label="Гњmumi mЙ™blЙ™Дџ" value={money(summary.total)} icon={Wallet} tone="success" />
        <MetricCard label="ЖЏDV" value={money(summary.vat)} icon={BarChart3} tone="info" />
        <MetricCard label="AГ§Д±q qalД±q" value={money(summary.balance)} icon={CircleAlert} tone="warning" />
      </section>

      <Panel className="invoice-control-panel">
        <PanelHeader
          title="E-qaimЙ™ idarЙ™ paneli"
          subtitle="SatД±Еџ sifariЕџlЙ™rindЙ™n avtomatik formalaЕџan faktura vЙ™ ЖЏDV bГ¶lgГјsГј"
          icon={FileText}
        />
        <div className="finance-control-grid">
          <div className="finance-control-tile">
            ...
            <small>{invoiceSettings.eTaxMode || "E-qaimЙ™ inteqrasiya rejimi"}</small>
          </div>
          <div className="finance-control-tile">
            <span>ЖЏDV dЙ™rЙ™cЙ™si</span>
            <strong>{invoiceSettings.vatRate || 18}%</strong>
            <small>SatД±Еџ mЙ™blЙ™ДџindЙ™n ayrД±lД±r</small>
          </div>
          <div className="finance-control-tile">
            <span>GГ¶ndЙ™riЕџЙ™ hazД±r</span>
            <strong>{summary.ready}</strong>
            <small>E-tax nГ¶vbЙ™si</small>
          </div>
          <div className="finance-control-tile">
            <span>Г–dЙ™nilib</span>
            <strong>{money(summary.paid)}</strong>
            <small>Kassaya dГјЕџЙ™n fakturalar</small>
          </div>
        </div>
      </Panel>
      ...
      <Panel className="invoice-operations-panel">
        <PanelHeader
          title="Faktura Й™mЙ™liyyat nЙ™zarЙ™ti"
          subtitle="Г–dЙ™niЕџ vaxtД±, gecikmЙ™ vЙ™ e-qaimЙ™ gГ¶ndЙ™riЕџ statusu bir yerdЙ™ izlЙ™nir"
          icon={CalendarClock}
        />
        <div className="invoice-control-grid">
          <div>
            <span>AГ§Д±q qalД±q</span>
          ...
          <div>
            <span>7 gГјnЙ™ qЙ™dЙ™r</span>
            <strong>{money(invoiceControl.dueSoonBalance)}</strong>
            <small>{invoiceControl.dueSoonCount} yaxД±n Г¶dЙ™niЕџ</small>
          </div>
          <div>
            <span>GecikЙ™n</span>
          ...
          <div>
            ...
            <small>{invoiceControl.ready} gГ¶ndЙ™riЕџЙ™ hazД±r</small>
          </div>
        </div>
        <DataTable
          columns={["YaЕџlanma", "Faktura", "QalД±q", "Гњmumi", "Pay"]}
      ...
      <Panel className="invoice-registry-panel">
        ...
        <DataTable
          columns={["Faktura", "SifariЕџ", "MГјЕџtЙ™ri", "MЙ™hsul", "Net", "ЖЏDV", "CЙ™mi", "QalД±q", "E-qaimЙ™", "ЖЏmЙ™liyyat"]}
          rows={visibleInvoices.map((invoice) => [
            ...
            <TwoLine title={invoice.customer} subtitle={`FД°N ${invoice.fin}`} />,
            invoice.products,
...
function InvoicePrintModal({ invoice, invoiceSettings = {}, onClose }) {
  function downloadHtml() {
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${invoice.id}</title></head><body><h1>${invoice.id}</h1><p>${invoice.customer}</p><p>${invoice.products}</p><p>CЙ™mi: ${money(invoice.totalAmount)}</p><p>ЖЏDV: ${money(invoice.vatAmount)}</p></body></html>`;
    ...
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card invoice-print-card">
        <div className="modal-head">
          <div>
            <h2>{invoice.id}</h2>
            <p>Faktura/e-qaimЙ™ Г§ap gГ¶rГјnГјЕџГј vЙ™ HTML/PDF export hazД±rlД±ДџД±.</p>
          </div>
        ...
        <section className="invoice-paper">
          <div className="invoice-paper-head">
            <div>
              ...
              <small>VГ–EN {invoice.voen}</small>
            </div>
            <div>
              <span>Faktura</span>
              <strong>{invoice.id}</strong>
              <small>{invoice.date} В· {invoice.currency}</small>
            </div>
          </div>
          <div className="invoice-paper-grid">
            <TwoLine title="AlД±cД±" subtitle={`${invoice.customer} В· FД°N ${invoice.fin}`} />
            <TwoLine title="SifariЕџ/MГјqavilЙ™" subtitle={`${invoice.orderId} В· ${invoice.contractId}`} />
            <TwoLine title="Г–dЙ™niЕџ tipi" subtitle={invoice.paymentMethod} />
            <TwoLine title="Son tarix" subtitle={invoice.dueDate} />
          </div>
          <div className="invoice-product-box">
            <span>MЙ™hsul/XidmЙ™t</span>
            <strong>{invoice.products}</strong>
          </div>
          <div className="invoice-total-grid">
            <TwoLine title="Net mЙ™blЙ™Дџ" subtitle={money(invoice.netAmount)} />
            <TwoLine title={`ЖЏDV ${invoiceSettings.vatRate || 18}%`} subtitle={money(invoice.vatAmount)} />
            <TwoLine title="CЙ™mi" subtitle={money(invoice.totalAmount)} />
            <TwoLine title="QalД±q" subtitle={invoice.balance > 0 ? money(invoice.balance) : "Yoxdur"} />
          </div>
          <StatusBadge status={invoice.eTaxStatus} />
        </section>
...
function ContractPrintModal({ contract, settings = {}, onClose }) {
  function downloadDocument() {
    const content = `<!doctype html><html><head><meta charset="utf-8"><title>${contract.id}</title></head><body><h1>${settings.company || "ERP+CRM AZ"}</h1><h2>SatД±Еџ mГјqavilЙ™si ${contract.id}</h2><p><strong>MГјЕџtЙ™ri:</strong> ${contract.customer}</p><p><strong>FД°N:</strong> ${contract.fin || "вЂ”"}</p><p><strong>MЙ™hsul:</strong> ${contract.product}</p><p><strong>MЙ™blЙ™Дџ:</strong> ${money(contract.amount)}</p><p><strong>Status:</strong> ${contract.status}</p><p>Bu sЙ™nЙ™d ERP+CRM AZ sistemindЙ™ formalaЕџdД±rД±lmД±ЕџdД±r.</p></body></html>`;
    ...
    <div className="modal-shell print-modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card invoice-print-card">
        <div className="modal-head no-print">
          <div>
            <h2>MГјqavilЙ™ sЙ™nЙ™di</h2>
            <p>PDF ГјГ§Гјn Г§ap dialoqundan вЂњSave as PDFвЂќ seГ§in vЙ™ ya Word sЙ™nЙ™dini endirin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <article className="invoice-paper">
          <div className="invoice-paper-head">
            <div>
              <strong>{settings.company || "ERP+CRM AZ"}</strong>
              <span>{settings.voen ? `VГ–EN: ${settings.voen}` : ""}</span>
            </div>
            <div><strong>MГњQAVД°LЖЏ</strong><span>{contract.id}</span></div>
          </div>
          <div className="invoice-meta-grid">
            <TwoLine title="MГјЕџtЙ™ri" subtitle={contract.customer} />
            <TwoLine title="FД°N" subtitle={contract.fin || "вЂ”"} />
            <TwoLine title="MЙ™hsul" subtitle={contract.product} />
            <TwoLine title="MЙ™blЙ™Дџ" subtitle={money(contract.amount)} />
          </div>
          <p className="contract-body-copy">TЙ™rЙ™flЙ™r mЙ™hsulun tЙ™hvil verilmЙ™si, Г¶dЙ™niЕџ vЙ™ zЙ™manЙ™t ЕџЙ™rtlЙ™rinin bu mГјqavilЙ™ ГјzrЙ™ tЙ™tbiq olunduДџunu tЙ™sdiq edir.</p>
          <div className="contract-signatures"><span>SatД±cД± imzasД±</span><span>MГјЕџtЙ™ri imzasД±</span></div>
        </article>
        <div className="modal-actions no-print">
          <button className="secondary-btn" onClick={downloadDocument}><Download size={16} /> Word sЙ™nЙ™di</button>
...
function AccountingPage({ accounting, closeRun }) {
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="AktivlЙ™r" value={money(balance.assets)} icon={Wallet} tone="primary" />
        <MetricCard label="Г–hdЙ™liklЙ™r" value={money(balance.liabilities)} icon={CircleAlert} tone="warning" />
        <MetricCard label="Kapital" value={money(balance.equity)} icon={ShieldCheck} tone="success" />
        <MetricCard label="Net mЙ™nfЙ™Й™t" value={money(pl.netProfit)} trend={percent(pl.margin)} icon={TrendingUp} tone="info" />
      </section>

      <Panel className="accounting-close-panel" data-testid="accounting-close-readiness">
        <PanelHeader
          title="Ay baДџlanД±ЕџД± nЙ™zarЙ™ti"
          subtitle="Balans, kassa, jurnal vЙ™ ЖЏDV sЙ™tirlЙ™ri real Й™mЙ™liyyat datasД±ndan yoxlanД±r"
          icon={ShieldCheck}
        />
        <div className="accounting-close-summary">
          <div>
            <span>HazД±r maddЙ™lЙ™r</span>
            <strong>{closeControl.readyCount}/{closeControl.checks.length}</strong>
            <small>{closeControl.warningCount} yoxlama gГ¶zlЙ™yir</small>
          </div>
          <div>
            <span>Balans fЙ™rqi</span>
            <strong>{money(closeControl.equationDiff)}</strong>
            <small>Aktiv - Г¶hdЙ™lik - kapital</small>
          </div>
          <div>
            <span>Kassa fЙ™rqi</span>
            <strong>{money(closeControl.cashDiff)}</strong>
            <small>1010 hesabД± vЙ™ cash-flow</small>
          </div>
          <div>
            <span>BГ¶lГјЕџdГјrГјlmЙ™miЕџ mЙ™nfЙ™Й™t</span>
            <strong>{money(closeControl.retainedEarnings)}</strong>
            <small>Net nЙ™ticЙ™</small>
          </div>
        </div>
        <DataTable
      ...
      {closeRun && (
        <Panel className="module-action-panel">
          <PanelHeader title="Son jurnal exportu" subtitle="MГјhasibat baДџlanД±ЕџД± ГјГ§Гјn hazД±rlanan son Й™mЙ™liyyat" icon={FileText} />
          <div className="db-status-grid">
            ...
            <div>
              <span>Export vaxtД±</span>
              <strong>{closeRun.exportedAt}</strong>
            </div>
            <div>
              <span>Jurnal sЙ™tri</span>
              <strong>{closeRun.journalCount}</strong>
            </div>
            <div>
              <span>Net mЙ™nfЙ™Й™t</span>
      ...
      <section className="accounting-statement-grid">
        <Panel>
          <PanelHeader title="Balans" subtitle="Aktiv, Г¶hdЙ™lik vЙ™ kapitalД±n qД±sa gГ¶rГјnГјЕџГј" icon={BarChart3} />
          <div className="statement-list">
            <TwoLine title="AktivlЙ™r" subtitle={money(balance.assets)} />
            <TwoLine title="Г–hdЙ™liklЙ™r" subtitle={money(balance.liabilities)} />
            <TwoLine title="Kapital" subtitle={money(balance.equity)} />
          </div>
        </Panel>
        <Panel>
          <PanelHeader title="P&L" subtitle="SatД±Еџ, maya, xЙ™rc vЙ™ mЙ™nfЙ™Й™t" icon={TrendingUp} />
          <div className="statement-list">
            <TwoLine title="SatД±Еџ gЙ™liri" subtitle={money(pl.revenue)} />
            <TwoLine title="Maya dЙ™yЙ™ri" subtitle={money(pl.costOfGoods)} />
            <TwoLine title="ЖЏmЙ™liyyat xЙ™rci" subtitle={money(pl.operatingExpenses)} />
            <TwoLine title="Net mЙ™nfЙ™Й™t" subtitle={money(pl.netProfit)} />
          </div>
        </Panel>
        <Panel>
          <PanelHeader title="Cash flow" subtitle="AГ§Д±lД±Еџ, daxilolma, Г§Д±xД±Еџ vЙ™ baДџlanД±Еџ" icon={Wallet} />
          <div className="statement-list">
            <TwoLine title="AГ§Д±lД±Еџ" subtitle={money(cashFlow.opening)} />
            <TwoLine title="Daxilolma" subtitle={money(cashFlow.inflow)} />
            <TwoLine title="Г‡Д±xД±Еџ" subtitle={money(cashFlow.outflow)} />
            <TwoLine title="BaДџlanД±Еџ" subtitle={money(cashFlow.closing)} />
          </div>
        </Panel>
      </section>

      <Panel>
        <PanelHeader title="Hesablar planД±" subtitle="IFRS mЙ™ntiqinЙ™ yaxД±nlaЕџdД±rД±lmД±Еџ Й™mЙ™liyyat hesab qalД±qlarД±" />
        <DataTable
          columns={["Kod", "Hesab", "Tip", "Debet", "Kredit", "QalД±q"]}
          rows={chartRows.map((row) => [
            <strong>{row.code}</strong>,
            row.account,
            <StatusBadge status={row.type} />,
            row.debit > 0 ? money(row.debit) : "вЂ”",
            row.credit > 0 ? money(row.credit) : "вЂ”",
      ...
      <Panel>
        <PanelHeader title="Jurnal yazД±lД±ЕџlarД±" subtitle="SatД±Еџ, kredit kassasД± vЙ™ xЙ™rc Й™mЙ™liyyatlarД±nД±n ikili yazД±lД±ЕџД±" />
        <DataTable
          columns={["Tarix", "MЙ™nbЙ™", "Debet", "Kredit", "MЙ™blЙ™Дџ", "Status"]}
...
function TaxPage({ taxRows, payrollTaxRows, invoiceSummary, accounting }) {
  ...
  const soon = taxRows.filter((row) => row.status === "Bu gГјn" || row.status === "YaxД±nlaЕџД±r");
  const paidTasks = taxRows.filter((row) => row.paymentTaskId || row.paymentStatus === "Г–dЙ™niЕџ tapЕџД±rД±ДџД±");
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="YaxД±n Г¶hdЙ™lik" value={soon.length} icon={CalendarClock} tone="warning" />
        <MetricCard label="GecikЙ™n" value={overdue.length} icon={CircleAlert} tone={overdue.length ? "danger" : "success"} />
        <MetricCard label="ЖЏDV bazasД±" value={money(invoiceSummary.vat)} icon={FileText} tone="info" />
      ...
      <Panel className="tax-control-panel" data-testid="tax-control-panel">
        <PanelHeader
          title="Vergi Г¶hdЙ™lik nЙ™zarЙ™ti"
          subtitle="ЖЏDV, mЙ™nfЙ™Й™t vЙ™ payroll Г¶hdЙ™liklЙ™ri maliyyЙ™ vЙ™ faktura datalarД±ndan avtomatik hesablanД±r"
          icon={CalendarClock}
        />
        <div className="tax-control-grid">
          <div>
            <span>Toplam Г¶hdЙ™lik</span>
            <strong>{money(totalTaxAmount)}</strong>
            <small>{taxRows.length} nЙ™zarЙ™t sЙ™tri</small>
          </div>
          <div>
            <span>YaxД±n Г¶dЙ™niЕџ</span>
            <strong>{money(soonAmount)}</strong>
            <small>{soon.length} sЙ™tir prioritetdir</small>
          </div>
          <div>
            <span>GecikmiЕџ risk</span>
            <strong>{money(overdueAmount)}</strong>
            <small>{overdue.length} gecikЙ™n Г¶hdЙ™lik</small>
          </div>
          <div>
            <span>Г–dЙ™niЕџ tapЕџД±rД±ДџД±</span>
            <strong>{paidTasks.length}</strong>
            <small>{autoRows.length} avtomatik hesablanД±b</small>
          </div>
        </div>
      </Panel>

      <Panel className="tax-calendar-panel">
        <PanelHeader title={`${currentBusinessYear} vergi tЙ™qvimi`} subtitle={`BugГјnkГј nЙ™zarЙ™t tarixi: ${formatPaymentDate(parsePaymentDate(currentBusinessDate))}`} icon={CalendarClock} />
        <DataTable
          columns={["Г–hdЙ™lik", "DГ¶vr", "Son tarix", "Qalan gГјn", "TЙ™xmini mЙ™blЙ™Дџ", "MЙ™sul", "MЙ™nbЙ™", "Status"]}
          rows={taxRows.map((row) => [
            <TwoLine title={row.title} subtitle={row.type} />,
            row.period,
            row.dueDate,
            row.daysLeft >= 0 ? `${row.daysLeft} gГјn` : `${Math.abs(row.daysLeft)} gГјn gecikib`,
            money(row.amount),
            row.owner,
            row.paymentTaskId ? <TwoLine title="TapЕџД±rД±q yaradД±lД±b" subtitle={row.paymentTaskId} /> : row.source || "Manual",
            <StatusBadge status={row.status} />,
          ])}
        />
      </Panel>
      ...
      <section className="accounting-statement-grid">
        <Panel>
          <PanelHeader title="ЖЏDV icmalД±" subtitle="Faktura modulundan gЙ™lЙ™n ЖЏDV hesabД±" />
          <div className="statement-list">
            <TwoLine title="SatД±Еџ ЖЏDV" subtitle={money(invoiceSummary.vat)} />
            <TwoLine title="ЖЏDV Г¶hdЙ™liyi" subtitle={money(accounting.vatPayable)} />
            <TwoLine title="E-qaimЙ™ sayД±" subtitle={`${invoiceSummary.count} faktura`} />
          </div>
        </Panel>
        <Panel>
          <PanelHeader title="ЖЏmЙ™k haqqД± kalkulyatoru" subtitle={`${currentBusinessYear} vergi, DSMF vЙ™ iЕџЙ™gГ¶tГјrЙ™n xЙ™rci`} />
          <div className="statement-list">
            <TwoLine title="Д°ЕџЙ™gГ¶tГјrЙ™n xЙ™rci" subtitle={money(employerCost)} />
            <TwoLine title="Tutulmalar" subtitle={money(payrollLiability)} />
            <TwoLine title="ЖЏmЙ™kdaЕџ sayД±" subtitle={payrollTaxRows.length} />
          </div>
        </Panel>
      </section>

      <Panel>
        <PanelHeader title="Payroll vergi hesabД±" subtitle="Gross, gЙ™lir vergisi, sosial Г¶dЙ™niЕџ vЙ™ net Й™mЙ™k haqqД±" />
        <DataTable
          columns={["ЖЏmЙ™kdaЕџ", "ЕћГ¶bЙ™", "Gross", "GЙ™lir vergisi", "Д°ЕџГ§i DSMF/iЕџsizlik", "Net", "Д°ЕџЙ™gГ¶tГјrЙ™n xЙ™rci"]}
...
function ApiPage({
  ...
  const failedLogs = logs.filter((log) => log.result !== "UДџurlu");
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        ...
        <MetricCard label="GГ¶ndЙ™riЕџ nГ¶vbЙ™si" value={queueTotal} icon={Bell} tone="warning" />
        <MetricCard label="Retry nГ¶vbЙ™si" value={retryTotal} trend={lastLog ? `${lastLog.responseCode} В· ${lastLog.result}` : "Test gГ¶zlЙ™yir"} icon={RefreshCw} tone={retryTotal ? "warning" : "info"} />
      </section>

      <Panel className="api-console-panel" data-testid="api-console-panel">
        <PanelHeader
          title="API konsolu"
          subtitle="Endpoint health, test nЙ™ticЙ™lЙ™ri, retry vЙ™ secret idarЙ™etmЙ™si"
          icon={ShieldCheck}
        />
        <div className="db-status-grid">
          <div>
          ...
          <div>
            <span>Son Й™mЙ™liyyat</span>
            <strong>{dbMeta.lastAction || "вЂ”"}</strong>
          </div>
          <div>
            <span>Versiya</span>
        ...
        <div className="api-action-row">
          ...
          <span>{lastLog ? `${lastLog.webhookId} В· ${lastLog.at}` : "HЙ™lЙ™ test nЙ™ticЙ™si yoxdur"}</span>
        </div>
      </Panel>

      <Panel className="api-endpoint-panel" data-testid="api-endpoint-panel">
        <PanelHeader title="Endpoint xЙ™ritЙ™si" subtitle="Modul hadisЙ™lЙ™rinin gГ¶ndЙ™rildiyi real inteqrasiya endpoint-lЙ™ri" />
        <DataTable
          columns={["Endpoint", "Event", "URL", "Auth", "SLA", "NГ¶vbЙ™", "Retry", "Status"]}
          rows={webhooks.map((webhook) => [
            <TwoLine title={webhook.name} subtitle={`${webhook.method || "POST"} В· ${webhook.id}`} />,
      ...
      <section className="api-ops-grid">
        <Panel className="api-secret-panel" data-testid="api-secret-panel">
          <PanelHeader title="Token / Secret vault" subtitle="MaskalanmД±Еџ tokenlЙ™r, istifadЙ™ vЙ™ rotasiya nЙ™zarЙ™ti" icon={Database} />
          <DataTable
            columns={["Secret", "Maska", "BaДџlД± endpoint", "Rotasiya", "Status", "ЖЏmЙ™liyyat"]}
            rows={secrets.map((secret) => [
              <TwoLine title={secret.label} subtitle={`${secret.key} В· v${secret.version || 1}`} />,
              secret.maskedValue,
              <TwoLine title={`${secret.linkedCount} endpoint`} subtitle={secret.linkedEvents} />,
              <TwoLine title={`${secret.daysLeft} gГјn`} subtitle={secret.lastRotatedAt || "Tarix yoxdur"} />,
        ...
        <Panel className="api-testlog-panel" data-testid="api-testlog-panel">
          <PanelHeader title="Test nЙ™ticЙ™lЙ™ri vЙ™ retry logu" subtitle={`${logs.length} log В· ${failedLogs.length} uДџursuz cЙ™hd`} icon={FileText} />
          <DataTable
            ...
            rows={logs.slice(0, 8).map((log) => [
              ...
              log.mode,
              <TwoLine title={`${log.responseCode} В· ${log.result}`} subtitle={log.error || log.target} />,
              `${log.latencyMs} ms`,
              <TwoLine title={String(log.retryQueue || 0)} subtitle={log.nextRetryAt || "вЂ”"} />,
              log.at,
            ])}
          />
        </Panel>
      ...
      <Panel className="api-webhook-rule-panel">
        <PanelHeader title="Webhook qaydalarД±" subtitle="HadisЙ™, son payload vЙ™ son test nЙ™ticЙ™si" />
        <DataTable
          columns={["Qayda", "Son payload", "Son test", "Cavab", "MЙ™sul", "Retry vЙ™ziyyЙ™ti"]}
          rows={webhooks.map((webhook) => [
            <TwoLine title={webhook.name} subtitle={webhook.id} />,
            webhook.lastPayload,
            webhook.lastTestAt || "вЂ”",
            <TwoLine title={webhook.lastResponseCode} subtitle={webhook.lastLatencyMs ? `${webhook.lastLatencyMs} ms` : "вЂ”"} />,
            webhook.owner,
...
function CreditListRow({ item, active, onSelect }) {
  ...
  const statusText = paymentState.isOverdue
    ? `${paymentState.daysOverdue} gГјn gecikib`
  ...
  return (
    ...
    >
      <div className="credit-list-main">
        ...
        <span>
          {credit.id} В· {credit.contractId || "MГјqavilЙ™siz"}
        </span>
      </div>
      ...
      <div className="credit-list-extra">
        ...
        <strong>{money(plan.balance)} qalД±q</strong>
      </div>
    ...
    <div className="modal-shell credit-detail-modal-shell" role="dialog" aria-modal="true" aria-labelledby="credit-detail-modal-title">
      <div className="modal-card credit-detail-modal-card">
        <div className="modal-head credit-detail-modal-head">
          <div>
            <h2 id="credit-detail-modal-title">Kredit mГјqavilЙ™si</h2>
            <p>{credit.customer} ГјzrЙ™ fЙ™rdi mГјqavilЙ™, cihaz, Г¶dЙ™niЕџ vЙ™ tarixГ§Й™ mЙ™lumatlarД±</p>
            <div className="credit-detail-title-meta">
              <span>{credit.id}</span>
              <span>{credit.contractId || "MГјqavilЙ™siz"}</span>
              <span>{credit.device || credit.product || "Cihaz qeyd edilmЙ™yib"}</span>
            </div>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
      ...
      <div className="credit-detail-layout">
        <section className="credit-detail-primary">
          <div className="credit-detail-head">
            ...
            <StatusBadge status={paymentState.isOverdue ? `${paymentState.daysOverdue} gГјn gecikib` : credit.status} />
          </div>
          ...
          <div className="credit-detail-values">
            <TwoLine title="Д°lkin mГјqavilЙ™" subtitle={money(debt.total)} />
            <TwoLine title="Д°lkin Г¶dЙ™niЕџ" subtitle={money(plan.initialPayment)} />
            <TwoLine title="Qalan ay" subtitle={`${debt.remainingMonths} ay`} />
            <TwoLine title="MГјddЙ™t" subtitle={`${plan.months} ay`} />
          </div>
          <div className="credit-plan-card">
            <div className="credit-plan-note">
              <span>
                {plan.months > 1 ? `${plan.months - 1} ay` : "AylД±q"} <strong>{money(plan.monthly)}</strong>
              </span>
              <span>
          ...
          <div className="credit-detail-records">
            ...
            <div className="credit-schedule-edit-block">
              <div className="credit-schedule-head">
                <div>
                  <h3>Г–dЙ™niЕџ tarixlЙ™ri</h3>
                  <p>HЙ™lЙ™lik tarix redaktЙ™si bГјtГјn istifadЙ™Г§ilЙ™r ГјГ§Гјn aГ§Д±qdД±r.</p>
                </div>
              </div>
              <CreditSchedule
        ...
        <aside className="credit-detail-aside">
          ...
          <div className="credit-detail-actions">
            <span>NГ¶vbЙ™ti: {paymentState.nextInstallment?.due || credit.next}</span>
  ...
  return (
    <div className="credit-debt-formula" data-testid="credit-debt-formula">
      ...
      <small className={formulaIsBalanced ? "ok" : "warning"}>
        {formulaIsBalanced ? "Borcla Г¶dЙ™niЕџ balansД± uyДџundur" : "Balans yenidЙ™n yoxlanmalД±dД±r"}
      </small>
    </div>
  );
}
  ...
  return (
    <div className="credit-health-grid">
      <div>
        ...
        <small>{Math.round(progress)}% tamamlanД±b</small>
      </div>
      <div>
        <span>QalД±q borc</span>
        <strong>{money(plan.balance)}</strong>
        <small>{plan.months} aylД±q plan</small>
      </div>
      <div className={paymentState.isOverdue ? "danger" : paymentState.isDueToday ? "info" : ""}>
        <span>YД±ДџД±m statusu</span>
      ...
      <div>
        ...
        <small>{credit.orderId ? `${credit.orderId} sifariЕџi` : "Manual qeyd"}</small>
      </div>
    </div>
  );
}
...
function CreditContext({ credit, onOpenSalesOrder }) {
  return (
    <div className="credit-context-grid">
      <div>
        <span>MГјqavilЙ™</span>
        <strong>{credit.contractId || "MГјqavilЙ™ qeyd edilmЙ™yib"}</strong>
      </div>
      <div>
        <span>Cihaz</span>
        <strong>{credit.device || credit.product || "Cihaz qeyd edilmЙ™yib"}</strong>
      </div>
      {credit.orderId && (
        <div>
          <span>SifariЕџ</span>
          <button
            ...
            title="BaДџlД± sifariЕџ detalД±na keГ§"
          >
            {credit.orderId}
          </button>
        </div>
...
function CreditPaymentAlert({ paymentState }) {
  ...
  const due = paymentState.nextInstallment?.due || "вЂ”";
  const label = paymentState.isOverdue
    ? `${paymentState.daysOverdue} gГјn gecikib`
    : paymentState.isDueToday
      ? "Bu gГјn Г¶dЙ™nilmЙ™lidir"
      : "NГ¶vbЙ™ti Г¶dЙ™niЕџ";
  ...
  return (
    <form className="credit-payment-form" onSubmit={submit}>
      <div className="credit-payment-form-head">
        <div>
          <h3>Г–dЙ™niЕџ qЙ™bul et</h3>
          <p>ЖЏsas mЙ™blЙ™Дџ borcdan silinir, gecikmЙ™ faizi yalnД±z kassaya daxil olur.</p>
        </div>
      </div>
      <div className="credit-payment-inputs">
        <label>
          <span>ЖЏsas mЙ™blЙ™Дџ</span>
          <input
            aria-label="ЖЏsas mЙ™blЙ™Дџ"
            type="number"
            min="0"
        ...
        <label>
          <span>GecikmЙ™ faizi</span>
          <input
            aria-label="GecikmЙ™ faizi"
            type="number"
            min="0"
            value={penaltyAmount}
      ...
      <div className="credit-payment-preview">
        ...
        <span>
          GecikmЙ™ gЙ™liri <strong>{money(penalty)}</strong>
        </span>
        <span>
        ...
        {extraPrincipal > 0 && (
          <span className="success">
            NГ¶vbЙ™ti aydan azalД±r <strong>{money(extraPrincipal)}</strong>
          </span>
        )}
      </div>
      <button type="submit" className="primary-btn">
        Г–dЙ™niЕџi qЙ™bul et
      </button>
    </form>
  );
}
  ...
  return (
    <div className="credit-payment-history">
      <div className="credit-history-head">
        <div>
          <h3>Г–dЙ™niЕџ tarixГ§Й™si</h3>
          <p>ЖЏsas mЙ™blЙ™Дџ borcdan silinir, gecikmЙ™ faizi yalnД±z kassaya gЙ™lir.</p>
        </div>
        <span>{rows.length} Й™mЙ™liyyat</span>
      </div>
      {rows.length === 0 ? (
        <div className="credit-history-empty">Bu kredit ГјzrЙ™ hЙ™lЙ™ Г¶dЙ™niЕџ qЙ™bul edilmЙ™yib.</div>
      ) : (
            ...
            return (
              <div className="credit-payment-row" key={`${payment.date}-${index}`}>
                <div>
                  ...
                  <span>
                    ЖЏsas {money(principal)} В· GecikmЙ™ {money(penalty)}
                  </span>
                  {extraApplied > 0 && <em>NГ¶vbЙ™ti aylardan azaldД±ldД±: {money(extraApplied)}</em>}
                </div>
                <b>{money(cashIn)}</b>
              </div>
            );
...
function CreditSchedule({ installments, onUpdatePaymentDate }) {
  ...
  return (
    <div className="credit-schedule" aria-label="Kredit Г¶dЙ™niЕџ tarixlЙ™ri">
      {installments.map((installment) => {
        const amount = Number(installment.amount || 0);
        const status = amount <= 0 ? "BaДџlanД±b" : installment.month === firstOpenMonth ? "Cari ay" : "GГ¶zlЙ™yir";
        ...
        return (
          <label key={installment.month} className={amount <= 0 ? "closed" : installment.month === firstOpenMonth ? "current" : ""}>
            ...
            <small>{status}</small>
            <input
              aria-label={`${installment.month}. ay Г¶dЙ™niЕџ tarixi`}
              type="date"
...
function ReceivablesPage({ rows, syncMeta, closures = [], onCloseDebt }) {
  const [typeFilter, setTypeFilter] = useState("HamД±sД±");
  const [sourceTypeFilter, setSourceTypeFilter] = useState("HamД±sД±");
  const [riskFilter, setRiskFilter] = useState("HamД±sД±");
  const [collectionFilter, setCollectionFilter] = useState("HamД±sД±");
  const [agingFilter, setAgingFilter] = useState("HamД±sД±");
  ...
  const highRiskRows = rows.filter((row) => ["Kritik", "YГјksЙ™k"].includes(row.riskCategory));
  ...
  const sourceTypeOptions = ["HamД±sД±", ...new Set(rows.map((row) => row.sourceTypeLabel || row.sourceType).filter(Boolean))];
  const riskOptions = ["HamД±sД±", ...new Set(rows.map((row) => row.riskCategory).filter(Boolean))];
  const collectionOptions = ["HamД±sД±", ...new Set(rows.map((row) => row.collectionStatus).filter(Boolean))];
  const agingOptions = ["HamД±sД±", ...agingSummary.map((row) => row.bucket)];
  const visibleRows = rows.filter((row) => {
    const matchesType = typeFilter === "HamД±sД±" || row.type === typeFilter;
    const rowSourceType = row.sourceTypeLabel || row.sourceType;
    const matchesSourceType = sourceTypeFilter === "HamД±sД±" || rowSourceType === sourceTypeFilter;
    const matchesRisk = riskFilter === "HamД±sД±" || row.riskCategory === riskFilter;
    const matchesCollection = collectionFilter === "HamД±sД±" || row.collectionStatus === collectionFilter;
    const matchesAging = agingFilter === "HamД±sД±" || row.agingBucket === agingFilter;
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="Debitor borcu" value={money(totalDebitor)} trend={`${debtorRows.length} borc sЙ™tri`} icon={Wallet} tone="primary" />
        <MetricCard label="Kreditor borcu" value={money(totalCreditor)} trend={`${creditorRows.length} borc sЙ™tri`} icon={Building2} tone="warning" />
        <MetricCard label="Net mГ¶vqe" value={money(netPosition)} icon={TrendingUp} tone={netPosition >= 0 ? "success" : "danger"} />
      ...
      <section className="receivable-aging-grid" data-testid="receivable-aging-panel">
        {agingSummary.map((bucket) => (
          <div key={bucket.bucket} className={`aging-bucket ${bucket.count > 0 ? "active" : ""}`}>
            ...
            <small>{bucket.count} borc sЙ™tri</small>
          </div>
        ))}
      </section>
      {syncMeta && (
        <Panel className="module-action-panel">
          <PanelHeader title="Son balans yenilЙ™nmЙ™si" subtitle="SatД±Еџ, kredit vЙ™ vendor mЙ™lumatlarД±ndan son sinxron nЙ™ticЙ™" icon={RefreshCw} />
          <div className="db-status-grid">
            <div>
              <span>Vaxt</span>
      ...
      <Panel className="receivable-control-panel" data-testid="receivable-control-panel">
        <PanelHeader
          ...
          subtitle="Aging, kolleksiya statusu, risk kateqoriyasД± vЙ™ baДџlanД±Еџ workflow-u"
          icon={Wallet}
        />
        <div className="receivable-filter-bar">
          <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}>
            {["HamД±sД±", "Debitor", "Kreditor"].map((item) => (
          ...
          <span>{visibleRows.length}/{rows.length} sЙ™tir</span>
        </div>
        <DataTable
          columns={["Tip", "TЙ™rЙ™f", "MЙ™nbЙ™", "MЙ™blЙ™Дџ", "Aging", "Risk", "Kolleksiya", "NГ¶vbЙ™ti addД±m", "MЙ™sul", "BaДџlanД±Еџ"]}
          rows={visibleRows.map((row) => [
            <StatusBadge status={row.type} />,
            <TwoLine title={row.party} subtitle={row.detail} />,
            <TwoLine title={row.source} subtitle={row.sourceTypeLabel || row.sourceType || "MЙ™nbЙ™"} />,
            <strong>{money(row.amount)}</strong>,
            <TwoLine title={row.agingBucket} subtitle={Number(row.overdueDays || 0) > 0 ? `${row.overdueDays} gГјn` : "GecikmЙ™ yoxdur"} />,
            ...
            Number(row.amount || 0) > 0 ? (
              ...
              <StatusBadge status="BaДџlandД±" />
            ),
          ])}
        />
      </Panel>
      <Panel className="receivable-closure-panel">
        <PanelHeader title="BaДџlanД±Еџ tarixГ§Й™si" subtitle="Debitor kassa mЙ™daxili vЙ™ kreditor Г¶dЙ™niЕџ Г§Д±xД±ЕџlarД± audit izi ilЙ™ saxlanД±lД±r" icon={Check} />
        {closures.length === 0 ? (
          <EmptyState title="Borc baДџlanД±ЕџД± hЙ™lЙ™ yoxdur" />
        ) : (
          <DataTable
            columns={["ID", "Tip", "TЙ™rЙ™f", "MЙ™blЙ™Дџ", "Aging", "Risk", "Tarix", "Status"]}
            rows={closures.slice(0, 8).map((row) => [
              ...
              <StatusBadge status={row.riskCategory} />,
              row.at,
              <StatusBadge status={row.collectionStatus || "BaДџlandД±"} />,
            ])}
          />
        )}
      </Panel>
...
function VendorsPage({
  ...
  const vendorRiskCount = vendors.filter(
    (vendor) => normalize(vendor.status).includes("risk") || normalize(vendor.status).includes("aЕџaДџД±"),
  ).length;
  const pendingPoCount = purchaseOrders.filter((po) => po.status === "TЙ™sdiq gГ¶zlЙ™yir").length;

  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="Aktiv vendorlar" value={vendors.length} icon={Building2} tone="primary" />
        <MetricCard label="Гњmumi SKU" value={total(vendors, "sku")} trend="+24 bu ay" icon={Boxes} tone="info" />
        <MetricCard
          label="Kvota icrasД±"
        ...
        />
        <MetricCard
          label="PO tГ¶vsiyЙ™si"
          value={pendingPoCount || purchaseNeed.length}
          trend={pendingPoCount > 0 ? `${pendingPoCount} tЙ™sdiq gГ¶zlЙ™yir` : money(procurementBudget)}
          icon={Package}
      ...
      <Panel className="procurement-panel">
        <PanelHeader
          title="Procurement planД±"
          subtitle="Anbar qalД±ДџД± vЙ™ satД±Еџ tempinЙ™ gГ¶rЙ™ vendor ГјzrЙ™ sifariЕџ tГ¶vsiyЙ™lЙ™ri"
          icon={Package}
        />
        <div className="procurement-actions">
          <button
            ...
            title={products.length === 0 || warehouses.length === 0 ? "ЖЏvvЙ™l mЙ™hsul vЙ™ anbar yaradД±n" : "Zavoddan mЙ™hsul sifariЕџi yaradД±n"}
        ...
        <div className="procurement-summary-grid">
          <div>
            <span>SatД±nalma bГјdcЙ™si</span>
            <strong>{money(procurementBudget)}</strong>
            <small>{purchaseNeed.length} mЙ™hsul ГјГ§Гјn PO aГ§Д±la bilЙ™r</small>
          </div>
          <div>
            <span>Vendor riski</span>
            <strong>{vendorRiskCount}</strong>
            <small>Kvota vЙ™ icra nЙ™zarЙ™ti</small>
          </div>
          <div>
            <span>SifariЕџdЙ™</span>
            <strong>{openPoQty}</strong>
            <small>AГ§Д±q PO ГјzrЙ™ yolda olan mЙ™hsul</small>
          </div>
        </div>
        <DataTable
          columns={["MЙ™hsul", "Vendor", "SatД±Еџ", "SatД±Еџ ГјГ§Гјn", "Minimum", "TГ¶vsiyЙ™", "SifariЕџdЙ™", "BГјdcЙ™", "Status", "PO"]}
          rows={procurementRows.slice(0, 8).map((row) => [
            <strong>{row.product}</strong>,
            row.vendor,
            `${row.sold} Й™dЙ™d`,
            `${row.available} Й™dЙ™d`,
            row.reorderPoint > 0 ? `${row.reorderPoint} Й™dЙ™d` : "вЂ”",
            row.recommendedQty > 0 ? `${row.recommendedQty} Й™dЙ™d` : "Yoxdur",
            row.orderedQty > 0 ? <TwoLine title={`${row.orderedQty} Й™dЙ™d`} subtitle={row.latestPoId || `${row.openPoCount} PO`} /> : "Yoxdur",
            row.estimatedCost > 0 ? money(row.estimatedCost) : "вЂ”",
            <StatusBadge status={row.status} />,
            <button
              className="text-btn"
            ...
            >
              {row.orderGap > 0 ? "PO yarat" : "BaДџlД±dД±r"}
            </button>,
          ])}
        />
      </Panel>
      <Panel className="po-action-panel">
        <PanelHeader
          title="Purchase Order axД±nД±"
          subtitle="PO tЙ™sdiqlЙ™nЙ™ndЙ™ mЙ™hsul avtomatik anbara mЙ™daxil edilir vЙ™ alД±Еџ xЙ™rci maliyyЙ™yЙ™ dГјЕџГјr"
          icon={FileText}
        />
        <DataTable
          columns={["PO", "MЙ™nbЙ™", "MЙ™hsul", "Anbar", "Say", "AlД±Еџ", "MЙ™blЙ™Дџ", "GГ¶zlЙ™nЙ™n", "Status", "ЖЏmЙ™liyyat"]}
          rows={purchaseOrders.map((po) => [
            ...
            po.status === "TЙ™sdiq gГ¶zlЙ™yir" ? (
              <button className="text-btn" disabled={!canManagePo} onClick={() => onApprovePurchaseOrder(po.id)}>
                TЙ™sdiq et
              </button>
            ) : (
              <TwoLine title="MЙ™daxil edilib" subtitle={po.receivedAt || po.approvedAt} />
            ),
          ])}
        />
      </Panel>
      <Panel>
        <PanelHeader title={`Vendor Kvota CЙ™dvЙ™li вЂ” ${currentBusinessYear} Q${currentBusinessQuarter}`} subtitle="SatД±Еџ hЙ™dЙ™fi vЙ™ risk statusu" />
        <DataTable
          columns={["Vendor", "Г–lkЙ™", "SKU", "SatД±lД±b", "Kvota", "Д°cra", "Status"]}
...
const hrLevelOptions = ["RЙ™hbЙ™rlik", "ЕћГ¶bЙ™ rЙ™hbЙ™ri", "Komanda lideri", "Komanda ГјzvГј", "TЙ™crГјbЙ™Г§i"];
const hrPlatformTabs = ["Komanda", "Д°Еџ vaxtД±", "MЙ™zuniyyЙ™t", "Payroll", "Recruitment"];
...
function getEmployeeLevel(employee) {
  ...
  if (position.includes("direktor")) return "RЙ™hbЙ™rlik";
  if (position.includes("baЕџ") || position.includes("rЙ™hbЙ™r")) return "ЕћГ¶bЙ™ rЙ™hbЙ™ri";
  return "Komanda ГјzvГј";
}

function isHrLeadershipLevel(level) {
  const text = normalize(level);
  return text.includes("rЙ™hb") || text.includes("lider") || text.includes("direktor");
}
...
function getEmployeeManagerName(employee, employees) {
  ...
  const position = normalize(employee.position);
  if (position.includes("direktor") || position.includes("baЕџ") || position.includes("rЙ™hbЙ™r")) return "";
  ...
  const departmentLead = employees.find((item) => {
    ...
    return leadPosition.includes("baЕџ") || leadPosition.includes("rЙ™hbЙ™r");
  });
...
function getHrDraft(employee, employees) {
  return {
    ...
    level: employee ? getEmployeeLevel(employee) : "Komanda ГјzvГј",
  };
}
...
function buildHrStructure(employees, departmentRecords = []) {
  ...
  const departmentNames = [...new Set(normalizedEmployees.map((employee) => employee.department || "ЕћГ¶bЙ™siz"))].sort((a, b) =>
    a.localeCompare(b, "az"),
  );

  const structures = departmentNames.map((department) => {
    const departmentRows = normalizedEmployees.filter((employee) => (employee.department || "ЕћГ¶bЙ™siz") === department);
...
function buildHrPlanningRows(structure) {
  return structure.map((department, index) => {
    ...
    const trainingNeed = department.avgKpi < 95 ? "TЙ™lim planД±" : "Standart izlЙ™mЙ™";
    ...
    return {
      ...
      trainingNeed,
      payrollForecast,
      onboarding: vacancyNeed > 0 ? (index % 2 ? "SatД±Еџ tЙ™crГјbЙ™Г§isi" : "ЖЏmЙ™liyyat assistenti") : "Yeni qЙ™bul yoxdur",
      status: vacancyNeed > 0 ? "Vakansiya aГ§" : department.avgKpi < 95 ? "TЙ™lim lazД±mdД±r" : "Stabil",
    };
  });
}
...
function getHrDocumentRows(employee = {}) {
  ...
  const rows = [
    { key: "identity", title: "ЕћЙ™xsiyyЙ™t/FД°N", threshold: 25 },
    { key: "contract", title: "ЖЏmЙ™k mГјqavilЙ™si", threshold: 60 },
    { key: "job", title: "VЙ™zifЙ™ tЙ™limatД±", threshold: 80 },
  ...
  return rows.map((row) => {
    ...
    return {
      ...row,
      complete,
      status: complete ? "Tamam" : critical ? "YenilЙ™nmЙ™lidir" : "TЙ™sdiq gГ¶zlЙ™yir",
...
function getHrDocumentHealth(record = {}) {
  ...
  return {
    ...
    status:
      missing.length === 0
        ? "Tamam"
        : missing.some((document) => document.status === "YenilЙ™nmЙ™lidir")
          ? "YenilЙ™nmЙ™lidir"
          : "TЙ™sdiq gГ¶zlЙ™yir",
  };
}
...
function buildHrLeaveUsageMap(leaveRequests = []) {
  return leaveRequests.reduce((map, request) => {
    ...
    if (request.status === "TЙ™sdiq edildi") current.approved += Number(request.days || 0);
    if (request.status === "TЙ™sdiq gГ¶zlЙ™yir") current.pending += Number(request.days || 0);
    map.set(key, current);
    if (request.employeeName && request.employeeName !== key) {
      const byName = map.get(request.employeeName) || { approved: 0, pending: 0 };
      if (request.status === "TЙ™sdiq edildi") byName.approved += Number(request.days || 0);
      if (request.status === "TЙ™sdiq gГ¶zlЙ™yir") byName.pending += Number(request.days || 0);
...
function buildHrEmployeeRecords(employees, leaveRequests = []) {
  ...
  return employees.map((employee) => {
    ...
    const documentReviewRequired = Boolean(employee.documentReviewRequired || employee.hrStatus === "MЙ™lumat gГ¶zlЙ™yir");
    ...
    return {
      ...
      workMode: employee.workMode || "TЙ™yin edilmЙ™yib",
      shift: employee.shift || "TЙ™yin edilmЙ™yib",
      employmentType: employee.employmentType || "TЙ™yin edilmЙ™yib",
      leaveBalance,
...
function buildHrAttendanceRows(records) {
  return records.filter((record) => record.checkIn || record.checkOut).map((record) => ({
    ...
    department: record.department,
    shift: record.shift,
    checkIn: record.checkIn || "вЂ”",
    checkOut: record.checkOut || "вЂ”",
    lateDays: record.lateDays,
    attendanceRate: record.attendanceRate,
    status: record.attendanceRate < 90 ? "NЙ™zarЙ™t" : record.lateDays > 2 ? "GecikmЙ™ var" : "Normal",
  }));
}
...
function buildHrLeaveRows(records, leaveRequests = []) {
  ...
  return leaveRequests.map((request) => {
    ...
    return {
      id: request.id,
      employee: employee?.name || request.employeeName || "ЖЏmЙ™kdaЕџ silinib",
      department: employee?.department || request.department || "вЂ”",
      type: request.type,
      from: request.from,
      to: request.to,
      ...
      status: request.status || "TЙ™sdiq gГ¶zlЙ™yir",
...
function buildHrRecruitmentRows(planningRows, vacancies = []) {
  const activeVacancies = vacancies.map((vacancy) => ({
    ...
    stage: vacancy.stage || "NamizЙ™d gГ¶zlЙ™nilir",
  ...
  const plannedVacancies = planningRows
    ...
    .map((row) => ({
      ...
      role: `${row.department} ГјzrЙ™ mГјtЙ™xЙ™ssis`,
      department: row.department,
      candidates: 0,
      stage: "PlanlanД±r",
      owner: "HR",
      targetDate: "TЙ™yin edilmЙ™yib",
      status: "PlanlanД±r",
    }));
...
function HrPage({
  ...
  const [teamDepartment, setTeamDepartment] = useState("HamД±sД±");
  const [teamStatus, setTeamStatus] = useState("HamД±sД±");
  ...
  const visibleHrEmployeeRecords = useMemo(() => {
    ...
    return hrEmployeeRecords.filter((record) => {
      ...
      const matchesDepartment = teamDepartment === "HamД±sД±" || record.department === teamDepartment;
      const matchesStatus = teamStatus === "HamД±sД±" || record.hrStatus === teamStatus;
  ...
  const pendingLeaveCount = leaveRows.filter((row) => row.status === "TЙ™sdiq gГ¶zlЙ™yir").length;
  const paidPayrollCount = payrollRows.filter((row) => row.status === "Г–dЙ™nildi").length;
  const documentRiskCount = hrEmployeeRecords.filter((row) => row.missingDocumentCount > 0).length;

  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="Гњmumi Й™mЙ™kdaЕџ" value={employees.length} icon={Users} tone="primary" />
        <MetricCard label="AylД±q maaЕџ fondu" value={money(total(employees, "salary"))} icon={Wallet} tone="success" />
        <MetricCard
          label="Orta KPI"
          ...
          tone="info"
        />
        <MetricCard label="Struktur ЕџГ¶bЙ™lЙ™ri" value={departmentCount} trend={`${leaders.length} rЙ™hbЙ™r rol`} icon={Building2} tone="warning" />
      </section>

      <Panel className="hr-platform-panel">
        <PanelHeader
          title="HR Platform"
          subtitle="ЖЏmЙ™kdaЕџ 360, iЕџ vaxtД±, mЙ™zuniyyЙ™t, payroll vЙ™ recruitment axД±nlarД±"
          icon={UserCog}
        />
        <div className="hr-platform-toolbar">
          ...
          <div className="hr-platform-kpis">
            <span>{pendingLeaveCount} mЙ™zuniyyЙ™t tЙ™sdiqdЙ™</span>
            <span>{paidPayrollCount}/{payrollRows.length} payroll Г¶dЙ™nildi</span>
            <span>{documentRiskCount} sЙ™nЙ™d aГ§Д±ДџД±</span>
          ...
          <div className="hr-team-workspace">
            <div className="hr-team-controls">
              <label className="hr-team-search">
                <Search size={16} />
                <input value={teamQuery} onChange={(event) => setTeamQuery(event.target.value)} placeholder="ЖЏmЙ™kdaЕџ, vЙ™zifЙ™ vЙ™ ya rЙ™hbЙ™r axtar..." />
              </label>
              ...
              <select value={teamStatus} onChange={(event) => setTeamStatus(event.target.value)}>
                ...
                <option>MЙ™lumat gГ¶zlЙ™yir</option>
              </select>
              <strong>{visibleHrEmployeeRecords.length} Й™mЙ™kdaЕџ</strong>
            </div>
            <HrEmployeePlatform
        ...
        {hrView === "Д°Еџ vaxtД±" && <HrAttendancePlatform rows={attendanceRows} />}
        {hrView === "MЙ™zuniyyЙ™t" && <HrLeavePlatform rows={leaveRows} onCreate={onCreateLeaveRequest} onUpdateStatus={onUpdateLeaveStatus} />}
      ...
      <Panel className="hr-planning-panel">
        <PanelHeader
          title="HR planlama"
          subtitle="Vakansiya, onboarding, tЙ™lim vЙ™ maaЕџ forecast gГ¶stЙ™ricilЙ™ri"
          icon={UserCog}
        />
        <div className="hr-planning-summary">
          <div>
            <span>AГ§Д±lacaq vakansiya</span>
            <strong>{vacancyCount}</strong>
            <small>ЕћГ¶bЙ™ yГјkГјnЙ™ gГ¶rЙ™</small>
          </div>
          <div>
            <span>NГ¶vbЙ™ti maaЕџ forecast</span>
            <strong>{money(hrPlanningRows.reduce((sum, row) => sum + row.payrollForecast, 0))}</strong>
            <small>8% artД±m modeli</small>
          </div>
          <div>
            <span>TЙ™lim ehtiyacД±</span>
            <strong>{hrPlanningRows.filter((row) => row.status === "TЙ™lim lazД±mdД±r").length}</strong>
            <small>KPI 95%-dЙ™n aЕџaДџД±</small>
          </div>
        </div>
        <DataTable
          columns={["ЕћГ¶bЙ™", "Headcount", "RЙ™hbЙ™r", "Orta KPI", "Onboarding", "MaaЕџ forecast", "Status"]}
      ...
      <section className="hr-structure-layout">
        <Panel className="hr-builder-panel">
          <PanelHeader title="Struktur qurucusu" subtitle="ЖЏmЙ™kdaЕџД± seГ§in, rЙ™hbЙ™r vЙ™ ЕџГ¶bЙ™ Й™laqЙ™sini tЙ™yin edin" icon={UserCog} />
          {selectedEmployee ? (
            <HrStructureBuilder
            ...
            <EmptyState title="Struktur ГјГ§Гјn Й™mЙ™kdaЕџ yoxdur" />
          )}
        </Panel>

        <Panel className="hr-tree-panel">
          <PanelHeader title="Struktur aДџacД±" subtitle="ЕћГ¶bЙ™, rЙ™hbЙ™r vЙ™ komanda xЙ™tti" icon={Building2} />
          <div className="hr-structure-actions">
            <button className="secondary-btn" onClick={onCreateDepartment}><Plus size={16} /> ЕћГ¶bЙ™ Й™lavЙ™ et</button>
          </div>
      ...
      <Panel className="hr-employee-registry-panel">
        <PanelHeader title={`ЖЏmЙ™kdaЕџlar (${visibleRegistryEmployees.length})`} subtitle="VЙ™zifЙ™, ЕџГ¶bЙ™, maaЕџ vЙ™ KPI" />
        <DataTable
          columns={["ЖЏmЙ™kdaЕџ", "VЙ™zifЙ™", "ЕћГ¶bЙ™", "RЙ™hbЙ™r", "SЙ™viyyЙ™", "MaaЕџ", "KPI", ""]}
          rows={visibleRegistryEmployees.map((employee) => [
            <AvatarLine initials={employee.initials} title={employee.name} />,
            employee.position,
            employee.department,
            getEmployeeManagerName(employee, allEmployees) || "BirbaЕџa",
            ...
            <div className="hr-row-actions">
              <button className="icon-btn hr-row-edit" title="ЖЏmЙ™kdaЕџД± redaktЙ™ et" aria-label={`${employee.name} Й™mЙ™kdaЕџД±nД± redaktЙ™ et`} onClick={() => onEditEmployee(employee)}><Pencil size={16} /></button>
              <button className="icon-btn hr-row-delete" title="ЖЏmЙ™kdaЕџД± sil" aria-label={`${employee.name} Й™mЙ™kdaЕџД±nД± sil`} onClick={() => onDeleteEmployee(employee)}><Trash2 size={16} /></button>
            </div>,
          ])}
        />
          ...
          {visibleRegistryEmployees.map((employee) => (
            <div className="hr-mobile-employee-card" key={employee.name}>
              ...
              <div className="hr-mobile-employee-meta">
                ...
                <span>{getEmployeeManagerName(employee, allEmployees) || "BirbaЕџa"}</span>
...
function HrEmployeePlatform({ records, selectedRecord, onSelect, onEdit, onDelete, onUpdateDocuments }) {
  if (!selectedRecord) return <EmptyState title={records.length ? "ЖЏmЙ™kdaЕџ seГ§ilmЙ™yib" : "FilterЙ™ uyДџun Й™mЙ™kdaЕџ tapД±lmadД±"} />;
    ...
    <section className="hr-employee-platform">
      <div className="hr-people-list">
        {records.map((record) => (
          ...
          >
            <AvatarLine initials={record.initials} title={record.name} subtitle={`${record.department} В· ${record.position}`} />
      ...
      <div className="hr-profile-360" data-testid="hr-employee-360">
        <div className="hr-profile-head">
          ...
          <div>
            ...
            <p>{selectedRecord.position} В· {selectedRecord.department}</p>
          </div>
          <div className="hr-profile-actions">
            <StatusBadge status={selectedRecord.hrStatus} />
            <button className="icon-btn hr-profile-edit" title="ЖЏmЙ™kdaЕџД± redaktЙ™ et" aria-label={`${selectedRecord.name} Й™mЙ™kdaЕџД±nД± redaktЙ™ et`} onClick={() => onEdit(selectedRecord)}><Pencil size={16} /></button>
            <button className="icon-btn hr-row-delete hr-profile-delete" title="ЖЏmЙ™kdaЕџД± sil" aria-label={`${selectedRecord.name} Й™mЙ™kdaЕџД±nД± sil`} onClick={() => onDelete(selectedRecord)}><Trash2 size={16} /></button>
          </div>
        </div>
        <div className="hr-profile-grid">
          <TwoLine title="RЙ™hbЙ™r" subtitle={selectedRecord.managerName || "BirbaЕџa rЙ™hbЙ™rlik"} />
          <TwoLine title="Д°Еџ rejimi" subtitle={`${selectedRecord.workMode} В· ${selectedRecord.shift}`} />
          <TwoLine title="Д°ЕџЙ™ qЙ™bul" subtitle={selectedRecord.hireDate} />
          <TwoLine title="NГ¶vbЙ™ti review" subtitle={selectedRecord.nextReview} />
          <TwoLine title="Attendance" subtitle={percent(selectedRecord.attendanceRate)} />
          <TwoLine title="MЙ™zuniyyЙ™t balansД±" subtitle={`${selectedRecord.leaveBalance} gГјn`} />
          <TwoLine title="SЙ™nЙ™d uyДџunluДџu" subtitle={percent(selectedRecord.documentsComplete)} />
          ...
          <TwoLine title="Г–dЙ™nilmЙ™ tarixi" subtitle={selectedRecord.payrollPaidAt || "HЙ™lЙ™ baДџlanmayД±b"} />
          <TwoLine title="SЙ™nЙ™d statusu" subtitle={selectedRecord.documentStatus} />
        </div>
        <div className="hr-profile-snapshot">
          <div>
            <span>MЙ™zuniyyЙ™t</span>
            <strong>{selectedRecord.leaveBalance} gГјn qalД±q</strong>
            <small>{selectedRecord.usedLeave} gГјn istifadЙ™ В· {selectedRecord.pendingLeaveDays} gГјn tЙ™sdiqdЙ™</small>
          </div>
          <div>
            <span>Payroll</span>
            <strong>{selectedRecord.payrollStatus}</strong>
            <small>{money(selectedRecord.netSalary)} net В· {money(selectedRecord.employerCost)} iЕџЙ™gГ¶tГјrЙ™n xЙ™rci</small>
          </div>
          <div>
            <span>SЙ™nЙ™dlЙ™r</span>
            <strong>{selectedRecord.documentStatus}</strong>
            <small>{selectedRecord.missingDocumentCount} aГ§Д±q sЙ™nЙ™d В· {percent(selectedRecord.documentsComplete)}</small>
          </div>
        </div>
        <div className="hr-skill-strip">
          {(selectedRecord.skills.length ? selectedRecord.skills : ["Profil bacarД±qlarД± Й™lavЙ™ edilmЙ™yib"]).map((skill) => (
  ...
  return (
    <div className="hr-platform-section">
      <div className="hr-platform-summary">
        <div>
          <span>Orta davamiyyЙ™t</span>
          <strong>{percent(averageAttendance)}</strong>
          <small>Bu ay ГјzrЙ™</small>
        </div>
        <div>
          <span>GecikmЙ™</span>
          <strong>{lateTotal}</strong>
          <small>Toplam gecikmЙ™ gГјnГј</small>
        </div>
        <div>
          <span>NЙ™zarЙ™t</span>
      ...
      <DataTable
        columns={["ЖЏmЙ™kdaЕџ", "ЕћГ¶bЙ™", "NГ¶vbЙ™", "GiriЕџ", "Г‡Д±xД±Еџ", "GecikmЙ™", "DavamiyyЙ™t", "Status"]}
        rows={rows.map((row) => [
          <strong>{row.employee}</strong>,
          row.department,
          row.shift,
...
function HrLeavePlatform({ rows, onCreate, onUpdateStatus }) {
  const pending = rows.filter((row) => row.status === "TЙ™sdiq gГ¶zlЙ™yir");
  const plannedDays = rows.reduce((sum, row) => sum + Number(row.days || 0), 0);
  const approvedDays = rows
    .filter((row) => row.status === "TЙ™sdiq edildi")
    .reduce((sum, row) => sum + Number(row.days || 0), 0);

  return (
    <div className="hr-platform-section">
      <div className="hr-operation-toolbar">
        <span>MЙ™zuniyyЙ™t tЙ™lЙ™blЙ™ri vЙ™ balans nЙ™zarЙ™ti</span>
        <button className="secondary-btn" onClick={onCreate}><Plus size={16} /> MЙ™zuniyyЙ™t qeydi</button>
      </div>
      <div className="hr-platform-summary">
        <div>
          <span>TЙ™sdiq gГ¶zlЙ™yir</span>
          <strong>{pending.length}</strong>
          <small>RЙ™hbЙ™r baxД±ЕџД±</small>
        </div>
        <div>
          <span>TЙ™sdiqlЙ™nЙ™n gГјn</span>
          <strong>{approvedDays}</strong>
          <small>Balansdan silinir</small>
        </div>
        <div>
          <span>Planlanan gГјn</span>
          <strong>{plannedDays}</strong>
          <small>MЙ™zuniyyЙ™t yГјkГј</small>
        </div>
        <div>
          <span>Orta balans</span>
          <strong>{rows.length ? Math.round(rows.reduce((sum, row) => sum + row.balance, 0) / rows.length) : 0}</strong>
          <small>QalД±q gГјn</small>
        </div>
      </div>
      <DataTable
        columns={["ЖЏmЙ™kdaЕџ", "Tip", "Tarix", "GГјn", "Balans", "TЙ™sdiqlЙ™yЙ™n", "Status", ""]}
        rows={rows.map((row) => [
          ...
          row.approver,
          <StatusBadge status={row.status} />,
          row.status === "TЙ™sdiq gГ¶zlЙ™yir" && onUpdateStatus ? (
            <div className="hr-leave-actions">
              <button className="text-btn" onClick={() => onUpdateStatus(row.id, "TЙ™sdiq edildi")}>TЙ™sdiq</button>
              <button className="text-btn danger" onClick={() => onUpdateStatus(row.id, "Д°mtina edildi")}>Д°mtina</button>
            </div>
          ) : (
            <small>{row.decidedAt || "вЂ”"}</small>
          ),
        ])}
      />
    </div>
...
function HrPayrollPlatform({ rows, totalNet, onMarkPaid }) {
  ...
  const paidRows = rows.filter((row) => row.status === "Г–dЙ™nildi");
  const readyRows = rows.filter((row) => row.status === "Hesablama hazД±rdД±r");
  const blockedRows = rows.filter((row) => row.status === "SЙ™nЙ™d gГ¶zlЙ™yir");
    ...
    <div className="hr-platform-section">
      <div className="hr-platform-summary">
        <div>
          ...
          <small>MaaЕџ + bonus</small>
        </div>
        <div>
          <span>Tutulmalar</span>
          <strong>{money(deductions)}</strong>
          <small>Vergi vЙ™ sosial</small>
        </div>
        <div>
          <span>Net Г¶dЙ™niЕџ</span>
          <strong>{money(net)}</strong>
          <small>Payroll uГ§otu</small>
        </div>
        <div>
          <span>Д°ЕџЙ™gГ¶tГјrЙ™n xЙ™rci</span>
          <strong>{money(employerCost)}</strong>
          <small>Gross + iЕџЙ™gГ¶tГјrЙ™n Г¶dЙ™niЕџlЙ™ri</small>
        </div>
        <div>
          <span>Status</span>
          <strong>{paidRows.length}/{rows.length}</strong>
          <small>{readyRows.length} hazД±r В· {blockedRows.length} sЙ™nЙ™d gГ¶zlЙ™yir</small>
        </div>
      </div>
      <DataTable
        columns={["ЖЏmЙ™kdaЕџ", "ЕћГ¶bЙ™", "Period", "MaaЕџ", "Bonus", "Tutulma", "Net", "Д°ЕџЙ™gГ¶tГјrЙ™n xЙ™rci", "Status", ""]}
        rows={rows.map((row) => [
          <strong>{row.employee}</strong>,
          row.department,
          <TwoLine title={row.period} subtitle={row.paidAt || "Г–dЙ™nilmЙ™yib"} />,
          money(row.salary),
          money(row.bonus),
          ...
          money(row.employerCost),
          <StatusBadge status={row.status} />,
          row.status === "Г–dЙ™nildi" ? (
            <small>{row.paidAt}</small>
          ) : (
            <div className="hr-payroll-actions">
              <button
                className="text-btn"
                disabled={row.status === "SЙ™nЙ™d gГ¶zlЙ™yir" || !onMarkPaid}
...
function HrRecruitmentPlatform({ rows, onCreate }) {
  ...
  return (
    <div className="hr-platform-section">
      <div className="hr-operation-toolbar">
        <span>Vakansiya pipeline vЙ™ namizЙ™d mЙ™rhЙ™lЙ™lЙ™ri</span>
        <button className="secondary-btn" onClick={onCreate}><Plus size={16} /> Vakansiya Й™lavЙ™ et</button>
      </div>
        ...
        {rows.map((row) => (
          <div className="hr-recruitment-card" key={`${row.department}-${row.role}`}>
            <div>
              ...
              <span>{row.department} В· {row.owner}</span>
            </div>
            <div className="hr-recruitment-meta">
              <b>{row.candidates}</b>
              <small>namizЙ™d</small>
            </div>
            <StatusBadge status={row.stage} />
          </div>
        ))}
      ...
      <DataTable
        columns={["Rol", "ЕћГ¶bЙ™", "NamizЙ™d", "MЙ™rhЙ™lЙ™", "Owner", "HЙ™dЙ™f tarix", "Status"]}
        rows={rows.map((row) => [
          <strong>{row.role}</strong>,
          row.department,
          row.candidates,
  ...
  return (
    <form className="hr-builder-form" onSubmit={submit}>
      ...
      <div className="hr-builder-preview">
        <TwoLine title={selectedEmployee.name} subtitle={`${draft.position} В· ${draft.department}`} />
        <StatusBadge status={draft.level} />
        <small>{draft.managerName ? `${draft.managerName} rЙ™hbЙ™rliyindЙ™` : "BirbaЕџa rЙ™hbЙ™rlik xЙ™tti"} В· {draft.departmentParent || "ЖЏsas ЕџГ¶bЙ™"}</small>
      </div>
...
function HrStructureTree({ structure, employees, onSelectEmployee }) {
  ...
  const selectedTitle = selectedDepartment === "all" ? "BГјtГјn ЕџirkЙ™t" : selectedDepartment;
  ...
  if (structure.length === 0) {
    return <EmptyState title="Struktur aДџacД± boЕџdur" />;
  }
    ...
    <div className="hr-tree">
        ...
        <div className="hr-org-chart-canvas">
          <button className={`hr-org-company-card ${selectedDepartment === "all" ? "active" : ""}`} onClick={() => setSelectedDepartment("all")}>
            ...
            <span>
              ...
              <small>ЕћirkЙ™t strukturu</small>
            </span>
          </button>
      ...
      <div className="hr-reporting-panel">
        <div className="hr-reporting-head">
          <div>
            <strong>{selectedTitle} ГјzrЙ™ tabeГ§ilik xЙ™tti</strong>
            <span>{departmentScope ? `${departmentScope.size} ЕџГ¶bЙ™` : `${employees.length} Й™mЙ™kdaЕџ`}</span>
          </div>
          <StatusBadge status={`${reportingRoots.length} rЙ™hbЙ™rlik xЙ™tti`} />
        </div>
        <div className="hr-employee-branch">
          ...
          {reportingRoots.length === 0 && <EmptyState title="Bu ЕџГ¶bЙ™ ГјzrЙ™ tabeГ§ilik xЙ™tti yoxdur" />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={`hr-org-branch ${hasChildren ? "has-children" : ""}`}>
      ...
      >
        ...
        <span className="hr-org-card-label">{department.isVirtual ? "ЕћГ¶bЙ™ qrupu" : "ЕћГ¶bЙ™"}</span>
        <strong>{department.department}</strong>
        <span className="hr-org-card-count"><Users size={13} />{department.count} Й™mЙ™kdaЕџ</span>
        <span className="hr-org-card-lead">
          <span className="small-avatar">{lead?.initials || "HR"}</span>
          <span>
            <b>{lead?.name || "RЙ™hbЙ™r tЙ™yin edilmЙ™yib"}</b>
            <small>{lead?.position || "Alt ЕџГ¶bЙ™lЙ™r"}</small>
          </span>
        </span>
    ...
    <div className="hr-tree-item">
      <button className={`hr-employee-node ${employee.isInScope ? "in-scope" : ""}`} onClick={() => onSelectEmployee(employee.name)}>
        ...
        <div>
          ...
          <span>{employee.position} В· {employee.department}</span>
          <small>{employee.managerName ? `${employee.managerName}-a tabedir` : "BirbaЕџa rЙ™hbЙ™rlik"}</small>
        </div>
...
function KpiPage({
  ...
  const [bonusFilter, setBonusFilter] = useState("HamД±sД±");
  ...
  const visibleBonuses = salesBonuses.filter((row) => bonusFilter === "HamД±sД±" || row.seller === bonusFilter);
  ...
  const canClose = activePeriod.status !== "Period baДџlandД±";
  const canApprove = activePeriod.status === "Period baДџlandД±" && activePeriod.approvalStatus !== "TЙ™sdiq edildi";
  const canPayout = activePeriod.approvalStatus === "TЙ™sdiq edildi" && activePeriod.payoutStatus !== "Г–dЙ™nildi";

  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="ЕћirkЙ™t Гјmumi KPI" value={percent(companyKpi)} trend={`${ranking.length} Й™mЙ™kdaЕџ ГјzrЙ™`} icon={TrendingUp} tone="success" />
        ...
        <MetricCard label="Payout fondu" value={money(activePeriod.payoutAmount || 0)} trend={`${payoutRows.length} Й™mЙ™kdaЕџ`} icon={Wallet} tone="warning" />
        <MetricCard label="Top performer" value={topPerformer?.name || "MЙ™lumat yoxdur"} trend={topPerformer ? `${topPerformer.kpi}% KPI` : "ЖЏmЙ™kdaЕџ Й™lavЙ™ edilmЙ™yib"} icon={ShieldCheck} tone="primary" />
      </section>

      <section className="kpi-control-grid">
        <Panel className="kpi-target-plan-panel" data-testid="kpi-target-plan-panel">
          <PanelHeader title="KPI hЙ™dЙ™f planД±" subtitle="Г‡Й™kili hЙ™dЙ™flЙ™r vЙ™ cari faktiki nЙ™ticЙ™lЙ™r" icon={SlidersHorizontal} />
          <DataTable
            columns={["HЙ™dЙ™f", "MЙ™sul", "Г‡Й™ki", "Faktiki / HЙ™dЙ™f", "Progress", "Status"]}
        ...
        <Panel className="kpi-period-panel" data-testid="kpi-period-panel">
          <PanelHeader title="Period baДџlanД±ЕџД±" subtitle="BaДџla, tЙ™sdiq et vЙ™ payout-u maliyyЙ™yЙ™ yaz" icon={CalendarClock} />
          <div className="kpi-period-grid">
            <div className="kpi-period-card">
              <span>Period</span>
              <strong>{activePeriod.period || getKpiPeriodKey()}</strong>
              <small>{activePeriod.closedAt || "AГ§Д±q hesablama"}</small>
            </div>
            <div className="kpi-period-card">
              <span>Status</span>
              <StatusBadge status={activePeriod.status || "AГ§Д±q period"} />
              <small>{percent(activePeriod.companyScore || 0)} score</small>
            </div>
            <div className="kpi-period-card">
              <span>TЙ™sdiq</span>
              <StatusBadge status={activePeriod.approvalStatus || "HazД±rlanД±r"} />
              <small>{activePeriod.approvedBy || "TЙ™sdiq edЙ™n yoxdur"}</small>
            </div>
            <div className="kpi-period-card">
              <span>Payout</span>
              <StatusBadge status={activePeriod.payoutStatus || "GГ¶zlЙ™yir"} />
          ...
          {lastPayout && (
            <div className="kpi-last-payout">
              ...
              <small>{lastPayout.period} В· {lastPayout.at}</small>
            </div>
          )}
        </Panel>
      </section>
      ...
      <Panel className="kpi-payout-plan-panel" data-testid="kpi-payout-plan-panel">
        <PanelHeader title="ЖЏmЙ™kdaЕџ payout planД±" subtitle="HR performans bonusu vЙ™ satД±Еџ bonuslarД±nД±n yekun Г¶dЙ™niЕџ siyahД±sД±" icon={Wallet} />
        <DataTable
          columns={["ЖЏmЙ™kdaЕџ", "KPI", "SatД±Еџ", "Performans bonusu", "SatД±Еџ bonusu", "Payout", "Status"]}
          rows={rankingSource.map((row) => [
            <AvatarLine initials={row.initials} title={row.name} subtitle={`${row.department} В· ${row.position}`} />,
            <strong>{percent(row.kpi || 0)}</strong>,
            <TwoLine title={`${row.salesOrders || 0} sifariЕџ`} subtitle={money(row.salesPaid || 0)} />,
      ...
      <section className="dashboard-grid">
        <Panel>
          <PanelHeader title="ЖЏmЙ™kdaЕџ reytinqi" subtitle="KPI nЙ™ticЙ™lЙ™rinЙ™ gГ¶rЙ™" />
          <div className="rank-list">
            ...
            {ranking.length === 0 && <EmptyState title="ЖЏmЙ™kdaЕџ mЙ™lumatД± yoxdur" />}
          </div>
        </Panel>
        <Panel className="kpi-period-history-panel">
          <PanelHeader title="Period tarixГ§Й™si" subtitle="BaДџlanmД±Еџ KPI periodlarД± vЙ™ payout statusu" icon={CalendarClock} />
          <DataTable
            columns={["Period", "Score", "Payout", "TЙ™sdiq", "Г–dЙ™niЕџ"]}
            rows={periodHistory.map((period) => [
              <TwoLine title={period.period} subtitle={period.closedAt || "AГ§Д±q"} />,
      ...
      <Panel className="kpi-bonus-panel">
        <PanelHeader
          title="SatД±Еџdan gЙ™lЙ™n bonuslar"
          subtitle="SatД±Еџ sifariЕџindЙ™ qeyd olunan satД±cД± bonus faizlЙ™rinЙ™ gГ¶rЙ™ hesablanД±r"
          icon={Wallet}
        />
        <div className="kpi-bonus-toolbar">
          <div className="tabs" aria-label="SatД±Еџ bonusu filtri">
            {["HamД±sД±", ...bonusSellers].map((filter) => (
              <button
                key={filter}
          ...
          <div className="kpi-bonus-total">
            <span>SeГ§ilmiЕџ bonus</span>
            <strong>{money(visibleBonusTotal)}</strong>
          </div>
        </div>
        <DataTable
          columns={["SifariЕџ", "SatД±cД±", "MГјЕџtЙ™ri", "MЙ™hsul", "Г–dЙ™niЕџ", "% bonus", "Bonus", "Status"]}
...
function SupportPage({
  ...
  const openTickets = enrichedTickets.filter((ticket) => ticket.status !== "BaДџlandД±");
  const highPriority = enrichedTickets.filter((ticket) => ticket.priority === "YГјksЙ™k");
  ...
  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="AГ§Д±q sorДџu" value={openTickets.length} icon={MessageSquare} tone="primary" />
        <MetricCard label="YГјksЙ™k prioritet" value={highPriority.length} icon={CircleAlert} tone={highPriority.length ? "warning" : "success"} />
        <MetricCard label="Orta SLA" value={`${avgSla} saat`} icon={CalendarClock} tone="info" />
        <MetricCard label="BaДџlД± task" value={linkedTickets.length} trend={`${new Set(enrichedTickets.map((ticket) => ticket.module)).size} modul`} icon={ShieldCheck} tone="success" />
      </section>
      <section className="support-workspace">
        <Panel className="support-panel support-queue-panel" data-testid="support-task-panel">
          <PanelHeader title="Support vЙ™ task nГ¶vbЙ™si" subtitle="SifariЕџ, kredit vЙ™ mГјЕџtЙ™riyЙ™ baДџlД± operativ tapЕџД±rД±qlar" icon={MessageSquare} />
          <DataTable
            columns={["SorДџu", "BaДџlantД±", "Prioritet", "Comment", "MЙ™sul", "SLA", "Status", "AГ§"]}
            rows={enrichedTickets.map((ticket) => [
              <TwoLine title={ticket.title} subtitle={`${ticket.id} В· ${ticket.createdAt}`} />,
            ...
            <>
              <div className="support-detail-head">
                <div>
                  ...
                  <p>{selected.id} В· {selected.requester} В· {selected.createdAt}</p>
                </div>
                <select value={selected.status} onChange={(event) => onUpdateStatus(selected.id, event.target.value)}>
                  <option>AГ§Д±q</option>
                  <option>Д°crada</option>
                  <option>GГ¶zlЙ™yir</option>
                  <option>BaДџlandД±</option>
                </select>
              </div>
              <div className="support-link-grid">
                <div>
                  <span>MГјЕџtЙ™ri</span>
                  <strong>{selected.customer || selected.customerRecord?.name || "вЂ”"}</strong>
                ...
                <div>
                  <span>SifariЕџ</span>
                  <strong>{selected.orderId || selected.order?.id || "вЂ”"}</strong>
                ...
                <div>
                  <span>Kredit</span>
                  <strong>{selected.creditId || selected.credit?.id || "вЂ”"}</strong>
              ...
              <div className="support-task-list">
                {(selected.tasks || []).map((task) => (
                  <div key={task.id}>
                    <TwoLine title={task.title} subtitle={`${task.id} В· ${task.owner} В· ${task.dueAt}`} />
              ...
              <div className="support-comment-composer">
                <textarea
                  ...
                  placeholder="Task ГјzrЙ™ comment yazД±n..."
        ...
        {selected ? (
          <>
            <div className="chat-head">
              <AvatarLine initials={selected.initials} title={selected.person} subtitle={`${selected.team} ЕџГ¶bЙ™si В· onlayn`} />
              {(selected.ticketId || selected.orderId || selected.creditId || selected.customerFin) && (
                <div className="message-context-strip">
                  ...
                  {selected.customerFin && <StatusBadge status={`MГјЕџtЙ™ri ${selected.customerFin}`} />}
                </div>
              )}
            </div>
            ...
            <div className="composer">
              <input
                ...
                placeholder="Mesaj yazД±n..."
                disabled={!canSend}
                title={!canSend ? "Daxili mesaj gГ¶ndЙ™rmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : ""}
              />
              <button
                className="primary-btn icon-only"
                onClick={sendMessage}
                aria-label="Mesaj gГ¶ndЙ™r"
                disabled={!canSend}
                title={!canSend ? "Daxili mesaj gГ¶ndЙ™rmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : ""}
              >
                <Send size={17} />
          ...
          <EmptyState title="Mesaj tapД±lmadД±" />
        )}
      </Panel>
    </section>
  );
...
function MessagesPageV2({
  ...
  const filters = [
    ...
    { id: "unread", label: "OxunmamД±Еџ", count: counts.unread },
    { id: "groups", label: "Qruplar", count: counts.groups },
    { id: "linked", label: "BaДџlД±", count: counts.linked },
    { id: "archived", label: "Arxiv", count: counts.archived },
    { id: "all", label: "HamД±sД±", count: counts.all },
  ];
  ...
  return (
    <section className="messages-workspace">
      <div className="messages-summary-grid">
        <MetricCard label="Aktiv sГ¶hbЙ™t" value={counts.active} trend={`${counts.unread} oxunmamД±Еџ`} icon={MessageSquare} tone="primary" />
        <MetricCard label="Qrup" value={counts.groups} trend="Daxili komanda kanallarД±" icon={Users} tone="success" />
        <MetricCard label="BaДџlД± thread" value={counts.linked} trend="SifariЕџ/kredit/task" icon={GitBranch} tone="info" />
        <MetricCard label="Arxiv" value={counts.archived} trend="BaДџlanmД±Еџ yazД±Еџmalar" icon={FileText} tone="warning" />
      </div>
      ...
      <section className="messages-layout">
        <Panel className="message-list-panel">
          <div className="message-list-head">
            <div>
              <h3>Inbox</h3>
              <p>{currentUser?.name || "Д°stifadЙ™Г§i"} ГјГ§Гјn daxili yazД±Еџmalar</p>
            </div>
          ...
          {composerOpen && (
            <div className="message-thread-form">
              ...
              <label className="message-field">
                <span>{newThread.type === "group" ? "Qrup adД±" : "BaЕџlД±q"}</span>
                <input
                  value={newThread.title}
                  onChange={(event) => setNewThread((current) => ({ ...current, title: event.target.value }))}
                  placeholder={newThread.type === "group" ? "MЙ™s: SatД±Еџ komandasД±" : "BoЕџ qala bilЙ™r"}
                />
              </label>

              <label className="message-field">
                <span>ЕћГ¶bЙ™ / kanal</span>
                <input
                  value={newThread.team}
                  onChange={(event) => setNewThread((current) => ({ ...current, team: event.target.value }))}
                  placeholder="SatД±Еџ, Anbar, MaliyyЙ™..."
                />
              </label>
              ...
              <div className="message-participant-picker">
                ...
                {selectedParticipantNames.length === 0 && <small>ЖЏn azД± bir iЕџtirakГ§Д± seГ§in.</small>}
              </div>

              <label className="message-field">
                <span>Д°lk mesaj</span>
                <textarea
                  value={newThread.firstMessage}
                  onChange={(event) => setNewThread((current) => ({ ...current, firstMessage: event.target.value }))}
                  placeholder="Д°stЙ™yЙ™ baДџlД± baЕџlanДџД±c mesajД±..."
                />
              </label>
          ...
          <div className="conversation-list">
            {visibleConversations.map((conversation) => (
              ...
              >
                ...
                <div className="conversation-meta">
                  <span>{conversation.time}</span>
                  <small>{conversation.type === "group" ? "Qrup" : conversation.ticketId ? "Task" : "ЕћЙ™xsi"}</small>
                  {conversation.unread > 0 && <strong>{conversation.unread}</strong>}
                </div>
              </button>
            ))}
            {visibleConversations.length === 0 && <EmptyState title="Bu filter ГјzrЙ™ sГ¶hbЙ™t yoxdur" />}
          </div>
        </Panel>
            ...
            <>
              <div className="chat-head">
                <div className="chat-head-main">
                  <AvatarLine
                    ...
                    subtitle={`${selected.team} В· ${selected.participants.length || 1} iЕџtirakГ§Д±`}
                  />
                  <div className="chat-head-actions">
                    ...
                    <button className="secondary-btn compact" onClick={() => onArchiveConversation?.(selected.id)} disabled={!canManage}>
                      {selected.archived ? "Aktiv et" : "ArxivlЙ™"}
                    </button>
                    <button
                      className="secondary-btn compact danger-soft"
                      onClick={() => {
                        if (window.confirm("Bu sГ¶hbЙ™t silinsin?")) onDeleteConversation?.(selected.id);
                      }}
                      disabled={!canManage}
                    >
                      <Trash2 size={15} />
                  ...
                  <div className="message-context-strip">
                    ...
                    {selected.customerFin && (
                      <button className="secondary-btn compact" onClick={() => onOpenCustomer?.(selected.customerFin)}>
                        MГјЕџtЙ™ri {selected.customerFin}
                      </button>
                    )}
                  </div>
                )}
              ...
              <div className="chat-body">
                {selectedMessages.map((message, index) => (
                  <div key={message.id || `${message.time}-${index}`} className={`bubble ${message.mine ? "mine" : ""}`}>
                    <div className="bubble-author">{message.from || "Д°stifadЙ™Г§i"}</div>
                    <p>{message.text}</p>
                    <span>{message.time} В· {message.status || (message.readAt ? "Oxundu" : "GГ¶ndЙ™rildi")}</span>
                  </div>
                ))}
                {selectedMessages.length === 0 && <EmptyState title="Bu sГ¶hbЙ™tdЙ™ hЙ™lЙ™ mesaj yoxdur" />}
              </div>
              ...
              <div className="composer">
                ...
                <button
                  ...
                  aria-label="Mesaj gГ¶ndЙ™r"
                  disabled={!canSend || selected.archived}
                  title={selected.archived ? "Arxiv sГ¶hbЙ™tЙ™ mesaj yazmaq ГјГ§Гјn Й™vvЙ™l aktiv edin" : !canSend ? "Daxili mesaj gГ¶ndЙ™rmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : ""}
                >
                  <Send size={17} />
                </button>
              </div>
...
function NotificationsPage({
  ...
  const filters = ["CЙ™mi", "Kredit", "Push", "SMS", "Email", "OxunmamД±Еџ"];
  const list = notifications.filter((item) => {
    if (filter === "CЙ™mi") return true;
    if (filter === "OxunmamД±Еџ") return item.unread;
  ...
  const sentCount = sendLog.filter((row) => row.status === "GГ¶ndЙ™rildi").length;
  const blockedCount = sendLog.filter((row) => row.status !== "GГ¶ndЙ™rildi").length;

  return (
    <div className="stack">
      <section className="metric-grid four">
        <MetricCard label="HazД±r nГ¶vbЙ™" value={queueTotal} trend={`${eventTotal} hadisЙ™ В· ${cooldownTotal} cooldown`} icon={Bell} tone={queueTotal ? "warning" : "success"} />
        ...
        <MetricCard label="GГ¶ndЙ™rildi" value={sentCount} trend={dispatchSnapshot ? `Son: ${dispatchSnapshot.sent}` : "Log ГјzrЙ™"} icon={Send} tone="success" />
        <MetricCard label="BloklandД±" value={blockedCount} trend={dispatchSnapshot ? `${dispatchSnapshot.source || "Son"}: ${dispatchSnapshot.blocked}` : "Provider/kanal"} icon={CircleAlert} tone={blockedCount ? "danger" : "info"} />
      </section>
      <Panel>
        <div className="filter-bar">
          ...
          <div className="notification-toolbar-actions">
            <button
              ...
              title={!canManage ? "BildiriЕџlЙ™ri idarЙ™ etmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : queueTotal === 0 ? "GГ¶ndЙ™riЕџ nГ¶vbЙ™si boЕџdur" : ""}
            ...
            <button
              ...
              onClick={markAll}
              disabled={!canManage}
              title={!canManage ? "BildiriЕџlЙ™ri idarЙ™ etmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : ""}
            >
              <Check size={16} />
              OxunmuЕџ et
            </button>
        ...
        {(lastSweepAt || dispatchSnapshot) && (
          <div className="module-action-note">
            ...
            <span>
              {dispatchSnapshot
                ? `Son gГ¶ndЙ™riЕџ: ${dispatchSnapshot.at} В· ${dispatchSnapshot.source || "NГ¶vbЙ™"} В· ${dispatchSnapshot.sent} gГ¶ndЙ™rildi В· ${dispatchSnapshot.blocked} bloklandД±`
                : `Son oxunma yoxlamasД±: ${lastSweepAt}`}
            </span>
          </div>
        )}
      </Panel>
      <Panel className="notification-provider-panel" data-testid="notification-provider-panel">
        <PanelHeader
          title="Provider baДџlantД±larД±"
          subtitle="SMS, email vЙ™ push kanallarД± ГјГ§Гјn endpoint, secret statusu vЙ™ gГ¶ndЙ™riЕџ saДџlamlД±ДџД±"
          icon={ShieldCheck}
        />
        <DataTable
          columns={["Kanal", "Provider", "Endpoint", "Sender", "Rejim", "Secret", "GГ¶ndЙ™rilib", "Status"]}
          rows={providerRows.map((provider) => [
            ...
            <TwoLine title={provider.sentCount || 0} subtitle={provider.lastSentAt || "HЙ™lЙ™ yoxdur"} />,
      ...
      <Panel className="notification-automation-panel">
        <PanelHeader
          title="Avtomatik xatД±rlatma qaydalarД±"
          subtitle="Kredit, anbar, PO, payroll vЙ™ tЙ™hvil SLA siqnallarД± ГјГ§Гјn gГ¶ndЙ™riЕџ nГ¶vbЙ™si"
          icon={Bell}
        />
        <DataTable
          columns={["Qayda", "Kanal", "Provider", "Trigger", "HazД±r/Cooldown", "Son hadisЙ™", "Prioritet", "Son run", "Status"]}
          rows={automationRows.map((rule) => [
            <TwoLine title={rule.name} subtitle={rule.id} />,
            <StatusBadge status={rule.channel} />,
            rule.providerName,
            rule.trigger,
            <TwoLine title={`${rule.queueCount} hazД±r`} subtitle={`${rule.cooldownCount || 0} cooldown / ${rule.totalEventCount || 0} hadisЙ™`} />,
            rule.lastEvent,
            rule.events?.[0]?.priority || rule.cooldownEvents?.[0]?.priority || "вЂ”",
            rule.lastRunAt || "вЂ”",
      ...
      <Panel className="notification-sendlog-panel" data-testid="notification-sendlog-panel">
        <PanelHeader title="GГ¶ndЙ™riЕџ logu" subtitle="Provider cavabД±, kanal, alД±cД± vЙ™ baДџlД± modul ГјzrЙ™ son gГ¶ndЙ™riЕџlЙ™r" icon={Send} />
        <DataTable
          columns={["Tarix", "Kanal", "Provider", "AlД±cД±", "MЙ™nbЙ™", "Son tarix", "MЙ™tn", "Status"]}
      ...
      <Panel>
        <PanelHeader title="Sistem iГ§i xЙ™bЙ™rdarlД±qlar" subtitle="GГ¶ndЙ™riЕџlЙ™r vЙ™ modul hadisЙ™lЙ™ri ГјzrЙ™ daxili bildiriЕџ axД±nД±" icon={Bell} />
        <div className="notification-list">
          ...
          {list.length === 0 && <EmptyState title="Bu filtrdЙ™ bildiriЕџ yoxdur" />}
        </div>
      </Panel>
    </div>
  );
...
export function Toggle({ label, checked, onChange, disabled = false }) {
  return (
    <button className="toggle-row" onClick={onChange} disabled={disabled} title={disabled ? "Bu ayarД± dЙ™yiЕџmЙ™k ГјГ§Гјn icazЙ™ yoxdur" : ""}>
      <span>{label}</span>
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card">
        <div className="modal-head">
          <div>
            <h2>{mode === "edit" ? "AnbarД± redaktЙ™ et" : "Yeni anbar yarat"}</h2>
            <p>Anbar adД±, kodu, ГјnvanД±, mЙ™sul ЕџЙ™xsi vЙ™ tutum mЙ™lumatlarД±nД± daxil edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="modal-form">
          ...
          <label>
            ...
            <select value={values.type} onChange={(event) => updateValue("type", event.target.value)}>
              <option>MЙ™rkЙ™zi</option>
              <option>Regional</option>
              <option>TЙ™hvil</option>
...
function ProductFormModal({ product, onClose, onSubmit, onDelete }) {
  const [values, setValues] = useState({
    ...
    unit: product?.unit || "Й™dЙ™d",
    costPrice: product?.costPrice || 0,
    salePrice: product?.salePrice || 0,
    reorderLevel: product?.reorderLevel || 0,
    serialTracked: product?.serialTracked ? "BЙ™li" : "Xeyr",
  });
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card">
        <div className="modal-head">
          <div>
            <h2>{product ? "MЙ™hsulu redaktЙ™ et" : "Yeni mЙ™hsul"}</h2>
            <p>SKU, qiymЙ™t, minimum stok vЙ™ serial izlЙ™mЙ™ qaydasД±nД± tЙ™yin edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="modal-form">
          ...
          <label>
            ...
            <select value={values.category} onChange={(event) => updateValue("category", event.target.value)}>
              ...
              <option>MЙ™iЕџЙ™t texnikasД±</option>
              <option>Aksesuar</option>
              <option>XidmЙ™t</option>
              <option>DigЙ™r</option>
            </select>
          </label>
          <label>
            <span>Г–lГ§Гј vahidi</span>
            <select value={values.unit} onChange={(event) => updateValue("unit", event.target.value)}>
              <option>Й™dЙ™d</option>
              <option>qutu</option>
              <option>kg</option>
              <option>metr</option>
              <option>litr</option>
          ...
          <div className="modal-actions">
            ...
            <button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button>
            <button type="submit" className="primary-btn">
              <Check size={16} />
              {product ? "Yadda saxla" : "MЙ™hsul yarat"}
            </button>
          </div>
        </form>
      </div>
...
function HrEmployeeModal({ employee = null, employees = [], departments: departmentRecords = [], onClose, onSubmit }) {
  ...
  const [values, setValues] = useState({
    ...
    employmentType: employee?.employmentType || "Tam Еџtat",
    leaveBalance: employee?.leaveBalance ?? "0",
    documentsComplete: String(savedDocumentsComplete),
    hrStatus: employee?.hrStatus === "MЙ™lumat gГ¶zlЙ™yir" ? "MЙ™lumat gГ¶zlЙ™yir" : "Stabil",
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card hr-employee-modal">
        <div className="modal-head">
          <div>
            <h2>{employee ? "ЖЏmЙ™kdaЕџД± redaktЙ™ et" : "Yeni Й™mЙ™kdaЕџ"}</h2>
            <p>{employee ? "ЖЏmЙ™kdaЕџД±n ЕџЙ™xsi, iЕџ vЙ™ tabeГ§ilik mЙ™lumatlarД±nД± yenilЙ™yin." : "ЕћГ¶bЙ™ vЙ™ tabeГ§ilik mЙ™lumatД±nД± daxil edin."}</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <form className="modal-form" onSubmit={submit}>
          ...
          <label><span>VЙ™zifЙ™</span><input value={values.position} required onChange={(event) => updateValue("position", event.target.value)} /></label>
          <label>
            <span>ЕћГ¶bЙ™</span>
          ...
          <label>
            ...
            <select value={values.managerId} onChange={(event) => updateValue("managerId", event.target.value)}>
              <option value="">BirbaЕџa rЙ™hbЙ™rlik</option>
              {managerOptions.map((manager) => <option key={getEmployeeKey(manager)} value={getEmployeeKey(manager)}>{manager.name} В· {manager.position}</option>)}
            </select>
          </label>
          <label>
            <span>SЙ™viyyЙ™</span>
          ...
          <label><span>MaaЕџ</span><input type="number" min="0" value={values.salary} required onChange={(event) => updateValue("salary", event.target.value)} /></label>
          <label><span>KPI</span><input type="number" min="0" value={values.kpi} onChange={(event) => updateValue("kpi", event.target.value)} /></label>
          <label><span>SЙ™nЙ™d uyДџunluДџu, %</span><input type="number" min="0" max="100" value={values.documentsComplete} onChange={(event) => updateValue("documentsComplete", event.target.value)} /></label>
          <label><span>HR statusu</span><select value={values.hrStatus} onChange={(event) => updateValue("hrStatus", event.target.value)}><option>Stabil</option><option>MЙ™lumat gГ¶zlЙ™yir</option></select></label>
          <label><span>Д°ЕџЙ™ qЙ™bul tarixi</span><input type="date" value={values.hireDate} onChange={(event) => updateValue("hireDate", event.target.value)} /></label>
          <label><span>Д°Еџ rejimi</span><select value={values.workMode} onChange={(event) => updateValue("workMode", event.target.value)}><option>Ofis</option><option>Hybrid</option><option>SahЙ™</option><option>Uzaqdan</option></select></label>
          <label><span>NГ¶vbЙ™</span><input value={values.shift} onChange={(event) => updateValue("shift", event.target.value)} /></label>
          <label><span>MЙ™ЕџДџulluq tipi</span><select value={values.employmentType} onChange={(event) => updateValue("employmentType", event.target.value)}><option>Tam Еџtat</option><option>YarД±m Еџtat</option><option>MГјqavilЙ™li</option><option>SД±naq mГјddЙ™ti</option></select></label>
          <label><span>MЙ™zuniyyЙ™t balansД±</span><input type="number" min="0" value={values.leaveBalance} onChange={(event) => updateValue("leaveBalance", event.target.value)} /></label>
          <label className="full"><span>BacarД±qlar</span><input value={values.skills} onChange={(event) => updateValue("skills", event.target.value)} /></label>
          <div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button><button type="submit" className="primary-btn">{employee ? <Check size={16} /> : <Plus size={16} />}{employee ? "Yadda saxla" : "ЖЏmЙ™kdaЕџ yarat"}</button></div>
        </form>
      </div>
    </div>
  );
    ...
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card hr-department-modal">
        <div className="modal-head">
          <div>
            <h2>Yeni ЕџГ¶bЙ™</h2>
            <p>ЕћГ¶bЙ™ni struktur aДџacД±na Й™lavЙ™ edin vЙ™ istЙ™sЙ™niz onu Гјst ЕџГ¶bЙ™yЙ™ baДџlayД±n.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <form className="modal-form" onSubmit={submit}>
          <label><span>ЕћГ¶bЙ™ adД±</span><input value={values.name} required autoFocus onChange={(event) => updateValue("name", event.target.value)} /></label>
          <label>
            <span>Гњst ЕџГ¶bЙ™</span>
          ...
          <label className="full"><span>QД±sa izah</span><textarea value={values.description} onChange={(event) => updateValue("description", event.target.value)} /></label>
          <label><span>Status</span><select value={values.status} onChange={(event) => updateValue("status", event.target.value)}><option>Aktiv</option><option>PlanlanД±r</option><option>Passiv</option></select></label>
          <div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button><button type="submit" className="primary-btn"><Plus size={16} /> ЕћГ¶bЙ™ Й™lavЙ™ et</button></div>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card hr-delete-modal">
        <div className="modal-head">
          <div>
            <h2>ЖЏmЙ™kdaЕџД± sil</h2>
            <p>Bu Й™mЙ™liyyat Й™mЙ™kdaЕџД± HR reyestrindЙ™n silir vЙ™ audit izini saxlayД±r.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <div className="hr-delete-summary">
          <AvatarLine initials={employee.initials} title={employee.name} subtitle={`${employee.position} В· ${employee.department}`} />
          <span>{directReports.length ? `${directReports.length} Й™mЙ™kdaЕџ bu ЕџЙ™xsЙ™ tabedir` : "BirbaЕџa tabe Й™mЙ™kdaЕџ yoxdur"}</span>
        </div>
        {directReports.length > 0 && (
          <label className="hr-delete-reassignment">
            <span>Tabe Й™mЙ™kdaЕџlarД±n yeni rЙ™hbЙ™ri</span>
            <select value={replacementManagerId} onChange={(event) => setReplacementManagerId(event.target.value)}>
              <option value="">BirbaЕџa rЙ™hbЙ™rlik</option>
              {replacementOptions.map((manager) => <option key={getEmployeeKey(manager)} value={getEmployeeKey(manager)}>{manager.name} В· {manager.position}</option>)}
            </select>
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card hr-operation-modal">
        <div className="modal-head">
          <div>
            <h2>MЙ™zuniyyЙ™t qeydi</h2>
            <p>ЖЏmЙ™kdaЕџ, mЙ™zuniyyЙ™t nГ¶vГј vЙ™ tarix aralД±ДџД±nД± qeyd edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <form className="modal-form" onSubmit={(event) => { event.preventDefault(); onSubmit(values); }}>
          <label><span>ЖЏmЙ™kdaЕџ</span><select value={values.employeeId} required onChange={(event) => updateValue("employeeId", event.target.value)}><option value="">ЖЏmЙ™kdaЕџ seГ§in</option>{employees.map((employee) => <option key={getEmployeeKey(employee)} value={getEmployeeKey(employee)}>{employee.name} В· {employee.department}</option>)}</select></label>
          <label><span>MЙ™zuniyyЙ™t nГ¶vГј</span><select value={values.type} onChange={(event) => updateValue("type", event.target.value)}><option>Д°llik mЙ™zuniyyЙ™t</option><option>Г–dЙ™niЕџsiz mЙ™zuniyyЙ™t</option><option>XЙ™stЙ™lik vЙ™rЙ™qЙ™si</option><option>EzamiyyЙ™t</option></select></label>
          <label><span>BaЕџlanДџД±c tarixi</span><input type="date" value={values.from} required onChange={(event) => updateValue("from", event.target.value)} /></label>
          <label><span>BitmЙ™ tarixi</span><input type="date" value={values.to} required onChange={(event) => updateValue("to", event.target.value)} /></label>
          <div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button><button type="submit" className="primary-btn"><CalendarClock size={16} /> Qeyd yarat</button></div>
        </form>
      </div>
    </div>
  );
      ...
      <div className="modal-card hr-operation-modal">
        <div className="modal-head">
          <div>
            ...
            <p>Rol, ЕџГ¶bЙ™ vЙ™ hЙ™dЙ™f tarixi qeyd etmЙ™klЙ™ recruitment pipeline baЕџlayД±n.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <form className="modal-form" onSubmit={(event) => { event.preventDefault(); onSubmit(values); }}>
          <label><span>Rol</span><input value={values.role} required autoFocus onChange={(event) => updateValue("role", event.target.value)} /></label>
          <label><span>ЕћГ¶bЙ™</span><input value={values.department} list="vacancy-departments" required onChange={(event) => updateValue("department", event.target.value)} /><datalist id="vacancy-departments">{departmentOptions.map((department) => <option key={department} value={department} />)}</datalist></label>
          <label><span>Owner</span><input value={values.owner} required onChange={(event) => updateValue("owner", event.target.value)} /></label>
          <label><span>HЙ™dЙ™f tarixi</span><input type="date" value={values.targetDate} onChange={(event) => updateValue("targetDate", event.target.value)} /></label>
          <div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button><button type="submit" className="primary-btn"><Plus size={16} /> Vakansiya yarat</button></div>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card">
        <div className="modal-head">
          <div>
            <h2>{account ? "HesabД± redaktЙ™ et" : "Yeni maliyyЙ™ hesabД±"}</h2>
            <p>Kassa vЙ™ bank aГ§Д±lД±Еџ balansД±nД± dГјzgГјn qeyd edin; bu dЙ™yЙ™r maliyyЙ™ hesabatlarД±na daxil olur.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        <form onSubmit={submit} className="modal-form">
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card">
        <div className="modal-head">
          <div>
            <h2>Д°lkin mЙ™daxil</h2>
            <p>Д°lk mЙ™hsulu seГ§ilmiЕџ anbara daxil edin. MЙ™hsul avtomatik Гјmumi stokda da gГ¶rГјnЙ™cЙ™k.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="modal-form">
          <label className="full">
            ...
            >
              {warehouses.map((warehouse) => (
                <option key={warehouse.id} value={warehouse.id}>
                  {warehouse.name} В· {warehouse.city}
                </option>
              ))}
            </select>
          </label>
          <label className="full">
            ...
            {products.length > 0 ? (
              ...
              >
                <option value="">MЙ™hsul seГ§in</option>
                {products.filter((product) => product.status !== "Passiv").map((product) => (
                  <option key={product.id} value={product.name}>{product.sku} В· {product.name}</option>
                ))}
              </select>
            ) : (
              <input
...
function VendorFormModal({ vendor, onClose, onSubmit, onDelete }) {
  ...
  const [values, setValues] = useState({
    ...
    paymentTerms: vendor?.paymentTerms || "30 gГјn",
    note: vendor?.note || "",
  });
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card vendor-form-modal">
        <div className="modal-head">
          <div>
            <h2>{vendor ? "Vendoru redaktЙ™ et" : "Yeni vendor"}</h2>
            <p>Vendor mЙ™lumatlarД±, kontakt, kvota vЙ™ tЙ™chizat ЕџЙ™rtlЙ™rini bir yerdЙ™ idarЙ™ edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        {vendor && (
          <div className="vendor-form-summary">
            <TwoLine title={normalizedVendor.name} subtitle={`${normalizedVendor.country} В· ${normalizedVendor.sku} SKU`} />
        ...
        <form onSubmit={submit} className="modal-form">
          ...
          <label>
            ...
            <select value={values.status} onChange={(event) => updateValue("status", event.target.value)}>
              <option>Aktiv</option>
              <option>NЙ™zarЙ™t</option>
              <option>Risk</option>
              <option>Passiv</option>
            </select>
          </label>
          <label>
            <span>Kontakt ЕџЙ™xs</span>
...
function FactoryPurchaseOrderModal({
  ...
  function submit(event) {
    ...
    const saved = onSubmit({
      ...
      procurementType: "Zavod sifariЕџi",
    });
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card factory-order-modal">
        <div className="modal-head">
          <div>
            <h2>Zavod sifariЕџi yarat</h2>
            <p>MЙ™hsulun hardan alД±ndД±ДџД±nД±, sayД±nД±, alД±Еџ qiymЙ™tini vЙ™ gГ¶zlЙ™nЙ™n mЙ™daxil tarixini qeyd edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="modal-form">
          ...
          <div className="factory-order-snapshot full">
            <div><span>SatД±Еџ ГјГ§Гјn</span><strong>{snapshot.available}</strong></div>
            <div><span>Minimum</span><strong>{snapshot.reorderPoint || "вЂ”"}</strong></div>
            <div><span>AГ§Д±q sifariЕџ</span><strong>{snapshot.orderedQty}</strong></div>
            <div><span>TЙ™klif</span><strong>{snapshot.orderGap || snapshot.suggestedQty || 1}</strong></div>
          </div>
          <label>
            <span>Haradan alД±nД±r</span>
            <input
          ...
          <label>
            ...
            <select value={values.warehouseId} required onChange={(event) => updateValue("warehouseId", event.target.value)}>
              {warehouses.map((warehouse) => (
                <option key={warehouse.id} value={warehouse.id}>{warehouse.name} В· {warehouse.city}</option>
              ))}
            </select>
          </label>
          <label>
          ...
          <label>
            <span>Stok/satД±Еџ qiymЙ™ti</span>
          ...
          <label>
            <span>GГ¶zlЙ™nЙ™n tarix</span>
            <input type="date" value={values.expectedAt} onChange={(event) => updateValue("expectedAt", event.target.value)} />
          </label>
          <label className="full">
            <span>Qeyd</span>
            <input value={values.note} placeholder="Zavod partiyasД±, invoice vЙ™ ya Г§atdД±rД±lma qeydi" onChange={(event) => updateValue("note", event.target.value)} />
          </label>
          <div className="factory-order-total full">
            <span>Toplam alД±Еџ mЙ™blЙ™Дџi</span>
            <strong>{money(amount)}</strong>
          </div>
          <div className="modal-actions">
            <button type="button" className="secondary-btn" onClick={onClose}>LЙ™Дџv et</button>
...
function WarehouseImportModal({ warehouses, onClose, onImport }) {
  ...
  async function readFile(file) {
    ...
    try {
      ...
      setAnalysis({ rows: [], errors: ["CSV faylД± oxunmadД±."] });
    } finally {
      setIsReading(false);
    }
  }
      ...
      <div className="modal-card warehouse-import-card">
        <div className="modal-head">
          <div>
            ...
            <p>CSV faylД±ndan anbar qalД±qlarД±nД± Й™lavЙ™ edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla"><X size={18} /></button>
        </div>
        ...
        {(analysis.rows.length > 0 || analysis.errors.length > 0) && (
          <>
            <div className="warehouse-import-summary">
              <strong>{analysis.rows.length} etibarlД± sЙ™tir</strong>
              <span>{analysis.errors.length} xЙ™ta</span>
            </div>
            {analysis.rows.length > 0 && (
              <DataTable
                columns={["MЙ™hsul", "SKU", "Anbar", "Miqdar", "SatД±Еџ", "Maya", "Minimum"]}
                rows={analysis.rows.slice(0, 8).map((row) => [
                  <strong>{row.product}</strong>,
                  row.sku || "Avtomatik",
                  row.warehouseName,
                  row.qty,
                  row.salePrice === null ? "вЂ”" : money(row.salePrice),
                  row.costPrice === null ? "вЂ”" : money(row.costPrice),
                  row.reorderLevel === null ? "вЂ”" : row.reorderLevel,
                ])}
              />
            )}
            {analysis.errors.length > 0 && (
              <div className="warehouse-import-errors">
                ...
                {analysis.errors.length > 5 && <span>vЙ™ daha {analysis.errors.length - 5} xЙ™ta</span>}
              </div>
            )}
          </>
        )}
...
function SalesOperationModal({ order, orderOptions, onClose, onSubmit }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState(order.paymentMethod || "NaДџd");
    ...
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card order-modal-card">
        <div className="modal-head order-modal-head">
          <div>
            <h2>SatД±Еџ Й™mЙ™liyyatД±nД± redaktЙ™ et</h2>
            <p>{order.id} ГјzrЙ™ mГјЕџtЙ™ri, Г¶dЙ™niЕџ, bonus vЙ™ rezerv mЙ™lumatlarД±nД± yenilЙ™yin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="order-modal-form">
          <section className="order-section">
            <label className="order-label">MГњЕћTЖЏRД° VЖЏ Г–DЖЏNД°Ећ</label>
            <div className="order-two-col">
              ...
              <select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}>
                <option>NaДџd</option>
                <option>Kart</option>
                <option>KГ¶Г§ГјrmЙ™</option>
                <option>Kredit</option>
              </select>
            </div>
            <div className="order-two-col">
              <label className="order-sub-field">
                <span>MГјЕџtЙ™ri adД±</span>
            ...
            <label className="order-sub-field">
              ...
              <select value={warehouseId} onChange={(event) => changeWarehouse(event.target.value)} disabled={delivered}>
                {warehouses.map((warehouse) => (
                  <option key={warehouse.id} value={warehouse.id}>
                    {warehouse.name} вЂ” {warehouse.city}
                  </option>
                ))}
              </select>
            </label>
          ...
          <section className="order-section">
            <div className="section-title-row">
              <span className="order-label">MЖЏHSULLAR</span>
            ...
            <div className="order-lines">
              {productRows.map((row) => (
                <div className="order-line-grid" key={row.id}>
                  <select value={row.product} onChange={(event) => changeProduct(row.id, "product", event.target.value)} disabled={delivered}>
                    {availableStock.map((item) => (
                      <option key={item.product} value={item.product}>
                        {item.product} вЂ” {getAvailableQuantity(item)} satД±Еџ ГјГ§Гјn
                      </option>
                    ))}
                  </select>
                  ...
                  <button type="button" className="line-delete" onClick={() => removeProductRow(row.id)} disabled={delivered} aria-label="MЙ™hsul sЙ™trini sil">
                    <Trash2 size={17} />
                  </button>
                </div>
              ))}
            ...
            <div className="order-total edit-order-total">
              <span>SЙ™tir cЙ™mi: {money(lineTotal)}</span>
              <label>
                <span>Yekun mЙ™blЙ™Дџ</span>
            ...
            <div className="order-lines">
              {sellerRows.map((row) => (
                <div className="seller-line-grid" key={row.id}>
                  ...
                  <button type="button" className="line-delete" onClick={() => removeSellerRow(row.id)} aria-label="SatД±cД± sЙ™trini sil">
                    <Trash2 size={17} />
                  </button>
                </div>
              ))}
            ...
            <div className="order-two-col">
              <label className="order-sub-field">
                ...
                <select value={status} onChange={(event) => setStatus(event.target.value)}>
                  {stages
                    .filter((stage) => delivered || stage !== "TЙ™hvil verilib")
                    .map((stage) => (
...
function ExpenseOperationModal({ expense, onClose, onSubmit }) {
  const [values, setValues] = useState({
    ...
    status: expense.status || "TЙ™sdiq gГ¶zlЙ™yir",
    note: expense.note || "",
  });
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card">
        <div className="modal-head">
          <div>
            <h2>XЙ™rc Й™mЙ™liyyatД±nД± redaktЙ™ et</h2>
            <p>{expense.id} ГјzrЙ™ mЙ™blЙ™Дџ, kateqoriya vЙ™ statusu yenilЙ™yin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
        ...
        <form onSubmit={submit} className="modal-form">
          ...
          <label>
            <span>Status</span>
            <select value={values.status} onChange={(event) => updateValue("status", event.target.value)}>
              <option>TЙ™sdiq gГ¶zlЙ™yir</option>
              <option>TЙ™sdiq edildi</option>
              <option>Д°mtina edildi</option>
            </select>
          </label>
          <label className="full">
            <span>Qeyd</span>
...
function SalesOrderModal({ type, onClose, onCreate, orderOptions, defaults = {} }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState(defaults.paymentMethod || "NaДџd");
  ...
  const stockIssues = productRows
    ...
    .map((row) => {
      ...
      return available < requested ? `${row.product}: ${available}/${requested} Й™dЙ™d satД±Еџ qalД±ДџД±` : "";
    })
    .filter(Boolean);
  ...
  return (
    <div className="modal-shell" role="dialog" aria-modal="true">
      <div className="modal-card order-modal-card">
        <div className="modal-head order-modal-head">
          <div>
            <h2>Yeni SatД±Еџ SifariЕџi</h2>
            <p>MГјЕџtЙ™ri, mЙ™hsul vЙ™ satД±cД± bonus faizlЙ™rini daxil edin.</p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="PЙ™ncЙ™rЙ™ni baДџla">
            <X size={18} />
          </button>
          ...
          <section className="order-section">
            ...
            <div className="order-lines">
              {productRows.map((row) => (
                <div className="order-line-grid" key={row.id}>
                  <select
                    aria-label="MЙ™hsul seГ§"
                    value={row.product}
                    onChange={(event) => changeProduct(row.id, "product", event.target.value)}
                  >
                    {availableStock.map((item) => (
                      <option key={item.product} value={item.product}>
                        {item.product} вЂ” {item.total - item.reserved} satД±Еџ ГјГ§Гјn
                      </option>
                    ))}
                  </select>
                  <input
                  ...
                  <button
                    ...
                    aria-label="MЙ™hsul sЙ™trini sil"
                  >
                    <Trash2 size={17} />
                  </button>
                    ...
                    <div className="serial-pick-list">
                      {Array.from({ length: Math.max(1, Number(row.qty || 1)) }).map((_, index) => (
                        <label key={`${row.id}-serial-${index}`}>
                          ...
                          >
                            <option value="">Serial seГ§</option>
          ...
          {paymentMethod === "Kredit" && (
            <section className="order-section credit-order-section">
              ...
              <div className="credit-order-grid">
                ...
                <label className="order-sub-field">
                  <span>Д°LKД°N Г–DЖЏNД°Ећ</span>
                  <input
                    aria-label="Д°lkin Г¶dЙ™niЕџ"
                    type="number"
                    min="0"
                    max={orderTotal}
