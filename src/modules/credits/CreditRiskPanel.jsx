...
export function CreditRiskPanel() {
  ...
  return <Panel className="credit-risk-panel">
    <PanelHeader title="Risk vЙ™ kolleksiya reyestri" subtitle={`${contracts.length} backend mГјqavilЙ™si В· gecikmЙ™, cЙ™rimЙ™ vЙ™ kolleksiya mЙ™rhЙ™lЙ™si`} icon={ShieldAlert}
      action={<button className="secondary-btn" type="button" onClick={recalculate} disabled={loading}><RefreshCw size={16} /> GecikmЙ™ni hesabla</button>} />
    {error && <div className="inline-alert danger">{error.message}</div>}
    <DataTable columns={["MГјqavilЙ™", "MГјЕџtЙ™ri", "ЖЏsas mЙ™blЙ™Дџ", "Risk", "Kolleksiya", "GecikЙ™n taksit"]}
      rows={contracts.slice(0, 20).map((credit) => {
        ...
        return [
          ...
          <TwoLine title={credit.customer?.name || "MГјЕџtЙ™ri"} subtitle={credit.customer?.fin || ""} />,
          money(Number(credit.principal || 0) - Number(credit.initial_payment || 0)),
          <TwoLine title={`${credit.risk_score || 0}/100`} subtitle={credit.last_risk_calculated_at ? "YenilЙ™nib" : "HesablanmayД±b"} />,
          ...
          overdue.length ? <StatusBadge status={`${overdue.length} gecikmЙ™`} /> : "вЂ”",
        ];
      })} />
  </Panel>;
}
