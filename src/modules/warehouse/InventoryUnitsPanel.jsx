...
export function InventoryUnitsPanel({ warehouses = [], products = [], selectedWarehouseId = "all" }) {
  ...
  async function submit(event) {
    ...
    await save(editor);
    setEditor(null);
    setNotice("Stok vahidi vЙ™ lokasiya mЙ™lumatД± saxlanД±ldД±.");
  }
  ...
  return (
    <Panel className="inventory-unit-panel">
      <PanelHeader
        title="Seriya, partiya vЙ™ lokasiya reyestri"
        subtitle={`${visibleUnits.length} stok vahidi В· IMEI, batch, rЙ™f vЙ™ bin izlЙ™mЙ™`}
        icon={Boxes}
      ...
      {loading ? <EmptyState title="Stok vahidlЙ™ri yГјklЙ™nir" /> : (
        <DataTable
          columns={["MЙ™hsul", "Anbar", "IMEI / Serial", "Partiya", "Lokasiya", "Miqdar", "Maya", "Status", "ЖЏmЙ™l"]}
          rows={visibleUnits.map((unit) => [
            <TwoLine title={productById.get(unit.product_id)?.name || "MЙ™hsul"} subtitle={productById.get(unit.product_id)?.sku || unit.product_id} />,
            warehouseById.get(unit.warehouse_id)?.name || unit.warehouse_id,
            <TwoLine title={unit.imei || unit.serial_no || "вЂ”"} subtitle={unit.imei && unit.serial_no ? unit.serial_no : ""} />,
            <TwoLine title={unit.batch_no || "вЂ”"} subtitle={unit.expiry_date ? `Son istifadЙ™: ${unit.expiry_date}` : ""} />,
            [unit.location_code, unit.rack_code, unit.bin_code].filter(Boolean).join(" / ") || "вЂ”",
            unit.quantity,
            money(unit.unit_cost),
            <StatusBadge status={unit.status} />,
            <div className="table-actions">
              <button className="icon-btn" title="RedaktЙ™ et" onClick={() => setEditor({ ...unit })}><Pencil size={15} /></button>
      ...
      {editor && (
        <div className="modal-backdrop" role="presentation">
          <form className="modal-card inventory-unit-modal" onSubmit={submit}>
            <div className="modal-head"><div><h2>{editor.id ? "Stok vahidini redaktЙ™ et" : "Yeni stok vahidi"}</h2><p>Seriya vЙ™ ya partiya izlЙ™mЙ™ mЙ™lumatlarД±nД± daxil edin.</p></div><button type="button" className="icon-btn" onClick={() => setEditor(null)}><X size={18} /></button></div>
            <div className="production-form-grid">
              ...
              <label><span>MЙ™hsul</span><select value={editor.product_id} onChange={(event) => setEditor({ ...editor, product_id: event.target.value })} required>{products.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
              ...
              <label><span>Son istifadЙ™</span><input type="date" value={editor.expiry_date || ""} onChange={(event) => setEditor({ ...editor, expiry_date: event.target.value })} /></label>
              ...
              <label><span>RЙ™f</span><input value={editor.rack_code || ""} onChange={(event) => setEditor({ ...editor, rack_code: event.target.value })} /></label>
            ...
            <div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setEditor(null)}>LЙ™Дџv et</button><button className="primary-btn" type="submit"><Save size={16} /> Saxla</button></div>
          </form>
        </div>
      )}
    </Panel>
